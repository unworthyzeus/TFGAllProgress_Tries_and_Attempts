...
def compute_topology_edge_path_loss_loss(
    outputs: torch.Tensor,
    targets: torch.Tensor,
    masks: torch.Tensor,
    inputs: torch.Tensor,
    ...
    edge_cfg = dict(cfg.get('topology_edge_path_loss', {}))
    if not bool(edge_cfg.get('enabled', False)) or 'path_loss' not in target_columns:
        return torch.tensor(0.0, device=outputs.device)

    loss_weight = float(edge_cfg.get('loss_weight', 0.0))
    if loss_weight <= 0.0:
        return torch.tensor(0.0, device=outputs.device)
    ...
    topo = inputs[:, :1]
    edge_threshold = float(edge_cfg.get('edge_threshold', 0.05))
    edge_boost = float(edge_cfg.get('edge_boost', 2.0))
    dilation = max(1, int(edge_cfg.get('dilation_kernel', 5)))
    if dilation % 2 == 0:
        dilation += 1

    dx = torch.abs(topo[:, :, :, 1:] - topo[:, :, :, :-1])
    dy = torch.abs(topo[:, :, 1:, :] - topo[:, :, :-1, :])
    edge_map = torch.zeros_like(topo)
    edge_map[:, :, :, 1:] = torch.maximum(edge_map[:, :, :, 1:], dx)
    edge_map[:, :, :, :-1] = torch.maximum(edge_map[:, :, :, :-1], dx)
    edge_map[:, :, 1:, :] = torch.maximum(edge_map[:, :, 1:, :], dy)
    edge_map[:, :, :-1, :] = torch.maximum(edge_map[:, :, :-1, :], dy)

    if dilation > 1:
        edge_map = F.max_pool2d(edge_map, kernel_size=dilation, stride=1, padding=dilation // 2)

    edge_mask = (edge_map >= edge_threshold).float()
    weighted_mask = path_mask * (1.0 + edge_boost * edge_mask)
    denom = weighted_mask.sum().clamp_min(1.0)

    pred_db = _denormalize_path_loss_to_db(pred_path, path_meta)
    target_db = _denormalize_path_loss_to_db(target_path, path_meta)
    weighted_mse_db = (((pred_db - target_db) ** 2) * weighted_mask).sum() / denom
    scale = max(float(path_meta.get('scale', 180.0)), 1.0)
    return loss_weight * (weighted_mse_db / (scale ** 2))
        ...
        for batch in tqdm(
            ...
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                if 'path_loss' in target_columns:
                    ...
                    topology_edge_path_loss = compute_topology_edge_path_loss_loss(
                        fake, y, m, x, target_columns, target_metadata, cfg,
                    )
                    n_total = len(target_columns)
                    n_other = n_total - 1
                    if n_other <= 0:
                        recon_loss = path_loss_mse_db + multiscale_path_loss + topology_edge_path_loss
                    else:
                        recon_loss = (
                            (recon_other * n_other + path_loss_mse_db) / n_total
                            + multiscale_path_loss
                            + topology_edge_path_loss
                        )
                else:
