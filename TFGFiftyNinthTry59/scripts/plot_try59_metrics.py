...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGFiftyNinthTry59"
...
def load_rows(output_dir: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(output_dir.glob("validate_metrics_epoch_*.json"), key=_epoch_from_path):
        ...
        row = {
            "epoch": epoch,
            "main_target": str(experiment.get("focused_target", "angular_spread")),
            ...
            "train_main_target_loss": _finite(_nested(train_payload, "loss_components.target_losses.angular_spread", history_row.get("loss_components", {}).get("target_losses", {}).get("angular_spread"))),
            ...
            "val_main_target_loss": _finite(_nested(payload, "_loss.target_losses.angular_spread")),
...
def plot_output_dir(output_dir: Path, save_path: Path | None = None) -> dict[str, Any]:
    ...
    topo = output_dir.name.replace("fiftyninthtry59_", "")
    ...
    axes[0].set_title(f"Try 59 metrics - {topo}")
    axes[0].legend()
    axes[0].grid(alpha=0.25)

    axes[1].plot(epochs, train_main_target_loss, label="train angular target loss", color="#f08c00", linewidth=2.0)
    axes[1].plot(epochs, val_main_target_loss, label="val angular target loss", color="#ffd8a8", linewidth=1.8)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 59 train/val delay and angular spread RMSE trends.")
    parser.add_argument("--output-dir", default="", help="Optional single expert output dir. If omitted, scans --root-dir.")
    parser.add_argument("--root-dir", default=str(DEFAULT_ROOT), help="Root containing downloaded Try59 expert output folders.")
