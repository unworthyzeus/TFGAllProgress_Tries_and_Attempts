...
def _resolve_try59_topology_thresholds(data_cfg: Dict[str, Any]) -> Dict[str, float]:
...
def _infer_try59_topology_class(density: float, height: float, thresholds: Dict[str, float]) -> str:
...
def _compute_try59_topology_metadata(
    raw_topology: np.ndarray,
    thresholds: Dict[str, float],
    *,
    topology_mask_threshold: float = 0.0,
) -> Dict[str, float | str]:
    occupied = raw_topology > float(topology_mask_threshold)
    density = float(np.mean(occupied))
    occupied_vals = raw_topology[occupied]
    mean_height = float(np.mean(occupied_vals)) if occupied_vals.size else 0.0
    return {
        'topology_class': _infer_try59_topology_class(density, mean_height, thresholds),
...
def compute_input_channels(cfg: Dict[str, Any]) -> int:
    ...
    if cfg['data'].get('los_input_column'):
        in_channels += 1
    if cfg['data'].get('topology_mask_channel', False):
        in_channels += 1
...
class CKMHDF5Dataset(Dataset):
    def __init__(
        ...
        topology_mask_channel: bool = False,
        topology_mask_threshold: float = 0.0,
        ...
        self.topology_mask_channel = bool(topology_mask_channel)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.append_no_data_target:
            target_tensors.append(topology_mask_tensor.clone())
        ...
        if self.augment:
            ...
            stack.append(topology_mask_tensor)
            ...
            if los_input_tensor is not None:
                ...
                cursor += 1
            topology_mask_tensor = aug[cursor]
            cursor += 1
            if distance_map_tensor is not None:
        ...
        if self.topology_mask_channel:
            model_input_channels.append(topology_mask_tensor)
        if distance_map_tensor is not None:
        ...
        if self.apply_topology_mask_to_targets:
            ...
            for idx_target, name in enumerate(self.target_columns):
                if name == 'no_data':
                    continue
...
def _resolve_hdf5_scalar_value_static(
    ...
    if column in group.attrs:
        try:
            value = float(np.asarray(group.attrs[column]).reshape(-1)[0])
    ...
    *,
    ...
    with h5py.File(hdf5_path, 'r') as handle:
        for city, sample in refs:
            ...
            meta = _compute_try59_topology_metadata(
                raw_topology,
                topology_thresholds,
                topology_mask_threshold=topology_mask_threshold,
            )
            if meta['topology_class'] == topology_class:
...
def build_dataset_splits_from_config(cfg: Dict[str, Any]) -> Dict[str, Dataset]:
    ...
    if topology_class_filter:
        topology_thresholds = _resolve_try59_topology_thresholds(data_cfg)
        ...
        print(f"[INFO] partition_filter topology_class={topology_class_filter!r} -> samples={len(sample_refs)}")
    ...
    common_hdf5 = dict(
        ...
        topology_mask_channel=bool(data_cfg.get('topology_mask_channel', False)),
        topology_mask_threshold=float(data_cfg.get('topology_mask_threshold', 0.0)),
        apply_topology_mask_to_targets=bool(data_cfg.get('apply_topology_mask_to_targets', False)),
        append_no_data_target=bool(data_cfg.get('append_no_data_target', False)),
    )
    splits = {
