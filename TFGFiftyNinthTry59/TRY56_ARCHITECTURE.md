...
`Try 59` reuses the same base model family as `Try 26`:
- same `UNetGenerator`
...
`Try 59` uses six topology experts instead of one global model. The routing is deterministic and comes from `topology_map` statistics:
- building density
