...
def reduce_running_terms(
    running_terms: Dict[str, float],
    batch_count: int,
    device: Any,
    distributed: bool,
) -> Dict[str, float]:
    if not running_terms:
        return {}
    names = list(running_terms.keys())
    if not distributed:
        denom = max(batch_count, 1)
        return {name: float(running_terms[name] / denom) for name in names}

    stats = torch.tensor(
        [float(running_terms[name]) for name in names] + [float(batch_count)],
        device=device,
        dtype=torch.float64,
    )
    dist.all_reduce(stats, op=dist.ReduceOp.SUM)
    denom = max(float(stats[-1].item()), 1.0)
    return {name: float(stats[idx].item() / denom) for idx, name in enumerate(names)}
...
def build_target_loss_snapshot(
    summary: Dict[str, Dict[str, float]],
    target_columns: List[str],
    target_losses: Dict[str, str],
    target_loss_weights: Dict[str, float],
    ...
    return raw_losses, weighted_losses, loss_kinds
...
def validate_generator(
    ...
    total = 0.0
    gradient_total = 0.0
    gradient_los_total = 0.0
    gradient_nlos_total = 0.0
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc='val', leave=False):
            ...
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                gradient_loss = compute_gradient_regression_loss(pred, y, m, target_columns, target_losses, cfg)
                if los_mask is not None:
                    los_masks = m * los_mask
                    nlos_masks = m * (1.0 - los_mask)
                    gradient_loss_los = compute_gradient_regression_loss(pred, y, los_masks, target_columns, target_losses, cfg)
                    gradient_loss_nlos = compute_gradient_regression_loss(pred, y, nlos_masks, target_columns, target_losses, cfg)
                else:
                    gradient_loss_los = torch.tensor(0.0, device=pred.device)
                    gradient_loss_nlos = torch.tensor(0.0, device=pred.device)
            total += recon.item() + confidence_loss.item()
            gradient_total += float(gradient_loss.item())
            gradient_los_total += float(gradient_loss_los.item())
            gradient_nlos_total += float(gradient_loss_nlos.item())
    ...
    summary['_loss'] = {
        ...
        'gradient_loss': float(gradient_total / max(len(loader), 1)),
        'gradient_loss_LoS': float(gradient_los_total / max(len(loader), 1)),
        'gradient_loss_NLoS': float(gradient_nlos_total / max(len(loader), 1)),
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Train a Try 58 topology expert')
    parser.add_argument('--config', type=str, default='experiments/fiftyeighthtry58_topology_experts/fiftyeighthtry58_expert_open_sparse_lowrise.yaml')
    ...
    generator_beta1 = float(cfg['training']['beta1'])
    generator_beta2 = float(cfg['training']['beta2'])
    ...
    for epoch in range(start_epoch, int(cfg['training']['epochs']) + 1):
        ...
        running_terms = {
            ...
            'multiscale_loss': 0.0,
            'gradient_loss': 0.0,
            'gradient_loss_LoS': 0.0,
            'gradient_loss_NLoS': 0.0,
            'confidence_loss': 0.0,
            'term_recon': 0.0,
            'term_confidence': 0.0,
        ...
        for batch in tqdm(
            ...
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                if 'path_loss' in target_columns:
                    ...
                    if los_mask is not None:
                        gradient_regression_los = compute_gradient_regression_loss(
                            fake, y, m * los_mask, target_columns, target_losses, cfg,
                        )
                        gradient_regression_nlos = compute_gradient_regression_loss(
                            fake, y, m * (1.0 - los_mask), target_columns, target_losses, cfg,
                        )
                    else:
                        gradient_regression_los = torch.tensor(0.0, device=fake.device)
                        gradient_regression_nlos = torch.tensor(0.0, device=fake.device)
                    n_total = len(target_columns)
                    n_other = n_total - 1
                    if n_other <= 0:
                        recon_core = path_loss_mse_db
                    else:
                        recon_core = (recon_other * n_other + path_loss_mse_db) / n_total
                else:
                    recon_core = compute_reconstruction_loss(
                        fake_primary, y, m, target_columns, loss_map, mse_weight, l1_weight, target_loss_weights,
                    )
                    multiscale_regression = compute_multiscale_regression_loss(
                        fake, y, m, target_columns, target_losses, cfg,
                    ...
                    if los_mask is not None:
                        ...
                        gradient_regression_los = torch.tensor(0.0, device=fake.device)
                        gradient_regression_nlos = torch.tensor(0.0, device=fake.device)
                recon_loss = recon_core + multiscale_regression + gradient_regression
                confidence_loss, _confidence_stats = compute_confidence_loss(fake, y, m, target_columns, target_metadata, cfg)
                term_recon = lambda_recon * recon_loss
                term_confidence = confidence_loss
                g_loss = term_recon + term_confidence
            ...
            running_terms['recon_loss'] += float(recon_loss.item())
            running_terms['multiscale_loss'] += float(multiscale_regression.item())
            running_terms['gradient_loss'] += float(gradient_regression.item())
            running_terms['gradient_loss_LoS'] += float(gradient_regression_los.item())
            running_terms['gradient_loss_NLoS'] += float(gradient_regression_nlos.item())
            running_terms['confidence_loss'] += float(confidence_loss.item())
            running_terms['term_recon'] += float(term_recon.item())
            running_terms['term_confidence'] += float(term_confidence.item())
            running_terms['generator_loss_total'] += float(g_loss.item())
            batch_count += 1
        ...
        train_loss_components = reduce_running_terms(
            running_terms,
            batch_count,
            device,
            bool(ddp_context['distributed']),
        ...
        train_target_losses, train_weighted_target_losses, train_target_loss_kinds = build_target_loss_snapshot(
            train_summary,
            target_columns,
            target_losses,
            target_loss_weights,
        ...
        train_loss_components['target_losses'] = dict(train_target_losses)
        train_loss_components['weighted_target_losses'] = dict(train_weighted_target_losses)
        train_loss_components['target_loss_kinds'] = dict(train_target_loss_kinds)
        if ddp_context['distributed']:
            dist.barrier()
        ...
        if ddp_context['is_main_process']:
            ...
            row = {
                ...
                'world_size': int(ddp_context['world_size']),
                'loss_components': copy.deepcopy(train_loss_components),
            }
            ...
            runtime_payload = {
                ...
                'loss_flags': {
                    ...
                    'multiscale_enabled': bool(cfg.get('multiscale_targets', {}).get('enabled', False)),
                    'multiscale_loss_weight': float(cfg.get('multiscale_targets', {}).get('loss_weight', 0.0)),
                    'gradient_enabled': bool(cfg.get('gradient_targets', {}).get('enabled', False)),
                    'gradient_loss_weight': float(cfg.get('gradient_targets', {}).get('loss_weight', 0.0)),
                    'confidence_enabled': bool(is_path_loss_hybrid_enabled(cfg)),
                    'confidence_loss_weight': float(get_path_loss_hybrid_cfg(cfg).get('confidence_loss_weight', 0.0)),
                    'target_loss_weights': dict(target_loss_weights),
                },
            }
        ...
        if control_value == 2:
            ...
            if scheduler_g is not None and hasattr(scheduler_g, 'base_lrs'):
                scheduler_g.base_lrs = [float(cfg['training']['generator_lr']) for _ in getattr(scheduler_g, 'base_lrs', [])]
            epochs_without_improvement = 0
