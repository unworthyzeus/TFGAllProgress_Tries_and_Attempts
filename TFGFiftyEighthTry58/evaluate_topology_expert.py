...
from topology_expert_heuristics import apply_path_loss_confidence_fallback, apply_regression_heuristics
...
def summarize_loader(
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc='eval_model', leave=False):
            inputs, targets, masks, sc = unpack_expert_batch(batch, device)
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                outputs = forward_expert_model(generator, inputs, sc)
            ...
            if 'path_loss' in target_columns:
                ...
                for batch_idx in range(pred_phys.shape[0]):
                    if confidence_np is not None:
                        ...
                        path_loss_preds.append(np.asarray(fallback_outputs['final_path_loss_db'], dtype=np.float32))
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Evaluate a Try 58 topology expert on validation data')
