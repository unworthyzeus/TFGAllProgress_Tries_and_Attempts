...
def load_registry() -> list[dict[str, Any]]:
    registry_path = (
        LOCAL_DIR
        / "experiments"
        / "fiftyeighthtry58_topology_experts"
        / "fiftyeighthtry58_expert_registry.yaml"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upload Try58 and submit the 6 topology experts as 2-GPU jobs with cleanup between jobs.")
    ...
    parser.add_argument("--wipe-try58-outputs", action="store_true", default=False)
    ...
    try:
        ...
        if args.wipe_try58_outputs:
            remote_exec(
                client,
                f"find {REMOTE_DIR}/outputs -mindepth 1 -maxdepth 1 -exec rm -rf {{}} + 2>/dev/null || true; "
                f"rm -f {REMOTE_DIR}/logs_try58_expert_* {REMOTE_DIR}/logs_cleanup_fiftyeighthtry58_*",
                check=False,
            )
        ...
        initial_cleanup = remote_sbatch(
            client,
            f"{sbatch_prefix} cluster/run_fiftyeighthtry58_cleanup_sert2001_1gpu.slurm",
        )
        ...
        for expert in experts:
            ...
            job_id = remote_sbatch(
                ...
                "cluster/run_try58_topology_expert_2gpu.slurm",
            )
            cleanup_after = remote_sbatch(
                client,
                f"{sbatch_prefix} --dependency=afterany:{job_id} cluster/run_fiftyeighthtry58_cleanup_sert2001_1gpu.slurm",
            )
            submitted.append(
                {
