...
OUTPUT_DIR = ROOT / 'experiments' / 'fiftyeighthtry58_topology_experts'
...
def _make_expert_cfg(base_cfg: Dict, topology_class: str) -> Dict:
    ...
    cfg['target_columns'] = ['delay_spread', 'no_data']
    cfg['target_losses'] = {
        'delay_spread': 'mse',
        'no_data': 'bce',
    }
    ...
    gradient_cfg['targets'] = ['delay_spread']
    gradient_cfg['target_weights'] = {'delay_spread': 1.0}
    gradient_cfg['min_valid_ratio'] = 0.5
    gradient_cfg['loss_weight'] = 0.1

    runtime_cfg = cfg.setdefault('runtime', {})
    runtime_cfg['output_dir'] = f'outputs/fiftyeighthtry58_{topology_class}'
    runtime_cfg['resume_checkpoint'] = None

    training_cfg['selection_metrics'] = {'delay_spread.rmse_physical': 1.0}

    cfg.setdefault('experiment', {})
    cfg['experiment'] = {
        'try': '58',
        'family': 'try58_delay_topology_experts',
        'topology_class': topology_class,
        'focused_target': 'delay_spread',
...
def main() -> None:
    ...
    for topology_class in TRY56_TOPOLOGY_CLASSES:
        ...
        out_path = OUTPUT_DIR / f'fiftyeighthtry58_expert_{topology_class}.yaml'
    ...
    registry_path = OUTPUT_DIR / 'fiftyeighthtry58_expert_registry.yaml'
    with registry_path.open('w', encoding='utf-8') as handle:
        yaml.safe_dump({'experts': registry}, handle, sort_keys=False)

    print(f'Generated {len(registry)} Try 58 expert configs in {OUTPUT_DIR}')
