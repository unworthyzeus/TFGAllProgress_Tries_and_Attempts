...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGSixtyFirstTry61"
        ...
        rows.append(
            {
                ...
                "selection_proxy": _finite(_nested(payload, "selection_proxy.composite_nlos_weighted_rmse")),
                ...
                "nlos_focus_loss": _finite(_nested(train_payload, "loss_components.nlos_focus_loss")),
                "term_nlos_focus": _finite(_nested(train_payload, "loss_components.term_nlos_focus")),
...
def plot_output_dir(output_dir: Path, save_path: Path | None = None) -> dict[str, Any]:
    ...
    selection_proxy = [r["selection_proxy"] for r in rows]
    ...
    nlos_focus_loss = [r["nlos_focus_loss"] for r in rows]
    term_nlos_focus = [r["term_nlos_focus"] for r in rows]
    ...
    axes[0].set_title(f"Try 61 metrics - {topo}")
    axes[0].legend()
    ...
    ax1 = axes[1].twinx()
    ax1.plot(epochs, selection_proxy, label="selection proxy", color="#5f3dc4", linestyle="--", linewidth=1.5)
    ...
    axes[3].plot(epochs, nlos_focus_loss, label="nlos focus loss", color="#c2255c", linewidth=1.6)
    ...
    ax3.plot(epochs, term_nlos_focus, label="term nlos focus", color="#ff87b6", linestyle="--", linewidth=1.2)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 61 train/val RMSE trends from validate_metrics_epoch_*.json files.")
    parser.add_argument("--output-dir", default="", help="Optional single expert output dir. If omitted, scans --root-dir.")
    parser.add_argument("--root-dir", default=str(DEFAULT_ROOT), help="Root containing downloaded Try61 expert output folders.")
