# Try 57/56 Training-Dynamics Note

This note documents a recurrent failure mode we observed in `Try 57`, and the
optimizer changes we decided to test in both `Try 57` and `Try 56`.
...
predicts `delay_spread` and `angular_spread` rather than path loss, and it now
...
- whether training collapses, which would indicate that `weight_decay = 0.10`
