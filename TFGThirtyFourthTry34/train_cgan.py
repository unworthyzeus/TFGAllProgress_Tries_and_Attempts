...
def _normalize_channel(values: torch.Tensor, metadata: Dict[str, object]) -> torch.Tensor:
...
def _distance_channel_index(cfg: Dict) -> int:
    index = 1
...
def _formula_input_channel_index(cfg: Dict) -> int:
    index = 1
    if cfg.get('data', {}).get('los_input_column'):
        index += 1
...
def compute_path_loss_prior_map(
    x: torch.Tensor,
    ...
    clip_max = float(path_meta.get('clip_max', clip_min + float(path_meta.get('scale', 180.0))))
    if not bool(prior_cfg.get('enabled', False)):
        return torch.full(
            (x.size(0), 1, x.size(2), x.size(3)),
            fill_value=_normalize_channel(torch.tensor(clip_min, device=x.device, dtype=x.dtype), path_meta).item(),
            device=x.device,
            dtype=x.dtype,
        )

    formula_input_cfg = dict(cfg.get('data', {}).get('path_loss_formula_input', {}))
    if bool(prior_cfg.get('use_formula_input_channel', True)) and bool(formula_input_cfg.get('enabled', False)):
        formula_idx = _formula_input_channel_index(cfg)
        if x.size(1) <= formula_idx:
            raise ValueError(f'Formula input channel index {formula_idx} not available for input with shape {tuple(x.shape)}')
        return x[:, formula_idx : formula_idx + 1].clamp(
            float(_normalize_channel(torch.tensor(clip_min, device=x.device, dtype=x.dtype), path_meta)),
            float(_normalize_channel(torch.tensor(clip_max, device=x.device, dtype=x.dtype), path_meta)),
        )

    if not bool(cfg.get('data', {}).get('distance_map_channel', False)):
        raise ValueError('Try 31 requires data.distance_map_channel = true')

    dist_index = _distance_channel_index(cfg)
    if x.size(1) <= dist_index:
        raise ValueError(f'Distance map channel index {dist_index} not available for input with shape {tuple(x.shape)}')

    distance_norm = x[:, dist_index : dist_index + 1].clamp(0.0, 1.0)
    image_size = int(cfg.get('data', {}).get('image_size', x.size(-1)))
    half = (image_size - 1) / 2.0
    max_dist_pixels = max(half * (2.0 ** 0.5), 1.0)
    meters_per_pixel = float(prior_cfg.get('meters_per_pixel', 1.0))
    frequency_ghz = float(prior_cfg.get('frequency_ghz', 7.125))
    min_distance_m = float(prior_cfg.get('min_distance_m', 1.0))
    min_height_m = float(prior_cfg.get('min_height_m', 1.0))

    ground_distance_m = distance_norm * (max_dist_pixels * meters_per_pixel)
    if scalar_cond is not None and scalar_cond.numel() > 0:
        scalar_names = list(cfg.get('data', {}).get('scalar_feature_columns', []))
        if 'antenna_height_m' in scalar_names:
            scalar_idx = scalar_names.index('antenna_height_m')
            scalar_norms = dict(cfg.get('data', {}).get('scalar_feature_norms', {}))
            antenna_norm = float(scalar_norms.get('antenna_height_m', 120.0))
            antenna_height_m = scalar_cond[:, scalar_idx : scalar_idx + 1].to(x.dtype) * antenna_norm
        else:
            antenna_height_m = torch.full((x.size(0), 1), min_height_m, device=x.device, dtype=x.dtype)
    else:
        antenna_height_m = torch.full((x.size(0), 1), min_height_m, device=x.device, dtype=x.dtype)
    antenna_height_m = antenna_height_m.clamp(min=min_height_m).view(x.size(0), 1, 1, 1)
    d_3d_m = torch.sqrt(ground_distance_m ** 2 + antenna_height_m ** 2).clamp(min=min_distance_m)

    prior_db = 20.0 * torch.log10(d_3d_m) + 20.0 * torch.log10(torch.tensor(frequency_ghz, device=x.device, dtype=x.dtype)) + 32.45
    prior_db = prior_db.clamp(min=clip_min, max=clip_max)
    prior_norm = _normalize_channel(prior_db, path_meta)
    return prior_norm.clamp(float(_normalize_channel(torch.tensor(clip_min, device=x.device, dtype=x.dtype), path_meta)), float(_normalize_channel(torch.tensor(clip_max, device=x.device, dtype=x.dtype), path_meta)))
...
def decode_generator_outputs(
    ...
    prior_cfg = dict(cfg.get('prior_residual_path_loss', {}))
    if bool(prior_cfg.get('enabled', False)) and 'path_loss' in target_columns:
        idx = target_columns.index('path_loss')
        residual_pred = primary[:, idx : idx + 1]
        prior_norm = compute_path_loss_prior_map(x, scalar_cond, target_metadata, cfg)
        final_path = residual_pred + prior_norm
        if bool(prior_cfg.get('clamp_final_output', True)):
            path_meta = target_metadata.get('path_loss', {})
            clip_min = float(path_meta.get('clip_min', path_meta.get('offset', 0.0)))
            clip_max = float(path_meta.get('clip_max', clip_min + float(path_meta.get('scale', 180.0))))
            final_path = final_path.clamp(
                float(_normalize_channel(torch.tensor(clip_min, device=final_path.device, dtype=final_path.dtype), path_meta)),
                float(_normalize_channel(torch.tensor(clip_max, device=final_path.device, dtype=final_path.dtype), path_meta)),
            )
        primary = primary.clone()
        primary[:, idx : idx + 1] = final_path
        aux['path_loss_prior'] = prior_norm
        aux['path_loss_residual'] = residual_pred
    return primary, aux


def compute_prior_residual_path_loss_loss(
    outputs: torch.Tensor,
    ...
    prior_cfg = dict(cfg.get('prior_residual_path_loss', {}))
    if not bool(prior_cfg.get('enabled', False)) or 'path_loss' not in target_columns:
        return torch.tensor(0.0, device=outputs.device)

    idx = target_columns.index('path_loss')
    residual_pred = outputs[:, idx : idx + 1]
    prior_norm = compute_path_loss_prior_map(x, scalar_cond, target_metadata, cfg)
    residual_target = targets[:, idx : idx + 1] - prior_norm
    path_mask = masks[:, idx : idx + 1]
    denom = path_mask.sum().clamp_min(1.0)
    mse = (((residual_pred - residual_target) ** 2) * path_mask).sum() / denom
    l1 = (torch.abs(residual_pred - residual_target) * path_mask).sum() / denom
    loss_weight = float(prior_cfg.get('loss_weight', 0.35))
    mse_weight = float(prior_cfg.get('mse_weight', 1.0))
    l1_weight = float(prior_cfg.get('l1_weight', 0.2))
...
def compute_multiscale_path_loss_loss(
    ...
    if not bool(ms_cfg.get('enabled', False)) or 'path_loss' not in target_columns:
        return torch.tensor(0.0, device=outputs.device)
    ...
    pred_db = _denormalize_path_loss_to_db(pred_path, path_meta)
    target_db = _denormalize_path_loss_to_db(target_path, path_meta)
    scale_db = max(float(path_meta.get('scale', 180.0)), 1.0)
    ...
    for factor, weight in zip(scales, raw_weights):
        ...
        mask_ds = F.avg_pool2d(path_mask, kernel_size=factor, stride=factor)
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc='val', leave=False):
            ...
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                primary_pred, _decoded = decode_generator_outputs(pred, x, sc, target_columns, target_metadata, cfg)
...
def main() -> None:
    ...
    disc_base_channels = int(cfg['model'].get('disc_base_channels', max(8, int(cfg['model']['base_channels']) // 8)))
            ...
            if gan_enabled:
                ...
                with torch.no_grad():
                    with amp.autocast(device_type='cuda', enabled=amp_enabled):
                        ...
                        fake_primary, _decoded = decode_generator_outputs(fake, x, sc, target_columns, target_metadata, cfg)
            ...
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                if 'path_loss' in target_columns:
                    ...
                    path_loss_mse_db = compute_path_loss_mse_db_direct(
                        fake_primary, y, m, target_columns, target_metadata, cfg,
                    )
                    multiscale_path_loss = compute_multiscale_path_loss_loss(
                        fake_primary, y, m, target_columns, target_metadata, cfg,
                    )
                    prior_residual_path_loss = compute_prior_residual_path_loss_loss(
                        fake, y, m, x, sc, target_columns, target_metadata, cfg,
                    )
                    n_total = len(target_columns)
                    n_other = n_total - 1
                    if n_other <= 0:
                        recon_loss = path_loss_mse_db + multiscale_path_loss + prior_residual_path_loss
                    else:
                        recon_loss = (recon_other * n_other + path_loss_mse_db) / n_total + multiscale_path_loss + prior_residual_path_loss
                else:
