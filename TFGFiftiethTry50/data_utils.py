...
def _a2g_nlos_excess_path_loss_db(
    d2d_m: torch.Tensor,
    h_tx: float,
    h_rx: float,
    freq_ghz: float,
    *,
    bias_db: float = -16.16,
    amp_db: float = 12.0436,
    tau_deg: float = 7.52,
) -> torch.Tensor:
    theta_deg = _elevation_angle_deg(d2d_m, h_tx, h_rx)
    lambda0_db = 20.0 * math.log10((4.0 * math.pi * h_tx * freq_ghz * 1.0e9) / 299792458.0)
    tau_deg = max(float(tau_deg), 1e-3)
    return lambda0_db + (float(bias_db) + float(amp_db) * torch.exp(-(90.0 - theta_deg) / tau_deg))
...
def _vinogradov_height_nlos_mean_path_loss_db(
    ...
    params = {
        "open_lowrise": {"n0": 4.2, "n_inf": 2.65, "h0": 16.0},
        "mixed_midrise": {"n0": 4.71, "n_inf": 2.82, "h0": 42.0},
        "dense_highrise": {"n0": 4.62, "n_inf": 2.80, "h0": 91.0},
    }
...
def _vinogradov_height_nlos_mean_path_loss_db_relaxed(
    ...
    params = {
        "open_lowrise": {"n0": 4.05, "n_inf": 2.55, "h0": 24.0},
        "mixed_midrise": {"n0": 4.45, "n_inf": 2.72, "h0": 60.0},
        "dense_highrise": {"n0": 4.55, "n_inf": 2.80, "h0": 120.0},
    }
    p = params.get(city_type, params["mixed_midrise"])
    n_h = p["n_inf"] + (p["n0"] - p["n_inf"]) * math.exp(-max(h_tx, 1.0) / p["h0"])
    return lambda0_db + 10.0 * n_h * torch.log10(d3d_m.clamp(min=1.0))
...
def _compute_formula_path_loss_db(
    ...
    if mode == 'two_ray_ground':
        ...
        a2g_nlos_db = _a2g_nlos_excess_path_loss_db(
            d2d_m,
            h_tx,
            h_rx,
            freq_ghz,
        ...
        nlos_floor_db = torch.maximum(
            ...
            los_path_db + float(a2g_params.get("nlos_los_margin_db", 6.0)),
        )
        nlos_path_db = torch.maximum(nlos_path_db, nlos_floor_db)
        if topology_tensor is not None:
            ...
            nlos_path_db = nlos_path_db + float(a2g_params.get("nlos_obstruction_scale", 0.30)) * obstruction_db
        ...
        nlos_base_db = torch.maximum(
            _vinogradov_height_nlos_mean_path_loss_db(d3d_m, h_tx, freq_ghz, city_type),
            _a2g_nlos_excess_path_loss_db(
                d2d_m,
                h_tx,
                h_rx,
                freq_ghz,
                bias_db=float(a2g_params.get("nlos_bias", -16.16)),
                amp_db=float(a2g_params.get("nlos_amp", 12.0436)),
                tau_deg=float(a2g_params.get("nlos_tau", 7.52)),
            ),
        )
        nlos_base_db = torch.maximum(
            nlos_base_db,
        ...
        nlos_base_db = torch.maximum(
            _vinogradov_height_nlos_mean_path_loss_db_relaxed(d3d_m, h_tx, freq_ghz, city_type),
            _a2g_nlos_excess_path_loss_db(
                d2d_m,
                h_tx,
                h_rx,
...
class CKMHDF5Dataset(Dataset):
    ...
    def _load_precomputed_formula_input(self, city: str, sample: str) -> Optional[torch.Tensor]:
        ...
        try:
            grp = handle[city][sample]
            ds = grp["formula_norm_f16"]
        except KeyError:
            return None
        arr = np.asarray(ds[...], dtype=np.float16)
        if arr.ndim == 2:
            arr = arr[None, ...]
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if bool(formula_cfg.get('enabled', False)):
            ...
            if precomputed_formula is not None:
                ...
                try:
                    formula_input_tensor = torch.load(cache_path, map_location='cpu', weights_only=False)
                except TypeError:
                    formula_input_tensor = torch.load(cache_path, map_location='cpu')
            else:
                ...
                if cache_path is not None:
                    ...
                    try:
                        torch.save(formula_input_tensor.cpu(), tmp_path)
