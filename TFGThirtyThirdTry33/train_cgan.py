        ...
        for batch in tqdm(loader, desc='val', leave=False):
            ...
            if is_path_loss_hybrid_enabled(cfg) and 'path_loss' in target_columns:
                ...
                if confidence_logits is not None:
                    ...
                    valid_mask_np = m[:, path_loss_idx : path_loss_idx + 1].detach().cpu().numpy()
                    ...
                    for batch_idx in range(pred_db.shape[0]):
                        ...
                        fused_path_loss_preds.append(np.where(valid_mask > 0, np.asarray(fallback_outputs['final_path_loss_db'], dtype=np.float32), np.nan))
                        fused_path_loss_targets.append(np.where(valid_mask > 0, np.asarray(tgt_db[batch_idx, 0], dtype=np.float32), np.nan))
        ...
        for batch in tqdm(
            ...
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                if 'path_loss' in target_columns:
                    ...
                    n_other = n_total - 1
                    if n_other <= 0:
                        recon_loss = path_loss_mse_db + multiscale_path_loss
                    else:
                        recon_loss = (recon_other * n_other + path_loss_mse_db) / n_total + multiscale_path_loss
                else:
