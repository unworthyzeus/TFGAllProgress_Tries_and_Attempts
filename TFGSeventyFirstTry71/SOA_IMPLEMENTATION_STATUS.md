# Try 71 — SOA claims vs. what is implemented

> Companion to **`SOA_COMPARISON.md`**. This file tracks which *thesis / SOA narrative* items are **already in the Try 71 codebase** versus **not implemented, partial, or config-only**.  
...
| **Topology-partitioned experts** (6 classes + registry) | **Implemented** | `try71_expert_registry.yaml`, `partition_filter.topology_class`, separate checkpoints |
...
| **Topology classifier** (Try 71) | **Separate script** | `train_topology_classifier.py` + `try71_topology_classifier.yaml` — not the same run as path-loss experts |
...
| **Test-time adaptation / calibration** (cf. RadioPiT, AIRMap-style) | **Not implemented** | Explicitly “No” in SOA fair-comparison table for Try 71 |
...
These are cited in **`SOA_COMPARISON.md` §7** as *future* ideas — **none** are integrated as first-class Try 71 features today:
