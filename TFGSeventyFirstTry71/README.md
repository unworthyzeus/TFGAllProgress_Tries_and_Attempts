# Try 71 — Try 66 re-run with PMHHNet stem + HF fix

`Try 71` reuses the **Try 66** pipeline (6 topology-partitioned experts, same configs and training code layout). Intentional code differences from the Try 66 fork:
...
Bootstrap: `TFGpractice/scripts/bootstrap_try68_from_try66.py`. Cluster resume: `cluster/launch_try71_resume_from_try68.py` (sync `outputs/try66_expert_*` → `try71_expert_*`); when submitting after a sync, use **`submit_try71_experts_4gpu_sequential.py --no-clean-outputs`** so the uploader does not delete resumed checkpoints.
...
Full changelog: `TFGpractice/VERSIONS.md` (Try 66 architecture note, Try 71 section).

---

The remainder of this README still describes the **Try 66 synthesis** design (literature, inputs, losses) that Try 71 inherits unchanged.
...
## What is new in Try 71 — Paper-backed additions
...
- **NLoS pixel reweighting**: Try 61 used *regime-level* reweighting (by city type),
  which was too coarse. Try 71 uses *pixel-level* NLoS reweighting (2.5× on NLoS
...
The target for Try 71 is to push per-expert RMSE lower than Try 64 while
...
- `cluster/run_seventyfirst_try71_4gpu.slurm` — Cluster entry
- `cluster/submit_try71_stage1_stage2_4gpu.py` — Cluster submission (sequential 6-expert chain)
...
```powershell
C:\TFG\.venv\Scripts\python.exe C:\TFG\TFGpractice\TFGSeventyFirstTry71\scripts\generate_try68_configs.py
```
...
```powershell
$env:SSH_PASSWORD = '***'
python C:\TFG\TFGpractice\TFGSeventyFirstTry71\cluster\submit_try71_stage1_stage2_4gpu.py
```
