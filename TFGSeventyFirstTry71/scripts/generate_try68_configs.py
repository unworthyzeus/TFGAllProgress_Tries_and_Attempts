"""Generate Try 71 configs: 6 topology experts (single-stage 513x513) + classifier.
...
def _build_expert(cfg: dict, spec: dict, model_spec: dict) -> dict:
    ...
    # --- Training recipe (matches try71_expert_*.yaml in repo) ---
    t = cfg["training"]
    t["batch_size"] = 1
...
def main() -> None:
    ...
    expert_dir = root / "experiments" / "seventyfirst_try71_experts"
    cl_dir = root / "experiments" / "seventyfirst_try71_classifier"
    expert_dir.mkdir(parents=True, exist_ok=True)
    cl_dir.mkdir(parents=True, exist_ok=True)

    _cleanup(expert_dir, "try71_expert_*.yaml")
    ...
    for spec in EXPERT_SPECS:
        ...
        name = f"try71_expert_{spec['expert_id']}"
    ...
    cl_cfg = {
        ...
        "runtime": {
            "device": "cuda",
            "output_dir": "outputs/try71_topology_classifier",
            "resume_checkpoint": "",
        },
    }
    cl_path = cl_dir / "try71_topology_classifier.yaml"
    with cl_path.open("w", encoding="utf-8") as f:
        yaml.safe_dump(cl_cfg, f, sort_keys=False)

    with (expert_dir / "try71_expert_registry.yaml").open("w", encoding="utf-8") as f:
