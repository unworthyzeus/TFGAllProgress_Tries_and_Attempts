...
def heteroscedastic_nll_loss(
    ...
    *,
    ...
    """Kendall & Gal (NeurIPS 2017) heteroscedastic NLL loss on masked residual pixels.

    L = mean[(mu - y)^2 / exp(lv) + lv]  over valid pixels.

    Both mu_residual and res_target are in normalized path-loss space (divided by scale).
    log_var is clamped to [lv_min, lv_max] before exponentiation.
    """
    lv = log_var.clamp(lv_min, lv_max)
    nll = (mu_residual - res_target) ** 2 / lv.exp() + lv
...
def _compute_coverage_rmse_curve(
    sigma_vals: "list[torch.Tensor]",
    sq_err_vals: "list[torch.Tensor]",
    n_thresholds: int = 20,
) -> "list[dict[str, float]]":
    """Build the RMSE-vs-coverage curve from accumulated (sigma, sq_err) tensors.

    sigma_vals: list of 1D CPU tensors of per-pixel sigma (physical dB units).
    sq_err_vals: list of 1D CPU tensors of per-pixel squared errors (physical dB² units).
    n_thresholds: number of tau thresholds (evenly spaced in percentile space).

    Returns list of dicts [{tau, coverage, rmse_phys}] sorted by descending tau
    (100% coverage first, down to the most selective threshold).
    """
    if not sigma_vals:
        return []
    all_sigma = torch.cat(sigma_vals)  # [N]
    all_sq_err = torch.cat(sq_err_vals)  # [N]
    n_total = all_sigma.numel()
    if n_total == 0:
        return []

    sorted_sigma, sort_idx = all_sigma.sort()
    sorted_sq_err = all_sq_err[sort_idx]

    # Cumulative sum from sorted → at each cut, the prefix is the "covered" set
    cum_sq_err = torch.cumsum(sorted_sq_err, dim=0)

    # Threshold percentiles: from 100% down to ~5%
    pcts = [1.0 - i / (n_thresholds - 1) * 0.95 for i in range(n_thresholds)]
    curve: list[dict[str, float]] = []
    for pct in pcts:
        # Cover the pct-fraction of pixels with the LOWEST sigma
        k = max(1, int(round(pct * n_total)))
        tau = float(sorted_sigma[k - 1].item())
        rmse = float(math.sqrt(float(cum_sq_err[k - 1].item()) / k))
        curve.append({"tau_sigma_db": round(tau, 4), "coverage": round(k / n_total, 4), "rmse_phys_db": round(rmse, 4)})
...
def evaluate_validation(
    ...
    *,
    ...
    # Heteroscedastic coverage-curve accumulators
    _hetero_mode = str(cfg.get("training", {}).get("generator_objective", "legacy")).lower() == "heteroscedastic"
    _hetero_cfg = dict(cfg.get("heteroscedastic", {}))
    _lv_min = float(_hetero_cfg.get("log_var_clamp_min", -10.0))
    _lv_max = float(_hetero_cfg.get("log_var_clamp_max", 10.0))
    _n_cov_thresholds = int(_hetero_cfg.get("coverage_thresholds", 20))
    _scale = float(meta.get("scale", 180.0))
    _cov_sigma_list: list[torch.Tensor] = []
    _cov_sq_err_list: list[torch.Tensor] = []
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(loader, desc="val", leave=False, disable=distributed and not is_main_process(rank))):
            ...
            # Heteroscedastic coverage-curve accumulation
            if _hetero_mode and no_data_logits is not None:
                lv_clamped = no_data_logits.clamp(_lv_min, _lv_max)
                # sigma in normalized space → physical dB = sigma_norm * scale
                sigma_phys = (0.5 * lv_clamped).exp() * _scale  # [B,1,H,W]
                sq_err_phys = ((pred_phys - target_phys) ** 2)    # [B,1,H,W]
                _cov_sigma_list.append(sigma_phys[valid_mask].cpu().float())
                _cov_sq_err_list.append(sq_err_phys[valid_mask].cpu().float())
    ...
    if _hetero_mode and _cov_sigma_list:
        summary["coverage_rmse_curve"] = _compute_coverage_rmse_curve(
            _cov_sigma_list, _cov_sq_err_list, n_thresholds=_n_cov_thresholds
        )
    return summary
    ...
    *,
    ...
    for batch in tqdm(loader, desc="train", leave=False, disable=distributed and not is_main_process(rank)):
        ...
        with amp.autocast(device_type="cuda", enabled=amp_enabled):
            ...
            if objective_mode not in ("full_map_rmse_only", "heteroscedastic"):
                weighted_mask = _apply_regime_reweighting(weighted_mask, x, cfg, city_type_resolver)

            if objective_mode in ("full_map_rmse_only", "heteroscedastic"):
            ...
            if objective_mode == "heteroscedastic":
                hetero_cfg = dict(cfg.get("heteroscedastic", {}))
                lv_min = float(hetero_cfg.get("log_var_clamp_min", -10.0))
                lv_max = float(hetero_cfg.get("log_var_clamp_max", 10.0))
                nll_weight = float(hetero_cfg.get("nll_weight", 1.0))
                # log_var is output channel 1, surfaced via no_data_logits
                log_var = no_data_logits if no_data_logits is not None else torch.zeros_like(pred)
                main_map_reg = heteroscedastic_nll_loss(
                    residual_pred, residual_target, log_var, mask, lv_min=lv_min, lv_max=lv_max
                )
                ms_loss = compute_multiscale_path_loss_loss(pred, target, mask, meta, cfg)
                ms_scaled = ms_loss * full_map_scale
                g_loss = nll_weight * main_map_reg + ms_scaled
            ...
            elif objective_mode == "full_map_rmse_only":
                if loss_type == "huber":
...
def main() -> None:
    ...
    if resume_path and resume_path.exists():
        ...
        _hetero_resume = str(cfg.get("training", {}).get("generator_objective", "legacy")).lower() == "heteroscedastic"
        _resume_missing, _resume_unexpected = target_generator.load_state_dict(
            _checkpoint_model_state(state), strict=not _hetero_resume
        )
        if _hetero_resume and is_main_process(rank) and (_resume_missing or _resume_unexpected):
            print(f"[resume heteroscedastic] missing={len(_resume_missing)} unexpected={len(_resume_unexpected)} (expected: new log_var head keys missing)")
