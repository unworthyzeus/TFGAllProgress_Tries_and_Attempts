# DataLoader, batch y gradient accumulation (Try 71)

Los expertos están en `experiments/seventyfirst_try71_experts/try71_expert_*.yaml`.
...
|------|--------|------|---------|
| 1 | `cluster/run_seventyfirst_try71_1gpu.slurm` | 6 | 38G |
| 2 | `cluster/run_seventyfirst_try71_2gpu.slurm` | 24 | 78G |
| 3 | `cluster/run_seventyfirst_try71_3gpu_medium_gpu.slurm` (`qos=medium_gpu`) | 24 | 120G |
| 4 | `cluster/run_seventyfirst_try71_4gpu.slurm` | 32 | 120G |
...
Subida + cancelar todos tus jobs + encolar cadena 3-GPU: desde `TFGpractice/`, `python cluster/relaunch_try68_3gpu_medium_upload.py --cancel-all-user-jobs` (requiere `SSH_PASSWORD` o `--ssh-key`).

## Baseline actual en repo (expertos Try 71)
