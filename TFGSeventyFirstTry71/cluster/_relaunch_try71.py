"""Cancel all jobs, wipe Try 71 logs (keep checkpoints), upload code, and relaunch all experts."""
...
def main() -> None:
    ...
    run(
        c,
        f"rm -f {REMOTE_DIR}/logs_train_try71_*.out {REMOTE_DIR}/logs_train_try71_*.err "
        f"{REMOTE_DIR}/logs_train_try71_chain_*.out {REMOTE_DIR}/logs_train_try71_chain_*.err",
    )

    # 3. Upload code
    print("\n=== Step 3: Upload code ===")
    run(c, f"mkdir -p {REMOTE_DIR}/experiments/seventyfirst_try71_experts")
    run(c, f"mkdir -p {REMOTE_DIR}/experiments/seventyfirst_try71_classifier")
    ...
    config = "experiments/seventyfirst_try71_experts/try71_expert_open_sparse_lowrise.yaml"
    job_name = "t68-open-sparse-lowrise"
    cleanup_script = "cluster/run_seventyfirst_try71_cleanup_sert2001_1gpu.slurm"
    submitted = []
    current_dep = ""
    ...
    for run_idx in range(3):
        ...
        cmd = (
            ...
            "cluster/run_seventyfirst_try71_3gpu.slurm"
        )
        out = run(c, cmd)
