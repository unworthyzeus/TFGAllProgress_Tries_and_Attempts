#!/usr/bin/env python3
"""Upload Try 71, optionally cancel all user Slurm jobs, submit chained 4-GPU jobs for open_sparse_lowrise.

Each job uses ``cluster/run_seventyfirst_try71_4gpu.slurm`` (4× RTX2080, 4:00:00 walltime). Jobs are
...
``outputs/try71_expert_open_sparse_lowrise/`` (path relative to the repo on the cluster). Each
...
LOCAL_DIR = ROOT / "TFGSeventyFirstTry71"
REMOTE_DIR = "/scratch/nas/3/gmoreno/TFGpractice/TFGSeventyFirstTry71"
HOST = "sert.ac.upc.edu"
USER = "gmoreno"
TARGET_NODE = "sert-2001"
DEFAULT_CONFIG = "experiments/seventyfirst_try71_experts/try71_expert_open_sparse_lowrise.yaml"
DEFAULT_RESUME_CHECKPOINT = "outputs/try71_expert_open_sparse_lowrise/best_model.pt"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Submit N sequential Try71 open_sparse_lowrise 4-GPU jobs.")
    ...
    try:
        ...
        for i in range(args.count):
            ...
            cmd = (
                ...
                "cluster/run_seventyfirst_try71_4gpu.slurm"
            )
