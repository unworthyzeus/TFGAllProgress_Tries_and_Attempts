...
def _compose_final_prediction(
    ...
    city_type = city_type_resolver.infer(input_batch) if city_type_resolver is not None else "mixed_midrise"
    _, nlos = _los_nlos_masks(input_batch, cfg)
    if _model_arch(cfg) == "city_routed_nlos_moe":
        selected_delta = _select_city_expert_map(raw_output, city_type)
...
def train_one_epoch(
    ...
    *,
    ...
    for batch in tqdm(loader, desc="train", leave=False):
        ...
        if bandit_reweighter is not None:
            bandit_reweighter.update(city_type, float(g_loss.item()))
        steps += 1
    denom = max(steps, 1)
