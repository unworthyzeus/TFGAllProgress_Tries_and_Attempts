# Try 67 — Implementation Details

Companion to `TRY67_DESIGN.md`. That doc explains *why*; this one documents
*where the code lives* and *how to enable each feature*. Use this as the
review checklist when porting a feature forward to Try 68+.
...
- `scripts/generate_try67_configs.py` — writes the `knife_edge_channel`
...
- `train_partitioned_pathloss_expert.py`:
  ...
  - `_compose_residual_prediction_with_aux(..., cfg=cfg)` — when dual
    ...
    shift to index 2+.
- `scripts/generate_try67_configs.py` — sets `model.out_channels = 2`
...
## 5. City-type partition (generalisable routing)

### Files
- `data_utils.py::_filter_hdf5_refs_by_partition` — accepts
  `city_type: Optional[str]`, routes through
  `_city_type_from_thresholds(meta, thresholds)` where thresholds come
  from `prior_calibration/regime_obstruction_train_only_from_try47.json`
  (`city_type_thresholds` block).
- `scripts/generate_try67_configs.py` — one expert per city_type:
  `open_lowrise`, `mixed_midrise`, `dense_highrise`.
...
data:
  partition_filter:
    city_type: open_lowrise       # or mixed_midrise / dense_highrise
```
...
### Files
- Generator writes `target_metadata.path_loss.clip_{min,max}_db` in
  every expert YAML.
- `train_partitioned_pathloss_expert.py` already clamps final predictions
  to this range (existing code path from Try 51).
...
Set from 99.99th-percentile of the target distribution within each
city class.
...
## 8. Plotter (`scripts/plot_try67_metrics.py`)

Renamed from the Try 66 plotter. Differences:

- `DEFAULT_ROOT = .../TFGSixtySeventhTry67`.
- `EXPERT_PREFIX = sixtyseventh_try67_expert_` (old prefixes accepted
...
### Usage
```bash
python scripts/plot_try67_metrics.py
python scripts/plot_try67_metrics.py --expert-id open_lowrise
python scripts/plot_try67_metrics.py --root-dir D:/cluster_outputs/TFGSixtySeventhTry67
```
...
Not implemented in Try 67, but the codebase has stubs / easy seams:
...
- **Cross-regime distillation** — the city-type registry at
  `experiments/sixtyseventh_try67_experts/try67_expert_registry.yaml`
