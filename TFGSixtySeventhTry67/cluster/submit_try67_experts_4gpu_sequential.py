#!/usr/bin/env python3
"""Submit chained 4-GPU Slurm jobs for Try67 experts (one job per expert, sequential).

Each job uses ``cluster/run_sixtyseventh_try67_4gpu.slurm`` (4 GPUs, 4:00:00 wall-time).
Experts are resolved from ``experiments/sixtyseventh_try67_experts/try67_expert_registry.yaml``.
...
LOCAL_DIR = ROOT / "TFGSixtySeventhTry67"
REGISTRY = LOCAL_DIR / "experiments" / "sixtyseventh_try67_experts" / "try67_expert_registry.yaml"
REMOTE_DIR = "/scratch/nas/3/gmoreno/TFGpractice/TFGSixtySeventhTry67"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try67 4-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
        help=(
            "expert_id values from try67_expert_registry.yaml. "
    ...
    try:
        ...
        if args.wipe_outputs:
            for row in rows:
                ...
                out_dir = f"{REMOTE_DIR}/outputs/try67_expert_{expert_id}"
        ...
        for i, row in enumerate(rows):
            ...
            job_name = f"t67-{expert_id.replace('_', '-')[:28]}"
            exports = [
            ...
            train_cmd = (
                ...
                "cluster/run_sixtyseventh_try67_4gpu.slurm"
            )
            ...
            cleanup_name = f"t67-cleanup-{expert_id.replace('_', '-')[:20]}"
            cleanup_cmd = (
                f"{sbatch_prefix} --dependency=afterany:{train_job_id} "
                f"-J {cleanup_name} "
                "cluster/run_sixtyseventh_try67_cleanup_sert2001_1gpu.slurm"
            )
