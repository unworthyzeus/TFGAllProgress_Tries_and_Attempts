"""Plot Try 67 training metrics from validate_metrics_epoch_*.json files.

Reads the Try 67 JSON schema (single-stage 513x513, dual LoS/NLoS head,
...
Usage:
  ...
  python scripts/plot_try67_metrics.py --root D:/cluster_outputs/TFGSixtySeventhTry67
...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGSixtySeventhTry67"
EXPERT_PREFIX = "sixtyseventh_try67_expert_"
...
def plot_expert(expert_id: str, output_dir: Path, save_path: Path | None = None) -> dict[str, Any]:
    ...
    ax.set_title(f"Try 67 — {topo} — Overall RMSE" + (" [EMA for val]" if uses_ema else ""))
    ax.grid(alpha=0.25)
    ...
    ax.set_ylabel("RMSE (dB)")
    ax.set_title("LoS vs NLoS RMSE (Try 67 dual-head target)")
    ax.grid(alpha=0.25)
    ...
    ax.set_ylabel("Loss")
    ax.set_title("Loss Components + Learning Rate (Try 67: +PDE residual)")
    ax.grid(alpha=0.25)
    lines_a, labels_a = ax.get_legend_handles_labels()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 67 single-stage metrics per expert.")
