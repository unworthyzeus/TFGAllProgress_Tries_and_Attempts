...
def _augmentation_kwargs(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        # Reciprocity / symmetry style: transpose (reflection across main diagonal) in sync on all stacked maps.
        'transpose_prob': float(aug_cfg.get('transpose_prob', 0.0)),
        # Height robustness (HDF5: recomputes formula / spatial height channels; manifest: jitters scalar only).
        'antenna_height_jitter_half_width_m': float(aug_cfg.get('antenna_height_jitter_half_width_m', 0.0)),
        'antenna_height_jitter_prob': float(aug_cfg.get('antenna_height_jitter_prob', 0.25)),
        'antenna_height_jitter_min_m': float(aug_cfg.get('antenna_height_jitter_min_m', 12.0)),
        'antenna_height_jitter_max_m': float(aug_cfg.get('antenna_height_jitter_max_m', 478.0)),
    }
...
def _compute_formula_path_loss_db(
    ...
    # Match device of batch tensors (e.g. LoS channel on CUDA during train); distance map starts on CPU.
    _ref = los_tensor if los_tensor is not None else topology_tensor
    if isinstance(_ref, torch.Tensor):
        _dev = _ref.device
        d2d_m = d2d_m.to(device=_dev, dtype=torch.float32)
        d3d_m = d3d_m.to(device=_dev, dtype=torch.float32)
...
def _filter_hdf5_refs_by_partition(
    ...
    *,
    ...
    if not topology_class and not antenna_bin and not city_type:
        return refs
    ...
    with h5py.File(hdf5_path, "r") as handle:
        for city, sample in refs:
            ...
            if antenna_bin and meta["antenna_bin"] != antenna_bin:
                continue
            if city_type and meta["city_type"] != city_type:
                continue
    ...
    *,
    ...
    if model_type in {"regime_obstruction_linear_v1", "regime_obstruction_multiscale_v1"}:
        ...
        for kernel_size in kernel_sizes:
            ...
            los_mask = (base_los_tensor <= 0.5).to(dtype=torch.float32)
            while los_mask.ndim < 4:
                los_mask = los_mask.unsqueeze(0)
            while los_mask.ndim > 4:
...
def compute_input_channels(cfg: Dict[str, Any]) -> int:
    ...
    if bool(cfg['data'].get('building_mask_channel', False)):
        in_channels += 1
    if bool(cfg['data'].get('knife_edge_channel', {}).get('enabled', False)):
        in_channels += 1
...
class CKMDataset(Dataset):
    def __init__(
        ...
        antenna_height_jitter_half_width_m: float = 0.0,
        antenna_height_jitter_prob: float = 0.25,
        antenna_height_jitter_min_m: float = 12.0,
        antenna_height_jitter_max_m: float = 478.0,
        ...
        self.transpose_prob = float(transpose_prob)
        self.antenna_height_jitter_half_width_m = float(antenna_height_jitter_half_width_m)
        self.antenna_height_jitter_prob = float(antenna_height_jitter_prob)
        self.antenna_height_jitter_min_m = float(antenna_height_jitter_min_m)
        self.antenna_height_jitter_max_m = float(antenna_height_jitter_max_m)
    ...
    def _apply_sync_aug(self, images: List[Image.Image]) -> List[Image.Image]:
        ...
        if self.transpose_prob > 0.0 and random.random() < self.transpose_prob:
            images = [img.transpose(Image.TRANSPOSE) for img in images]
        return images
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.add_scalar_channels or self.return_scalar_cond:
            for col in self.scalar_feature_columns:
                ...
                if (
                    col == "antenna_height_m"
                    and self.augment
                    and self.antenna_height_jitter_half_width_m > 0.0
                    and random.random() < self.antenna_height_jitter_prob
                ):
                    hw = float(self.antenna_height_jitter_half_width_m)
                    value = value + random.uniform(-hw, hw)
                    value = max(
                        self.antenna_height_jitter_min_m,
                        min(self.antenna_height_jitter_max_m, value),
                    )
...
class CKMHDF5Dataset(Dataset):
    def __init__(
        ...
        knife_edge_channel: Optional[Dict[str, Any]] = None,
    ) -> None:
        ...
        self.knife_edge_channel_cfg = dict(knife_edge_channel or {})
        self.knife_edge_channel_enabled = bool(self.knife_edge_channel_cfg.get("enabled", False))
    ...
    def _apply_sync_aug(self, images: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        if random.random() < self.rot90_prob:
            ...
            images = [TF.rotate(img, angle) for img in images]
        if self.transpose_prob > 0.0 and random.random() < self.transpose_prob:
            images = [img.transpose(1, 2).contiguous() for img in images]
        return images
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if 'antenna_height_m' in self.scalar_feature_columns:
            ah_base_m = float(self._resolve_hdf5_scalar_value(city, sample, 'antenna_height_m'))
        elif 'antenna_height_m' in self.constant_scalar_features:
            ah_base_m = float(self.constant_scalar_features['antenna_height_m'])

        formula_cfg = dict(self.path_loss_formula_input)
        precomputed_formula_tensor = (
            self._load_precomputed_formula_input(city, sample) if bool(formula_cfg.get('enabled', False)) else None
        )

        ah_eff_m = ah_base_m
        disable_formula_disk_cache = False
        if (
            self.augment
            and self.antenna_height_jitter_half_width_m > 0.0
            and random.random() < self.antenna_height_jitter_prob
            and 'antenna_height_m' in self.scalar_feature_columns
            and precomputed_formula_tensor is None
        ):
            hw = float(self.antenna_height_jitter_half_width_m)
            ah_eff_m = ah_base_m + random.uniform(-hw, hw)
            ah_eff_m = max(self.antenna_height_jitter_min_m, min(self.antenna_height_jitter_max_m, ah_eff_m))
            if abs(ah_eff_m - ah_base_m) > 1e-3:
                disable_formula_disk_cache = True
        ...
        if bool(formula_cfg.get('enabled', False)):
            ...
            precomputed_formula = precomputed_formula_tensor
            cache_path = None
            if precomputed_formula is None and not disable_formula_disk_cache:
                cache_path = self._formula_cache_path(city, sample, formula_cfg, path_meta)
            if precomputed_formula is not None:
                ...
                prior_db = _compute_formula_path_loss_db(
                    ...
                    antenna_height_m=float(ah_eff_m),
                ...
                if cache_path is not None and not disable_formula_disk_cache:
        ...
        if self.knife_edge_channel_enabled:
            from knife_edge import compute_knife_edge_loss_map, normalize_knife_edge_db
            ah_m_ke = float(ah_eff_m)
            input_scale_ke = float(self.input_metadata.get('scale', 255.0))
            topo_m_ke = input_tensor.float() * input_scale_ke  # (1, H, W) meters
            freq_ghz = float(self.knife_edge_channel_cfg.get(
                "frequency_ghz",
                self.path_loss_formula_input.get("frequency_ghz", 3.5),
            ))
            mpp = float(self.knife_edge_channel_cfg.get(
                "meters_per_pixel",
                self.path_loss_formula_input.get("meters_per_pixel", 1.0),
            ))
            rx_h = float(self.knife_edge_channel_cfg.get("rx_height_m", 1.5))
            n_samples = int(self.knife_edge_channel_cfg.get("num_ray_samples", 48))
            scale_db = float(self.knife_edge_channel_cfg.get("scale_db", 40.0))
            ke_db = compute_knife_edge_loss_map(
                topo_m_ke,
                antenna_height_m=ah_m_ke,
                frequency_ghz=freq_ghz,
                meters_per_pixel=mpp,
                rx_height_m=rx_h,
                num_ray_samples=n_samples,
            )
            ke_norm = normalize_knife_edge_db(ke_db, scale_db=scale_db)
            model_input_channels.append(ke_norm)
...
def build_dataset_splits_from_config(cfg: Dict[str, Any]) -> Dict[str, Dataset]:
    ...
    city_type_filter = str(partition_filter.get("city_type", "")).strip() or None

    if topology_class_filter or antenna_bin_filter or city_type_filter:
        ...
        sample_refs = _filter_hdf5_refs_by_partition(
            ...
            city_type=city_type_filter,
            non_ground_threshold=float(data_cfg.get("non_ground_threshold", 0.0)),
        )
        print(
            f"[INFO] partition_filter topology_class={topology_class_filter!r} "
            f"antenna_bin={antenna_bin_filter!r} city_type={city_type_filter!r} "
    ...
    common_hdf5 = dict(
        ...
        knife_edge_channel=dict(data_cfg.get('knife_edge_channel', {})),
    )
    splits = {
