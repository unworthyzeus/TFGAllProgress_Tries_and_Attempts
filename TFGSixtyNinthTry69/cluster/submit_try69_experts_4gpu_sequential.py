#!/usr/bin/env python3
"""Submit chained 4-GPU Slurm jobs for Try69 experts (one job per expert, sequential).

Each job uses ``cluster/run_sixtyninth_try69_4gpu.slurm`` (4 GPUs, 4:00:00 wall-time).
Experts are resolved from ``experiments/sixtyninth_try69_experts/try69_expert_registry.yaml``
(six ``topology_class`` experts by default).
...
Pass explicit expert_ids to submit a subset (e.g. ``open_sparse_lowrise dense_block_highrise``).
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try69 4-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
        help=(
            "expert_id values from try69_expert_registry.yaml. "
    ...
    parser.add_argument(
        "--wipe-outputs",
        action="store_true",
        help="Delete outputs/<expert_id>/ on the cluster before submitting (implies --no-resume).",
    )
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            job_name = f"t69-4gpu-{expert_id.replace('_', '-')[:22]}"
            exports = [
            ...
            train_cmd = (
                ...
                "cluster/run_sixtyninth_try69_4gpu.slurm"
            )
            ...
            cleanup_cmd = (
                ...
                f"-J {cleanup_name} "
                "cluster/run_sixtyninth_try69_cleanup_sert2001_1gpu.slurm"
            )
