#!/usr/bin/env python3
"""Submit chained 2-GPU Slurm jobs for Try69 experts (one job per expert, sequential).

Each job uses ``cluster/run_sixtyninth_try69_2gpu.slurm`` (2 GPUs, 4:00:00 wall-time).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try69 2-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            job_name = f"t69-2gpu-{expert_id.replace('_', '-')[:24]}"
            exports = [
            ...
            train_cmd = (
                ...
                "cluster/run_sixtyninth_try69_2gpu.slurm"
            )
