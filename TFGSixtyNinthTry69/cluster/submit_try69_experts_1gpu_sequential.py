#!/usr/bin/env python3
"""Submit chained 1-GPU Slurm jobs for Try69 experts (train -> cleanup -> …).

Expert list and order come from ``try69_expert_registry.yaml`` (six topology experts by default).
Uses ``cluster/run_sixtyninth_try69_1gpu.slurm`` and existing 1-GPU cleanup script.
"""
...
LOCAL_DIR = ROOT / "TFGSixtyNinthTry69"
REGISTRY = LOCAL_DIR / "experiments" / "sixtyninth_try69_experts" / "try69_expert_registry.yaml"
REMOTE_DIR = "/scratch/nas/3/gmoreno/TFGpractice/TFGSixtyNinthTry69"
HOST = "sert.ac.upc.edu"
USER = "gmoreno"
TARGET_NODE = "sert-2001"
TRAIN_SLURM = "cluster/run_sixtyninth_try69_1gpu.slurm"
CLEANUP_SLURM = "cluster/run_sixtyninth_try69_cleanup_sert2001_1gpu.slurm"
...
def sbatch_afterany_prefix(flag_value: str) -> str:
    ...
    if bad:
        raise SystemExit(
            "Invalid --dependency-afterany: numeric Slurm job id(s) only, e.g. 10030700. "
            f"Bad token(s): {bad!r}. Omit the flag for an independent Try69 chain."
        )
...
def resolve_expert(expert_id: str, experts: list[dict[str, Any]]) -> dict[str, Any]:
    for row in experts:
        if (
            str(row.get("expert_id")) == expert_id
            or str(row.get("topology_class")) == expert_id
            or str(row.get("city_type")) == expert_id
        ):
            return row
    ids = [str(r.get("topology_class") or r.get("expert_id")) for r in experts]
    raise SystemExit(f"Unknown expert_id {expert_id!r}. Known: {ids}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Submit sequential Try69 1-GPU jobs (train + cleanup).")
    ...
    parser.add_argument("--base-master-port", type=int, default=30340)
    ...
    parser.add_argument(
        ...
        default="",
        metavar="JOBID",
        help="Optional numeric job id(s) only; e.g. LAST_TRY68_CLEANUP_JOB_ID. Omit for independent chain.",
    )
    ...
    try:
        ...
        if args.wipe_outputs:
            for row in rows:
                ...
                out_dir = f"{REMOTE_DIR}/outputs/try69_expert_{expert_id}"
                remote_exec(client, f"rm -rf {out_dir}", check=False)
        ...
        for i, row in enumerate(rows):
            ...
            job_name = f"t69-1gpu-{expert_id.replace('_', '-')[:24]}"
            exports = [
            ...
            cleanup_name = f"t69-cleanup-{expert_id.replace('_', '-')[:18]}"
            cleanup_cmd = (
