...
from data_utils import (
    ...
    _load_formula_regime_calibration,
    _normalize_channel,
...
_FORMULA_CALIB_CACHE: dict[str, Any] = {}


def _cached_formula_regime_calibration(cfg: Dict[str, Any]) -> Any:
    raw = cfg.get("data", {}).get("path_loss_formula_input", {}).get("regime_calibration_json")
    key = str(raw or "")
    if key not in _FORMULA_CALIB_CACHE:
        _FORMULA_CALIB_CACHE[key] = _load_formula_regime_calibration(raw)
    return _FORMULA_CALIB_CACHE[key]


def _pmhhnet_input_channel_ranges(cfg: Dict[str, Any]) -> dict[str, tuple[int, int]]:
    """Channel order must match ``CKMHDF5Dataset`` / ``compute_input_channels``."""
    i = 0
    ...
    if bool(cfg["data"].get("building_mask_channel", False)):
        r["building"] = (i, i + 1)
        i += 1
    ke = dict(cfg["data"].get("knife_edge_channel", {}))
    if bool(ke.get("enabled", False)):
        r["knife"] = (i, i + 1)
        i += 1
    r["_total"] = i
    return r
...
_PHYSICS_RECOMPUTE_WARNED = False


def recompute_height_dependent_input_channels(
    x: torch.Tensor,
    scalar_cond: torch.Tensor,
    cfg: Dict[str, Any],
) -> torch.Tensor:
    """Rebuild formula / confidence / tx-depth / elevation / building / knife after **geometry-only** CutMix.

    Keeps ``scalar_cond`` (UAV height) and the pasted topology + LoS + distance channels; recomputes every
    channel that was produced with height-dependent physics in ``data_utils``. Requires the same channel
    layout as Try 69 HDF5 experts (no extra obstruction tensors in ``x``).
    """
    global _PHYSICS_RECOMPUTE_WARNED
    ranges = _pmhhnet_input_channel_ranges(cfg)
    if bool(cfg["data"].get("path_loss_obstruction_features", {}).get("enabled", False)):
        if not _PHYSICS_RECOMPUTE_WARNED:
            warnings.warn(
                "cutmix_film_safe: path_loss_obstruction_features enabled — skipping physics recompute.",
                RuntimeWarning,
            )
            _PHYSICS_RECOMPUTE_WARNED = True
        return x
    if ranges.get("_total", 0) != x.shape[1]:
        if not _PHYSICS_RECOMPUTE_WARNED:
            warnings.warn(
                f"cutmix_film_safe: channel count mismatch (expected {ranges.get('_total')}, got {x.shape[1]}); skip.",
                RuntimeWarning,
            )
            _PHYSICS_RECOMPUTE_WARNED = True
        return x
    ...
    if not bool(formula_cfg.get("enabled", False)):
        return x
    cols = list(data_cfg.get("scalar_feature_columns", []))
    if "antenna_height_m" not in cols:
        return x
    ah_idx = cols.index("antenna_height_m")
    sc = scalar_cond
    if sc.dim() == 1:
        sc = sc.view(-1, 1)
    ah_norm = float(dict(data_cfg.get("scalar_feature_norms", {})).get("antenna_height_m", 1.0))
    path_meta = dict(cfg.get("target_metadata", {}).get("path_loss", {}))
    if not path_meta:
        path_meta = {"scale": 180.0, "offset": 0.0, "clip_min": 0.0, "clip_max": 180.0}
    input_meta = dict(data_cfg.get("input_metadata", {}))
    input_scale = float(input_meta.get("scale", 255.0))
    ng_th = float(data_cfg.get("non_ground_threshold", 0.0))
    mpp = float(formula_cfg.get("meters_per_pixel", 1.0))
    recv_h = float(formula_cfg.get("receiver_height_m", 1.5))
    cal = _cached_formula_regime_calibration(cfg)
    ...
    conf_k = int(formula_cfg.get("confidence_kernel_size", 31))
    B, _, H, W = x.shape
    x_out = x.clone()
    for b in range(B):
        topo_n = x_out[b : b + 1, ranges["topology"][0] : ranges["topology"][1]]
        topo_m = topo_n * input_scale
        los = None
        if "los" in ranges:
            los = x_out[b : b + 1, ranges["los"][0] : ranges["los"][1]]
        if "distance" in ranges:
            dist_ch = x_out[b : b + 1, ranges["distance"][0] : ranges["distance"][1]]
        else:
            dist_ch = _compute_distance_map_2d(H).to(device=x.device, dtype=x.dtype)
        dist_hw = dist_ch.squeeze(0)
        if dist_hw.ndim == 3:
            dist_hw = dist_hw.squeeze(0)
        raw_flat = topo_m.view(-1)
        building_mask = (topo_m.squeeze(0) > max(ng_th, 1e-6)).float()
        building_density = float((topo_m.squeeze(0) > max(ng_th, 1e-6)).float().mean().item())
        nz = raw_flat[raw_flat > max(ng_th, 1e-6)]
        building_height_proxy = float(nz.mean().item()) if nz.numel() else 0.0
        ah_m = float(sc[b, ah_idx].item() * max(ah_norm, 1e-12))
        prior_db = _compute_formula_path_loss_db(
            image_size=H,
            antenna_height_m=ah_m,
            receiver_height_m=recv_h,
            frequency_ghz=float(formula_cfg.get("frequency_ghz", 7.125)),
            meters_per_pixel=mpp,
            formula_mode=str(formula_cfg.get("formula", "cost231_hata")),
            los_tensor=los,
            topology_tensor=topo_m,
            a2g_params=dict(formula_cfg.get("a2g_params", {})),
            building_density=building_density,
            building_height_proxy=building_height_proxy,
            non_ground_threshold=ng_th,
            clip_min=clip_min,
            clip_max=clip_max,
        )
        prior_db = _apply_formula_regime_calibration(
            ...
            antenna_height_m=ah_m,
            los_tensor=los,
            topology_tensor=topo_m.squeeze(0),
            distance_map_tensor=dist_hw,
            non_ground_threshold=ng_th,
            clip_min=clip_min,
            clip_max=clip_max,
            prefer_threshold_city_type=bool(formula_cfg.get("prefer_threshold_city_type", False)),
        )
        formula_norm = _normalize_channel(prior_db, path_meta)
        while formula_norm.ndim < 4:
            formula_norm = formula_norm.unsqueeze(0)
        bm4 = building_mask.view(1, 1, H, W)
        formula_norm = formula_norm * (1.0 - bm4)
        fs, fe = ranges["formula"]
        x_out[b : b + 1, fs:fe] = formula_norm
        if "confidence" in ranges:
            dist_for_conf = dist_hw.unsqueeze(0) if dist_hw.ndim == 2 else dist_hw
            conf = _build_prior_confidence_channel(
                los,
                dist_for_conf,
                topo_m.squeeze(0),
                non_ground_threshold=ng_th,
            ...
            conf = conf * (1.0 - building_mask)
            while conf.ndim < 4:
                conf = conf.unsqueeze(0)
            cs, ce = ranges["confidence"]
            x_out[b : b + 1, cs:ce] = conf
        if "tx_depth" in ranges:
            depth = (topo_m - ah_m) / max(input_scale, 1.0)
            ts, te = ranges["tx_depth"]
            x_out[b : b + 1, ts:te] = depth.clamp(-1.0, 1.0)
        if "elevation" in ranges:
            half = (H - 1) / 2.0
            max_dist_px = max(half * math.sqrt(2.0), 1.0)
            ground_d_m = dist_hw * max_dist_px * mpp
            theta = torch.atan2(
                torch.full_like(ground_d_m, ah_m - recv_h),
                ground_d_m.clamp(min=1.0),
            )
            theta_norm = (torch.rad2deg(theta) / 90.0).clamp(0.0, 1.0).unsqueeze(0)
            es, ee = ranges["elevation"]
            x_out[b : b + 1, es:ee] = theta_norm
        if "building" in ranges:
            bs, be = ranges["building"]
            x_out[b : b + 1, bs:be] = building_mask.unsqueeze(0)
        if "knife" in ranges:
            from knife_edge import compute_knife_edge_loss_map, normalize_knife_edge_db

            ke = dict(data_cfg.get("knife_edge_channel", {}))
            freq_ghz = float(ke.get("frequency_ghz", formula_cfg.get("frequency_ghz", 7.125)))
            rx_h = float(ke.get("rx_height_m", recv_h))
            n_samples = int(ke.get("num_ray_samples", 48))
            scale_db = float(ke.get("scale_db", 40.0))
            ke_db = compute_knife_edge_loss_map(
                topo_m,
                antenna_height_m=ah_m,
                frequency_ghz=freq_ghz,
                meters_per_pixel=float(ke.get("meters_per_pixel", mpp)),
                rx_height_m=rx_h,
                num_ray_samples=n_samples,
            )
            ks, kec = ranges["knife"]
            x_out[b : b + 1, ks:kec] = normalize_knife_edge_db(ke_db, scale_db=scale_db)
    return x_out


def _corridor_gao_ray_multiplier(
    x: torch.Tensor,
    cfg: Dict[str, Any],
    device: torch.device,
    dtype: torch.dtype,
) -> torch.Tensor:
    """Loss mask weights from mean building occupancy along Tx→pixel rays (Gao-style, cheap sampling)."""
    cw = dict(cfg.get("corridor_weighting", {}))
    k_samples = max(int(cw.get("num_ray_samples", 8)), 2)
    beta = float(cw.get("ray_obstruction_beta", 0.85))
    min_w = float(cw.get("min_weight", 0.3))
    kappa = float(cw.get("kappa", 150.0))
    mpp = float(dict(cfg["data"].get("path_loss_formula_input", {})).get("meters_per_pixel", 1.0))
    input_scale = float(cfg["data"].get("input_metadata", {}).get("scale", 255.0))
    ng_th = float(cfg["data"].get("non_ground_threshold", 0.0))
    topo_m = x[:, 0:1].to(dtype=dtype) * input_scale
    B, _, H, W = topo_m.shape
    ...
    cy = (H - 1) * 0.5
    cx = (W - 1) * 0.5
    d_px = torch.sqrt((yy - cy) ** 2 + (xx - cx) ** 2).clamp(min=0.0)
    d_m = d_px * mpp
    occ = torch.zeros(B, 1, H, W, device=device, dtype=dtype)
    thr = max(ng_th, 1e-4)
    for k in range(k_samples):
        t = float(k + 1) / float(k_samples + 1)
        sy = cy + t * (yy - cy)
        sx = cx + t * (xx - cx)
        gy = sy / max(H - 1, 1) * 2.0 - 1.0
        gx = sx / max(W - 1, 1) * 2.0 - 1.0
        grid = torch.stack([gx, gy], dim=-1)
        sampled = F.grid_sample(topo_m, grid, mode="bilinear", padding_mode="border", align_corners=True)
        occ = occ + (sampled > thr).to(dtype=dtype)
    occ = occ / float(k_samples)
    inv = kappa / (kappa + d_m)
    return (min_w + (1.0 - min_w) * inv * (1.0 + beta * occ)).clamp(min=min_w, max=1.0)
...
_CORRIDOR_GRID_CACHE: dict[tuple[Any, ...], torch.Tensor] = {}


def _corridor_spatial_multiplier(h: int, w: int, cfg: Dict[str, Any], device: torch.device, dtype: torch.dtype) -> Optional[torch.Tensor]:
    """Tx-centred corridor weights [1,1,h,w] in [min_weight,1]; None if disabled.

    Modes:
      * ``radial`` (default): isotropic Gaussian x inverse-square from map centre.
      * ``anisotropic``: elliptical tube through the UAV pixel (street-canyon proxy;
        Gao-style corridor loss, simplified).
    """
    cw = dict(cfg.get("corridor_weighting", {}))
    if not bool(cw.get("enabled", False)):
        return None
    mode = str(cw.get("mode", "radial")).lower().strip()
    if mode == "gao_ray":
        return None
    sigma = float(cw.get("sigma", 40.0))
    kappa = float(cw.get("kappa", 150.0))
    min_w = float(cw.get("min_weight", 0.3))
    if mode == "anisotropic":
        angle_deg = float(cw.get("angle_deg", 0.0))
        aspect = max(float(cw.get("aspect_ratio", 1.75)), 1.001)
        sigma_para = max(sigma * aspect, 1e-3)
        sigma_perp = max(sigma / aspect, 1e-3)
        key = ("aniso", h, w, sigma, kappa, min_w, angle_deg, aspect)
        if key not in _CORRIDOR_GRID_CACHE:
            cy = (h - 1) * 0.5
            cx = (w - 1) * 0.5
            ...
            xr = dx * ca + dy * sa
            yr = -dx * sa + dy * ca
            d = torch.sqrt(dx * dx + dy * dy).clamp(min=0.0)
            ell = torch.exp(-0.5 * (xr / sigma_para) ** 2 - 0.5 * (yr / sigma_perp) ** 2)
            inv = kappa / (kappa + d)
            mp = min_w + (1.0 - min_w) * ell * inv
            _CORRIDOR_GRID_CACHE[key] = mp.view(1, 1, h, w).clamp(min=min_w, max=1.0)
        return _CORRIDOR_GRID_CACHE[key].to(device=device, dtype=dtype)

    key = ("radial", h, w, sigma, kappa, min_w)
    if key not in _CORRIDOR_GRID_CACHE:
        cy = (h - 1) * 0.5
        cx = (w - 1) * 0.5
        yy = torch.arange(h, dtype=torch.float32).view(-1, 1)
        xx = torch.arange(w, dtype=torch.float32).view(1, -1)
        d = torch.sqrt((yy - cy) ** 2 + (xx - cx) ** 2).clamp(min=0.0)
        radial = torch.exp(-((d / max(sigma, 1e-3)) ** 2) * 0.5)
        inv = kappa / (kappa + d)
        mp = min_w + (1.0 - min_w) * radial * inv
        _CORRIDOR_GRID_CACHE[key] = mp.view(1, 1, h, w).clamp(min=min_w, max=1.0)
    return _CORRIDOR_GRID_CACHE[key].to(device=device, dtype=dtype)


def _cutmix_box_uniform(h: int, w: int, lam: float) -> tuple[int, int, int, int]:
...
def _point_to_box_edge_distance_sq(px: float, py: float, x1: int, x2: int, y1: int, y2: int) -> float:
    """Squared distance from a point to the border of an axis-aligned box (0 if inside)."""
    dx = 0.0
    if px < x1:
        dx = float(x1 - px)
    elif px > x2 - 1:
...
def _cutmix_box(h: int, w: int, lam: float, tx_cfg: Optional[Dict[str, Any]] = None) -> tuple[int, int, int, int]:
    """Random CutMix box; optionally keep a disk around the Tx (map centre) untouched (PathFinder-style)."""
    if tx_cfg is None or not bool(tx_cfg.get("enabled", False)):
        return _cutmix_box_uniform(h, w, lam)
    protect_r = max(float(tx_cfg.get("protect_radius_px", 24.0)), 0.0)
    max_attempts = max(int(tx_cfg.get("max_attempts", 48)), 1)
    rsq = protect_r * protect_r
    ccx = (w - 1) * 0.5
    ccy = (h - 1) * 0.5
    for _ in range(max_attempts):
        y1, y2, x1, x2 = _cutmix_box_uniform(h, w, lam)
        if _point_to_box_edge_distance_sq(ccx, ccy, x1, x2, y1, y2) >= rsq:
            return y1, y2, x1, x2
    return _cutmix_box_uniform(h, w, lam)
...
def build_experiment_summary(cfg: Dict[str, Any], dataset: Optional[Any] = None) -> dict[str, Any]:
    ...
    # Try69 SOA-feature snapshot — surfaces which emerging techniques are
...
def train_one_epoch(
    ...
    *,
    ...
    tx_oriented_cutmix_cfg = dict(cfg.get("training", {}).get("tx_oriented_cutmix", {}))
    cutmix_film_safe_cfg = dict(cfg.get("training", {}).get("cutmix_film_safe", {}))
    use_cutmix_film_safe = bool(cutmix_film_safe_cfg.get("enabled", False)) and return_scalar_cond_from_config(cfg)
    ...
    for batch in tqdm(loader, desc="train", leave=False, disable=distributed and not is_main_process(rank)):
        ...
        # Default: full-spatial CutMix. With FiLM + scalar_cond, that is inconsistent (height channels
        # from sample B vs scalar_cond from sample A). Either skip CutMix, or use ``cutmix_film_safe``:
        # paste only topology + LoS + distance, then recompute height-dependent channels on-device.
        do_cutmix = (
            cutmix_prob > 0.0
            and prev_cutmix_buf["x"] is not None
            and random.random() < cutmix_prob
        )
        film_safe_this = bool(use_cutmix_film_safe and scalar_cond is not None)
        if do_cutmix and return_scalar_cond_from_config(cfg) and scalar_cond is not None and not film_safe_this:
            do_cutmix = False
        if do_cutmix:
            lam = random.betavariate(cutmix_alpha, cutmix_alpha) if cutmix_alpha > 0.0 else 0.5
            _, _, h, w = x.shape
            tx_cm = tx_oriented_cutmix_cfg if bool(tx_oriented_cutmix_cfg.get("enabled", False)) else None
            by1, by2, bx1, bx2 = _cutmix_box(h, w, lam, tx_cm)
            ...
            if px is not None and py is not None and pm is not None:
                if px.shape[2:] == x.shape[2:]:
                    if film_safe_this:
                        geom_n = _pmhhnet_input_channel_ranges(cfg)
                        g_end = 1
                        if cfg["data"].get("los_input_column"):
                            g_end += 1
                        if bool(cfg["data"].get("distance_map_channel", False)):
                            g_end += 1
                        x[:, :g_end, by1:by2, bx1:bx2] = px[:, :g_end, by1:by2, bx1:bx2]
                        y[:, :, by1:by2, bx1:bx2] = py[:, :, by1:by2, bx1:bx2]
                        m[:, :, by1:by2, bx1:bx2] = pm[:, :, by1:by2, bx1:bx2]
                        x = recompute_height_dependent_input_channels(x, scalar_cond, cfg)
                    else:
        ...
        with amp.autocast(device_type="cuda", enabled=amp_enabled):
            ...
            cw_cfg = dict(cfg.get("corridor_weighting", {}))
            cw_mode = str(cw_cfg.get("mode", "radial")).lower()
            if cw_mode == "gao_ray" and bool(cw_cfg.get("enabled", False)):
                corridor_mul = _corridor_gao_ray_multiplier(x, cfg, x.device, x.dtype)
            else:
                corridor_mul = _corridor_spatial_multiplier(
                    int(x.shape[2]), int(x.shape[3]), cfg, x.device, x.dtype
                )
            if corridor_mul is not None:
                weighted_mask = weighted_mask * corridor_mul
            ...
            if objective_mode == "full_map_rmse_only":
                if loss_type == "huber":
                    g_loss = masked_huber_loss(pred, target, weighted_mask, delta=huber_delta_eff)
                else:
                    g_loss = masked_rmse_loss(pred, target, weighted_mask)
                ms_loss = compute_multiscale_path_loss_loss(pred, target, weighted_mask, meta, cfg)
