...
def _apply_path_loss_clamp(values: torch.Tensor, metadata: Dict[str, object]) -> torch.Tensor:
    clip_min = metadata.get('clip_min')
    clip_max = metadata.get('clip_max')
...
def _radial_bin_maps(reference: torch.Tensor, num_bins: int) -> torch.Tensor:
    _, _, h, w = reference.shape
    cy = (h - 1) / 2.0
    cx = (w - 1) / 2.0
    yy = torch.arange(h, device=reference.device, dtype=reference.dtype).view(1, 1, h, 1)
    xx = torch.arange(w, device=reference.device, dtype=reference.dtype).view(1, 1, 1, w)
    dist = torch.sqrt((xx - cx) ** 2 + (yy - cy) ** 2)
    max_dist = max((cx ** 2 + cy ** 2) ** 0.5, 1.0)
    normalized = (dist / max_dist).clamp(0.0, 1.0)
    bins = torch.clamp((normalized * num_bins).long(), min=0, max=max(0, num_bins - 1))
    return bins.expand(reference.shape[0], 1, h, w)


def _compute_radial_profile(values: torch.Tensor, mask: torch.Tensor, num_bins: int) -> Tuple[torch.Tensor, torch.Tensor]:
    bins = _radial_bin_maps(values, num_bins)
    means: List[torch.Tensor] = []
    valid: List[torch.Tensor] = []
    for bin_idx in range(num_bins):
        ring = ((bins == bin_idx).to(dtype=values.dtype)) * mask
        denom = ring.sum(dim=(2, 3))
        mean = ((values * ring).sum(dim=(2, 3)) / denom.clamp_min(1.0)).squeeze(1)
        means.append(mean)
        valid.append((denom > 0).squeeze(1))
    return torch.stack(means, dim=1), torch.stack(valid, dim=1)


def compute_radial_profile_path_loss_loss(
    outputs: torch.Tensor,
    targets: torch.Tensor,
    masks: torch.Tensor,
    target_columns: List[str],
    ...
    radial_cfg = dict(cfg.get('radial_profile_path_loss', {}))
    if not bool(radial_cfg.get('enabled', False)) or 'path_loss' not in target_columns:
        return torch.tensor(0.0, device=outputs.device)

    num_bins = max(8, int(radial_cfg.get('num_bins', 48)))
    loss_weight = float(radial_cfg.get('loss_weight', 0.2))
    if loss_weight <= 0.0:
        return torch.tensor(0.0, device=outputs.device)
    ...
    pred_db = _apply_path_loss_clamp(_denormalize_path_loss_to_db(pred_path, path_meta), path_meta)
    target_db = _apply_path_loss_clamp(_denormalize_path_loss_to_db(target_path, path_meta), path_meta)
    pred_profile, valid_pred = _compute_radial_profile(pred_db, path_mask, num_bins)
    target_profile, valid_tgt = _compute_radial_profile(target_db, path_mask, num_bins)
    valid = valid_pred & valid_tgt
    if not torch.any(valid):
        return torch.tensor(0.0, device=outputs.device)

    diff = (pred_profile - target_profile) ** 2
    scale_db = max(float(path_meta.get('scale', 180.0)), 1.0)
    return loss_weight * (diff[valid].mean() / (scale_db ** 2))


def compute_radial_gradient_path_loss_loss(
    outputs: torch.Tensor,
    targets: torch.Tensor,
    masks: torch.Tensor,
    target_columns: List[str],
    ...
    radial_cfg = dict(cfg.get('radial_gradient_path_loss', {}))
    if not bool(radial_cfg.get('enabled', False)) or 'path_loss' not in target_columns:
        return torch.tensor(0.0, device=outputs.device)

    num_bins = max(8, int(radial_cfg.get('num_bins', 48)))
    loss_weight = float(radial_cfg.get('loss_weight', 0.15))
    if loss_weight <= 0.0:
        return torch.tensor(0.0, device=outputs.device)
        ...
        for batch in tqdm(
            ...
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                if 'path_loss' in target_columns:
                    ...
                    radial_profile_path_loss = compute_radial_profile_path_loss_loss(
                        fake, y, m, target_columns, target_metadata, cfg,
                    )
                    radial_gradient_path_loss = compute_radial_gradient_path_loss_loss(
                        fake, y, m, target_columns, target_metadata, cfg,
                    )
                    ...
                    if n_other <= 0:
                        recon_loss = (
                            ...
                            + radial_profile_path_loss
                            + radial_gradient_path_loss
                        )
                    else:
                        recon_loss = (
                            (recon_other * n_other + path_loss_mse_db) / n_total
                            + multiscale_path_loss
                            + radial_profile_path_loss
                            + radial_gradient_path_loss
                        )
