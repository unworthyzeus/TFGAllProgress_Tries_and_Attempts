...
def main() -> None:
    ...
    topology_image = Image.open(args.input).convert('L')
    image = topology_image.resize((image_size, image_size), Image.BILINEAR)
    topology_mask_image = topology_image.resize((image_size, image_size), Image.NEAREST)
    topology_valid_mask = (np.asarray(topology_mask_image, dtype=np.float32) / 255.0) > 0.0
        ...
        if name == 'augmented_los':
            ...
            if name == 'path_loss' and hybrid_enabled and confidence_map is not None:
                fallback_outputs = apply_path_loss_confidence_fallback(
                    ...
                    valid_mask=topology_valid_mask,
                )
                np.save(out_dir / 'path_loss_coarse_physical.npy', np.where(topology_valid_mask, arr_physical, np.nan).astype(np.float32))
            ...
            if name == 'path_loss':
                arr_physical = np.where(topology_valid_mask, arr_physical, np.nan).astype(np.float32)
            np.save(out_dir / f'{name}_physical.npy', arr_physical)
            physical_outputs[name] = arr_physical
            # PNG debe reflejar unidades físicas (p. ej. dB), no la salida normalizada del modelo.
            arr = arr_physical
