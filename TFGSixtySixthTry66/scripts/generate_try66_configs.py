"""Generate Try 66 configs: 6 topology experts (single-stage 513x513) + classifier.
...
def _apply_common_data(cfg: dict, topology_class: str) -> None:
    ...
    formula["cache_dir"] = "prior_cache/try66_prior_auto_city_v1"
    formula["cache_version"] = "try66_prior_auto_city_v1"
...
def main() -> None:
    ...
    expert_dir = root / "experiments" / "sixtysixth_try66_experts"
    cl_dir = root / "experiments" / "sixtysixth_try66_classifier"
    expert_dir.mkdir(parents=True, exist_ok=True)
    cl_dir.mkdir(parents=True, exist_ok=True)

    _cleanup(expert_dir, "try66_expert_*.yaml")
    ...
    for spec in EXPERT_SPECS:
        ...
        name = f"try66_expert_{spec['expert_id']}"
    ...
    cl_cfg = {
        ...
        "runtime": {
            "device": "cuda",
            "output_dir": "outputs/try66_topology_classifier",
            "resume_checkpoint": "",
        },
    }
    cl_path = cl_dir / "try66_topology_classifier.yaml"
    with cl_path.open("w", encoding="utf-8") as f:
        yaml.safe_dump(cl_cfg, f, sort_keys=False)

    with (expert_dir / "try66_expert_registry.yaml").open("w", encoding="utf-8") as f:
