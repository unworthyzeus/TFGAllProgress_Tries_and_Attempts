"""Plot Try 66 training metrics from validate_metrics_epoch_*.json files.

Reads the Try 66 JSON schema (single-stage 513x513, no no-data head) and
...
Usage:
  python scripts/plot_try66_metrics.py
  python scripts/plot_try66_metrics.py --root D:/cluster_outputs/TFGSixtySixthTry66
  python scripts/plot_try66_metrics.py --expert-id open_sparse_lowrise
"""
...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGSixtySixthTry66"
EXPERT_PREFIX = "sixtysixth_try66_expert_"
...
def _expert_key(path: Path) -> str:
    ...
    for prefix in ("sixtysixth_try66_expert_", "sixtysixth_expert_", "try66_expert_", "sixtysixth_try66_"):
...
def plot_expert(expert_id: str, output_dir: Path, save_path: Path | None = None) -> dict[str, Any]:
    ...
    ax.set_title(f"Try 66 — {topo} — Overall RMSE" + (" [EMA for val]" if uses_ema else ""))
    ax.grid(alpha=0.25)
    ...
    resolved = save_path or output_dir / "metrics_plot_try66.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 66 single-stage metrics per expert.")
    ...
    for d in dirs:
        eid = _expert_key(d)
        save = Path(args.save_path) / f"{eid}_metrics_try66.png" if args.save_path else None
        try:
