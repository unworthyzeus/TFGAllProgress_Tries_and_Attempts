"""Cancel all jobs, wipe Try 66 logs (keep checkpoints), upload code, and relaunch all experts."""
...
def main() -> None:
    ...
    run(
        c,
        f"rm -f {REMOTE_DIR}/logs_train_try66_*.out {REMOTE_DIR}/logs_train_try66_*.err "
        f"{REMOTE_DIR}/logs_train_try66_chain_*.out {REMOTE_DIR}/logs_train_try66_chain_*.err",
    )

    # 3. Upload code
    print("\n=== Step 3: Upload code ===")
    run(c, f"mkdir -p {REMOTE_DIR}/experiments/sixtysixth_try66_experts")
    run(c, f"mkdir -p {REMOTE_DIR}/experiments/sixtysixth_try66_classifier")
    ...
    config = "experiments/sixtysixth_try66_experts/try66_expert_open_sparse_lowrise.yaml"
    job_name = "t66-open-sparse-lowrise"
    cleanup_script = "cluster/run_sixtysixth_try66_cleanup_sert2001_1gpu.slurm"
    submitted = []
    current_dep = ""
    ...
    for run_idx in range(3):
        ...
        cmd = (
            ...
            "cluster/run_sixtysixth_try66_3gpu.slurm"
        )
        out = run(c, cmd)
