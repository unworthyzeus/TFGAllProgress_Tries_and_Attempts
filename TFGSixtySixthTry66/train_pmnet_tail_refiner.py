...
from model_pmhhnet import PMNetResidualRegressor, UNetResidualRefiner, UNetResidualRefinerH
...
class Stage1Teacher:
    def __init__(self, cfg: Dict[str, Any], stage1_cfg: Dict[str, Any], checkpoint_path: str, device: object) -> None:
        ...
        self.teacher_image_size = int(stage1_cfg.get("data", {}).get("image_size", 128))
    ...
    def _resize_input_for_teacher(self, x: torch.Tensor) -> torch.Tensor:
        if x.shape[-2:] == (self.teacher_image_size, self.teacher_image_size):
            return x
    ...
    def _resize_output_to_target(self, tensor: torch.Tensor, target_hw: tuple[int, int]) -> torch.Tensor:
    ...
    def infer(self, x: torch.Tensor, target: torch.Tensor, amp_enabled: bool) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        teacher_x = self._resize_input_for_teacher(x)
        prior = teacher_x[:, self.formula_idx : self.formula_idx + 1]
        with torch.no_grad():
            with amp.autocast(device_type="cuda", enabled=amp_enabled):
                residual = self.model(teacher_x)
                pred = clip_to_target_range(prior + residual[:, :1], self.meta)
        pred = clip_to_target_range(self._resize_output_to_target(pred, target_hw), self.meta)
        prior = self._resize_output_to_target(prior, target_hw)
    ...
    def predict_only(self, x: torch.Tensor, amp_enabled: bool) -> tuple[torch.Tensor, torch.Tensor]:
        target_hw = tuple(int(v) for v in x.shape[-2:])
        teacher_x = self._resize_input_for_teacher(x)
        prior = teacher_x[:, self.formula_idx : self.formula_idx + 1]
        with torch.no_grad():
            with amp.autocast(device_type="cuda", enabled=amp_enabled):
...
def build_tail_refiner_model(cfg: Dict[str, Any], in_channels: int) -> nn.Module:
    ...
    if refiner_arch == "unet_h":
        from data_utils import compute_scalar_cond_dim
        scalar_dim = max(int(compute_scalar_cond_dim(cfg)), 1)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Try51 stage2 tail-refiner without GANs")
