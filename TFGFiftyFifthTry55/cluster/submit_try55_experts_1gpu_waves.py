...
def main() -> None:
    parser = argparse.ArgumentParser(description="Submit Try55 experts as two 3-job waves of 1-GPU jobs.")
    ...
    try:
        ...
        initial_cleanup = remote_sbatch(
            client,
            f"{sbatch_prefix} {cleanup_dependency}cluster/run_fiftyfifthtry55_cleanup_sert2001_1gpu.slurm",
        )
        ...
        final_cleanup = remote_sbatch(
            client,
            f"{sbatch_prefix} --dependency=afterany:{final_dependency} cluster/run_fiftyfifthtry55_cleanup_sert2001_1gpu.slurm",
        )
