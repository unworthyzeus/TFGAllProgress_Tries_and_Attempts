...
LOCAL_DIR = ROOT / "TFGFiftyFifthTry55"
REMOTE_DIR = "/scratch/nas/3/gmoreno/TFGpractice/TFGFiftyFifthTry55"
...
SHARED_FORMULA_CACHE_DIR = "/scratch/nas/3/gmoreno/TFGpractice/Datasets/try55_shared_formula_cache"
SHARED_FORMULA_PRECOMPUTED_HDF5 = f"{SHARED_FORMULA_CACHE_DIR}/try55_shared_formula_cache.h5"
...
def load_registry() -> list[dict[str, Any]]:
    registry_path = (
        LOCAL_DIR
        / "experiments"
        / "fiftyfifthtry55_partitioned_stage1"
        / "fiftyfifthtry55_expert_registry.yaml"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upload Try55 and submit the 6 topology experts as 4-GPU jobs with cleanup between jobs.")
    ...
    parser.add_argument("--wipe-try55-outputs", action="store_true")
    ...
    try:
        ...
        if args.wipe_try55_outputs:
            remote_exec(
                client,
                f"find {REMOTE_DIR}/outputs -mindepth 1 -maxdepth 1 -exec rm -rf {{}} + 2>/dev/null || true; "
                f"rm -f {REMOTE_DIR}/logs_train_fiftyfifthtry55_* {REMOTE_DIR}/logs_cleanup_fiftyfifthtry55_*",
                check=False,
            )
        ...
        if args.submit_shared_prior_precompute:
            precompute_cmd = (
                ...
                "cluster/run_fiftyfifthtry55_shared_prior_precompute_1gpu.slurm"
            )
        ...
        initial_cleanup = remote_sbatch(
            client,
            f"{sbatch_prefix} {dependency_arg}cluster/run_fiftyfifthtry55_cleanup_sert2001_1gpu.slurm",
        )
        ...
        for idx, expert in enumerate(experts):
            ...
            cmd = (
                ...
                "cluster/run_fiftyfifthtry55_partitioned_expert_4gpu.slurm"
            )
            job_id = remote_sbatch(client, cmd)
            cleanup_after = remote_sbatch(
                client,
                f"{sbatch_prefix} --dependency=afterany:{job_id} cluster/run_fiftyfifthtry55_cleanup_sert2001_1gpu.slurm",
            )
            submitted.append(
                {
