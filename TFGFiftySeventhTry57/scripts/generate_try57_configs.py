...
def main() -> None:
    ...
    out_dir = root / "experiments" / "fiftyseventhtry57_partitioned_stage1"
    classifier_dir = root / "experiments" / "fiftyseventhtry57_topology_classifier"
    out_dir.mkdir(parents=True, exist_ok=True)
    classifier_dir.mkdir(parents=True, exist_ok=True)

    for stale in out_dir.glob("fiftyseventhtry57_expert_*.yaml"):
        try:
            stale.unlink()
    ...
    for topology_class in TRY54_TOPOLOGY_CLASSES:
        ...
        cfg["data"]["path_loss_formula_input"]["cache_dir"] = "prior_cache/try57_prior_auto_city_v1"
        cfg["data"]["path_loss_formula_input"]["cache_version"] = "try57_prior_auto_city_v1"
        ...
        expert_name = f"fiftyseventhtry57_expert_{topology_class}"
    ...
    classifier_cfg = {
        ...
        "runtime": {
            "device": "cuda",
            "output_dir": "outputs/fiftyseventhtry57_topology_classifier",
            "resume_checkpoint": "",
        },
    }
    classifier_path = classifier_dir / "fiftyseventhtry57_topology_classifier.yaml"
    with classifier_path.open("w", encoding="utf-8") as handle:
        yaml.safe_dump(classifier_cfg, handle, sort_keys=False)

    registry_path = out_dir / "fiftyseventhtry57_expert_registry.yaml"
