...
`Try 57` starts from the partitioned-expert layout of `Try 54`, but changes
the training objective aggressively:

- the generator is optimized only with the **final-map RMSE**
- plus the auxiliary `no_data_loss`
- with no residual-only objective and no multiscale reconstruction term in the
  generator loss

The aim is to align optimization directly with the metric we actually care
about in validation: `path_loss.rmse_physical`.

See also:

- [TRY55_OBJECTIVE.md](C:/TFG/TFGpractice/TFGFiftySeventhTry57/TRY55_OBJECTIVE.md)
- [DATASET_QUANTIZATION_AND_COMPRESSION.md](C:/TFG/TFGpractice/TFGFiftySeventhTry57/DATASET_QUANTIZATION_AND_COMPRESSION.md)
- [GROKKING_STYLE_OPTIMIZATION_NOTE.md](C:/TFG/TFGpractice/TFGFiftySeventhTry57/GROKKING_STYLE_OPTIMIZATION_NOTE.md)
- [OVERFITTING_MITIGATION_OPTIONS.md](C:/TFG/TFGpractice/TFGFiftySeventhTry57/OVERFITTING_MITIGATION_OPTIONS.md)

The current expert profile also keeps a generator EMA with `ema_decay = 0.99`.
That is the smoothing factor for the EMA copy of the weights: values closer to
`1.0` keep more history and move more slowly, while lower values adapt faster.
We use it to make validation and checkpoint selection less noisy.

`Try 57` keeps the same **partitioned expert** strategy instead of a single
universal regressor.
