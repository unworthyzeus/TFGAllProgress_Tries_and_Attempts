...
LOCAL_DIR = ROOT / "TFGSixtyThirdTry63"
REMOTE_DIR = "/scratch/nas/3/gmoreno/TFGpractice/TFGSixtyThirdTry63"
HOST = "sert-entry-4"
USER = "gmoreno"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upload Try63 and submit coarse-to-fine 128->513 stage1->stage2 expert chain on 4 GPUs.")
    ...
    parser.add_argument("--wipe-try63-outputs", action="store_true")
    ...
    stage1_registry = load_registry(
        LOCAL_DIR / "experiments" / "sixtythirdtry63_partitioned_stage1" / "sixtythirdtry63_expert_registry.yaml"
    )
    stage2_registry = load_registry(
        LOCAL_DIR / "experiments" / "sixtythirdtry63_tail_refiner_stage2" / "sixtythirdtry63_tail_refiner_registry.yaml"
    )
    ...
    try:
        ...
        remote_exec(
            ...
            (
                f"ssh -T {args.node} "
                "\""
                "pkill -9 -u gmoreno -f 'torchrun.*TFGSixtyThirdTry63' || true; "
        ...
        for idx, expert in enumerate(stage1_registry):
            ...
            stage1_job_name = f"ckm-t63-s1-{topology_class.replace('_', '-')[:32]}"
            stage1_exports = [
        ...
        if not args.stage1_only:
            ...
            for expert in stage1_registry:
                ...
                stage2_job_name = f"ckm-t63-s2-{topology_class.replace('_', '-')[:32]}"
                stage2_exports = [
