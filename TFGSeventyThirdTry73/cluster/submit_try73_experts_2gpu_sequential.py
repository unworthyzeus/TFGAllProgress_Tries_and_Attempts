#!/usr/bin/env python3
"""Submit chained 2-GPU Slurm jobs for Try73 experts (one job per expert, sequential).

Each job uses ``cluster/run_seventythird_try73_2gpu.slurm`` (2 GPUs, 4:00:00 wall-time).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try73 2-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
