...
def apply_runtime_overrides(
    ...
    formula_precomputed_hdf5: Optional[str] = None,
    ...
    if formula_cache_dir or formula_precomputed_hdf5:
        data_cfg = dict(cfg.get('data', {}))
        ...
        if formula_precomputed_hdf5:
            formula_cfg['precomputed_hdf5'] = formula_precomputed_hdf5
...
def main() -> None:
    ...
    parser.add_argument(
        '--formula-precomputed-hdf5',
        default='',
        help='Override data.path_loss_formula_input.precomputed_hdf5 with a shared compressed HDF5 prior cache.',
    )
    parser.add_argument(
        '--resume-checkpoint',
        default='',
        help='Override runtime.resume_checkpoint.',
    )
    ...
    cfg = apply_runtime_overrides(
        ...
        args.formula_precomputed_hdf5 or None,
    ...
    ms_cfg = cfg.get('multiscale_path_loss', {}) or {}
    print(
        'loss_effective='
        f"type={loss_cfg.get('loss_type')} "
        f"huber_delta={loss_cfg.get('huber_delta')} "
        f"huber_delta_normalized={loss_cfg.get('huber_delta_normalized', False)} "
        f"generator_objective={tr_cfg.get('generator_objective')} "
        f"multiscale_loss_weight={ms_cfg.get('loss_weight')} "
        f"multiscale_enabled={ms_cfg.get('enabled')} "
        f"full_map_generator_scale={loss_cfg.get('full_map_generator_scale', 1.0)}"
    )
