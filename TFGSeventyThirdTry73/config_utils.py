...
def anchor_data_paths_to_config_file(cfg: Dict[str, Any], config_path: str) -> None:
    ...
    elif cfg_p.parent.parent.name == "experiments":
        root = cfg_p.parent.parent.parent
    else:
        root = cfg_p.parent
    data = cfg.get("data")
    ...
    formula_cfg = data.get("path_loss_formula_input")
    if isinstance(formula_cfg, dict):
        for key in ("cache_dir",):
            val = formula_cfg.get(key)
            if not val or not isinstance(val, str):
                continue
            p = Path(val)
            if p.is_absolute():
                continue
            resolved = (root / p).resolve()
            formula_cfg[key] = str(resolved)
    obstruction_cfg = data.get("path_loss_obstruction_features")
    if isinstance(obstruction_cfg, dict):
        for key in ("precomputed_hdf5",):
            val = obstruction_cfg.get(key)
            if not val or not isinstance(val, str):
                continue
            p = Path(val)
            if p.is_absolute():
                continue
            resolved = (root / p).resolve()
            obstruction_cfg[key] = str(resolved)
