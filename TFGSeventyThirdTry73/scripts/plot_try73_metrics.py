"""Plot Try 73 training metrics from validate_metrics_epoch_*.json files.

Reads the Try 73 JSON schema (single-stage 513x513, no no-data head) and
...
Usage:
  ...
  python scripts/plot_try73_metrics.py --root D:/cluster_outputs/TFGSeventyThirdTry73
  python scripts/plot_try73_metrics.py --expert-id open_sparse_lowrise
"""
...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGSeventyThirdTry73"
EXPERT_PREFIX = "seventythird_try73_expert_"
...
def _expert_key(path: Path) -> str:
    ...
    for prefix in ("seventythird_try73_expert_", "sixtysixth_expert_", "try73_expert_", "seventythird_try73_"):
...
def plot_expert(expert_id: str, output_dir: Path, save_path: Path | None = None) -> dict[str, Any]:
    ...
    ax.set_title(f"Try 73 — {topo} — Overall RMSE" + (" [EMA for val]" if uses_ema else ""))
    ax.grid(alpha=0.25)
    ...
    resolved = save_path or output_dir / "metrics_plot_try73.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 73 single-stage metrics per expert.")
