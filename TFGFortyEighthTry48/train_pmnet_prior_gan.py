...
from model_pmnet import PMNetResidualRegressor, PatchDiscriminator
...
def train_one_epoch(
    ...
    for batch in tqdm(loader, desc="train", leave=False):
        ...
        with torch.no_grad():
            with amp.autocast(device_type="cuda", enabled=amp_enabled):
                residual_fake_det = forward_cgan_generator(generator, x, scalar_cond)
        ...
        with amp.autocast(device_type="cuda", enabled=amp_enabled):
            residual_pred = forward_cgan_generator(generator, x, scalar_cond)
            pred = prior + residual_pred
            if clamp_final:
                pred = clip_to_target_range(pred, meta)
...
def main() -> None:
    ...
    generator = PMNetResidualRegressor(
    ...
    if resume_path and resume_path.exists():
        ...
        target_generator.load_state_dict(state["model"] if "model" in state else state["generator"])
