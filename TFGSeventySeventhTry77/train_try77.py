"""Try 77 — single-expert training entry point for spread prediction.

Usage:
    python train_try77.py --config experiments/seventyseventh_try77_experts/try77_expert_open_sparse_lowrise_delay_spread.yaml

DDP is launched via torchrun; single-GPU / CPU runs work without it.
"""
...
from src.config_try77 import Try77Cfg
...
from src.losses_try77 import LossWeights, combined_loss
from src.metrics_try77 import masked_rmse, masked_mae
from src.model_try77 import Try77Model, Try77ModelConfig

CODE_MARKER = "try77_stability_2026-04-20a"
...
def run_epoch(
    ...
    totals = {"total": 0.0, "map_nll": 0.0, "dist_kl": 0.0, "moment_match": 0.0,
              "outlier_budget": 0.0, "rmse_loss": 0.0, "mae_loss": 0.0}
    rmse_accum = 0.0
    mae_accum = 0.0
    n_images = 0
    ...
    for step, batch in enumerate(loader):
        ...
        totals["total"] += float(loss_terms["total"].detach().item())
        totals["map_nll"] += float(loss_terms["map_nll"].item())
        totals["dist_kl"] += float(loss_terms["dist_kl"].item())
        totals["moment_match"] += float(loss_terms["moment_match"].item())
        totals["outlier_budget"] += float(loss_terms["outlier_budget"].item())
        totals["rmse_loss"] += float(loss_terms["rmse"].item())
        totals["mae_loss"] += float(loss_terms["mae"].item())
        rmse_accum += masked_rmse(out["pred"].detach(), target, mask)
        mae_accum += masked_mae(out["pred"].detach(), target, mask)
        n_images += 1
...
def main() -> None:
    ...
    if main_:
        print(f"[rank0] code_marker={CODE_MARKER} device={device} metric={cfg.data.metric}")
    ...
    if main_:
        print(f"[rank0] expert={cfg.data.topology_class}_{cfg.data.metric}  "
              f"train={len(train_ds)} val={len(val_ds)} test={len(test_ds)}")

    def make_loader(ds: Try77ExpertDataset, train: bool) -> DataLoader:
    ...
    for epoch in range(cfg.training.epochs):
        ...
        train_stats = run_epoch(
            ...
            cfg.training.grad_accum_steps, is_train=True,
        )
        val_stats = run_epoch(
            model, val_loader, None, device, height_embed,
            LossWeights(), cfg.model.clamp_lo, cfg.model.clamp_hi,
            cfg.training.grad_accum_steps, is_train=False,
        )
        # Thesis objective on spreads is RMSE in native units (<50 ns / <20 deg).
        # We weight RMSE heavily and use KL / NLL as regularisers on shape.
        score = val_stats["rmse"] + 0.25 * val_stats["dist_kl"] + 0.1 * val_stats["map_nll"]
        dt = time.time() - t0
        row = {
            "epoch": epoch,
            "lr": lr,
        ...
        if main_:
            print(f"[epoch {epoch:3d}] lr={lr:.2e}  train RMSE={train_stats['rmse']:.3f}  "
                  f"val RMSE={val_stats['rmse']:.3f}  val map_nll={val_stats['map_nll']:.3f}  "
