"""Verify Try 76's city-holdout split matches Try 75 byte-for-byte.

Loads ``_split_hdf5_samples`` from Try 75 directly (no re-implementation)
and confronts it with ``src.data_utils.split_city_holdout``. Runs over the
real CKM HDF5 when available, otherwise a deterministic synthetic ref list.
"""
...
TRY76_ROOT = Path(__file__).resolve().parents[1]

if str(TRY76_ROOT) not in sys.path:
    sys.path.insert(0, str(TRY76_ROOT))
...
def _load_try75_split():
    ...
    except Exception as exc:  # Try 75 has heavy deps (h5py, torch, etc.)
...
@pytest.fixture
def synthetic_refs():
    # Mimic a small but realistic refs list: 20 cities, 10 samples each.
