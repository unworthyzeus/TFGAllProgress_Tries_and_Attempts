...
def main() -> None:
    ...
    stage1_dir = root / "experiments" / "sixtyfourthtry64_partitioned_stage1"
    stage2_dir = root / "experiments" / "sixtyfourthtry64_tail_refiner_stage2"
    classifier_dir = root / "experiments" / "sixtyfourthtry64_topology_classifier"
    stage1_dir.mkdir(parents=True, exist_ok=True)
    stage2_dir.mkdir(parents=True, exist_ok=True)
    classifier_dir.mkdir(parents=True, exist_ok=True)

    _cleanup_existing(stage1_dir, "sixtyfourthtry64_expert_*.yaml")
    _cleanup_existing(stage2_dir, "sixtyfourthtry64_tail_refiner_*.yaml")

    stage1_registry = {"experts": []}
    stage2_registry = {"experts": []}

    for spec in TRY64_EXPERT_SPECS:
        ...
        formula_cfg["cache_dir"] = "prior_cache/try64_prior_auto_city_v1"
        formula_cfg["cache_version"] = "try64_prior_auto_city_v1"
        ...
        stage1_name = f"sixtyfourthtry64_expert_{spec['expert_id']}"
        ...
        stage2_cfg["data"]["path_loss_formula_input"]["cache_dir"] = "prior_cache/try64_prior_auto_city_v1"
        stage2_cfg["data"]["path_loss_formula_input"]["cache_version"] = "try64_prior_auto_city_v1"
        stage2_obstruction_cfg = stage2_cfg["data"].setdefault("path_loss_obstruction_features", {})
        stage2_obstruction_cfg["enabled"] = False
        ...
        stage2_obstruction_cfg["include_shadow_depth"] = False
        stage2_obstruction_cfg["include_distance_since_los_break"] = False
        stage2_obstruction_cfg["include_max_blocker_height"] = False
        stage2_obstruction_cfg["include_blocker_count"] = False
        ...
        stage2_name = f"sixtyfourthtry64_tail_refiner_{spec['expert_id']}"
    ...
    classifier_cfg = {
        ...
        "runtime": {
            "device": "cuda",
            "output_dir": "outputs/sixtyfourthtry64_topology_classifier",
            "resume_checkpoint": "",
        },
    }
    classifier_path = classifier_dir / "sixtyfourthtry64_topology_classifier.yaml"
    with classifier_path.open("w", encoding="utf-8") as handle:
        yaml.safe_dump(classifier_cfg, handle, sort_keys=False)

    with (stage1_dir / "sixtyfourthtry64_expert_registry.yaml").open("w", encoding="utf-8") as handle:
        yaml.safe_dump(stage1_registry, handle, sort_keys=False)
    with (stage2_dir / "sixtyfourthtry64_tail_refiner_registry.yaml").open("w", encoding="utf-8") as handle:
