...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGSixtyFourthTry64"
...
def _expert_key_from_stage1_dir(path: Path) -> str:
    name = path.name
    prefix = "sixtyfourthtry64_expert_"
    return name[len(prefix):] if name.startswith(prefix) else name


def _expert_key_from_stage2_dir(path: Path) -> str:
    name = path.name
    prefix = "sixtyfourthtry64_tail_refiner_"
    return name[len(prefix):] if name.startswith(prefix) else name
...
def plot_expert_pair(
    ...
    fig, axes = plt.subplots(4, 1, figsize=(11.5, 13.5), sharex=False, constrained_layout=True)

    axes[0].plot(s1_epochs, s1_val_rmse_128, label="stage1 val RMSE 128", color="#0b7285", linewidth=2.2)
    if any(math.isfinite(v) for v in s1_val_rmse_513):
        axes[0].plot(s1_epochs, s1_val_rmse_513, label="stage1 val RMSE 513", color="#1098ad", linestyle="--", linewidth=2.0)
    axes[0].plot(s1_epochs, s1_train_rmse, label="stage1 train RMSE 128", color="#e8590c", linewidth=1.7)
    ...
    axes[0].set_title(f"Try 64 two-stage metrics - {topo}")
    axes[0].set_ylabel("RMSE (dB)")
    axes[0].grid(alpha=0.25)
    axes[0].legend(loc="best")

    delta_513_vs_128 = [
        (hi - lo) if math.isfinite(hi) and math.isfinite(lo) else float("nan")
        for lo, hi in zip(s1_val_rmse_128, s1_val_rmse_513)
    ]
    axes[1].plot(s1_epochs, s1_val_rmse_128, label="stage1 128", color="#0b7285", linewidth=2.0)
    axes[1].plot(s1_epochs, s1_val_rmse_513, label="stage1 513", color="#1098ad", linestyle="--", linewidth=2.0)
    ax1b = axes[1].twinx()
    ax1b.plot(s1_epochs, delta_513_vs_128, label="513 - 128 gap", color="#c92a2a", linewidth=1.5)
    axes[1].set_ylabel("RMSE (dB)")
    ax1b.set_ylabel("Gap (dB)")
    axes[1].grid(alpha=0.25)
    lines_a, labels_a = axes[1].get_legend_handles_labels()
    lines_b, labels_b = ax1b.get_legend_handles_labels()
    axes[1].legend(lines_a + lines_b, labels_a + labels_b, loc="best")

    axes[2].plot(s1_epochs, s1_los_rmse, label="stage1 LoS RMSE", color="#2f9e44", linewidth=1.7)
    axes[2].plot(s1_epochs, s1_nlos_rmse, label="stage1 NLoS RMSE", color="#c2255c", linewidth=1.7)
    if stage2_rows:
        axes[2].plot(s2_epochs, s2_los_rmse, label="stage2 LoS RMSE", color="#74c0fc", linewidth=1.7)
        axes[2].plot(s2_epochs, s2_nlos_rmse, label="stage2 NLoS RMSE", color="#ff87b6", linewidth=1.7)
    axes[2].set_ylabel("dB")
    axes[2].grid(alpha=0.25)
    axes[2].legend(loc="best", ncol=2)

    axes[3].plot(s1_epochs, s1_gen_loss, label="stage1 generator loss", color="#5f3dc4", linewidth=1.8)
    ax2 = axes[3].twinx()
    ax2.plot(s1_epochs, s1_lr, label="stage1 LR", color="#1c7ed6", linestyle="--", linewidth=1.4)
    if stage2_rows:
        axes[3].plot(s2_epochs, s2_loss, label="stage2 tail-refiner loss", color="#495057", linewidth=1.8)
        axes[3].plot(s2_epochs, s2_gain, label="stage2 gain vs teacher", color="#2b8a3e", linewidth=1.8)
        axes[3].axhline(0.0, color="#adb5bd", linestyle="--", linewidth=1.1)
    else:
        axes[3].text(0.5, 0.82, "No stage2 metrics found yet", ha="center", va="center", transform=axes[3].transAxes)
    axes[3].set_ylabel("Loss / Gain")
    ax2.set_ylabel("LR")
    axes[3].grid(alpha=0.25)
    lines_a, labels_a = axes[3].get_legend_handles_labels()
    lines_b, labels_b = ax2.get_legend_handles_labels()
    axes[3].legend(lines_a + lines_b, labels_a + labels_b, loc="best")
    axes[3].set_xlabel("Epoch")

    resolved_save_path = save_path or stage1_dir / "metrics_plot_try64_twostage.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 64 two-stage metrics with explicit 128 vs 513 comparison.")
    ...
    if not pairs:
        raise SystemExit(f"No Try64 stage1/stage2 output pairs found under {root_dir}")
    ...
    for expert_id, stage1_dir, stage2_dir in pairs:
        ...
        if save_root is not None:
            save_path = save_root / f"{expert_id}_metrics_plot_try64_twostage.png"
