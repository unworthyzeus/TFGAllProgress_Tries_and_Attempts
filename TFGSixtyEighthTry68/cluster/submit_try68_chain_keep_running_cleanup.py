...
Typical use: one ``t68-*`` job is already **R** on ``sert-2001``; cancel pending/held duplicates and PD
...
TRAIN_SLURM = "cluster/run_sixtyeighth_try68_4gpu.slurm"
...
def main() -> None:
    ...
    try:
        ...
        if keep and cleanup_between:
            cjid = remote_sbatch(
                client,
                f"{sbatch_prefix} {dependency}-J t68-cleanup-after-keep "
        ...
        for seg_i, (expert_id, resume_path) in enumerate(train_specs):
            ...
            job_name = f"t68-{expert_id.replace('_', '-')[:24]}-{train_idx}"
            exports = [
            ...
            if cleanup_between and seg_i < len(train_specs) - 1:
                cjid = remote_sbatch(
                    client,
                    f"{sbatch_prefix} {dependency}-J t68-cleanup-between "
                    f"--export=ALL {CLEANUP_SLURM}",
                )
