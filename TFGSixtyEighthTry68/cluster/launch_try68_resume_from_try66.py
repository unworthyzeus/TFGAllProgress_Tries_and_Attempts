#!/usr/bin/env python3
"""Copy cluster training outputs from Try 66 into Try 68, then optionally submit Try 68 jobs.

Try 68 is the same recipe as Try 66 with the PMHHNet HF-at-stem fix (see VERSIONS.md). Checkpoints
are layout-compatible (same state dict keys); training resumes from ``best_model.pt`` paths in
``try68_expert_registry.yaml`` after this copy.
...
Examples:
  ...
  python cluster/launch_try68_resume_from_try66.py --dry-run

  # Upload Try68 code (does not wipe remote outputs), copy Try66 checkpoints into Try68 outputs, then sbatch
  python cluster/launch_try68_resume_from_try66.py --submit

  # Copy only on the cluster (no code upload, no Slurm)
  python cluster/launch_try68_resume_from_try66.py
"""
...
for src in "{REMOTE_TRY66}"/try66_expert_*; do
  ...
  dst="{REMOTE_TRY68}/try68_expert_${{suf}}"
  echo "[sync] $src -> $dst"
  rm -rf "$dst"
  cp -a "$src" "$dst"
done
echo "[sync] done. Listing Try68 outputs:"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload Try68 with --no-clean-outputs, sync checkpoints from Try66, then run "
            "submit_try68_experts_4gpu_sequential.py --skip-upload."
        ),
    )
    ...
    try:
        print("REMOTE> bash -s  (sync Try66 outputs -> Try68)")
    ...
    if args.submit:
        submit = TRY68_DIR / "cluster" / "submit_try68_experts_4gpu_sequential.py"
