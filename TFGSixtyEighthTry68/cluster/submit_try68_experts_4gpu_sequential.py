#!/usr/bin/env python3
"""Submit chained 4-GPU Slurm jobs for Try68 experts (one job per expert, sequential).

Each job uses ``cluster/run_sixtyeighth_try68_4gpu.slurm`` (4 GPUs, 4:00:00 wall-time).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try68 4-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
    ...
    parser.add_argument(
        "--no-clean-outputs",
        action="store_true",
        help="When uploading, pass --no-clean-outputs (keep remote outputs/ e.g. after try66->try68 sync).",
    )
