#!/usr/bin/env python3
"""Submit chained 1-GPU Slurm jobs for Try68 experts (train -> cleanup -> …).

Same registry as ``submit_try68_experts_4gpu_sequential.py`` but uses
``cluster/run_sixtyeighth_try68_1gpu.slurm``.
"""
...
TRAIN_SLURM = "cluster/run_sixtyeighth_try68_1gpu.slurm"
CLEANUP_SLURM = "cluster/run_sixtyeighth_try68_cleanup_sert2001_1gpu.slurm"
...
def sbatch_afterany_prefix(flag_value: str) -> str:
    ...
    if bad:
        raise SystemExit(
            ...
            "Omit the flag for an independent Try68 chain."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Submit sequential Try68 1-GPU jobs (train + cleanup).")
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            job_name = f"t68-1gpu-{expert_id.replace('_', '-')[:24]}"
            exports = [
            ...
            cleanup_name = f"t68-cleanup-{expert_id.replace('_', '-')[:18]}"
            cleanup_cmd = (
