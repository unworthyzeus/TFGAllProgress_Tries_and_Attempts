#!/usr/bin/env python3
"""Upload Try 68, optionally cancel all user Slurm jobs, submit chained 4-GPU jobs for open_sparse_lowrise.

Each job uses ``cluster/run_sixtyeighth_try68_4gpu.slurm`` (4× RTX2080, 4:00:00 walltime). Jobs are
...
``outputs/try68_expert_open_sparse_lowrise/`` (path relative to the repo on the cluster). Each
...
DEFAULT_CONFIG = "experiments/sixtyeighth_try68_experts/try68_expert_open_sparse_lowrise.yaml"
DEFAULT_RESUME_CHECKPOINT = "outputs/try68_expert_open_sparse_lowrise/best_model.pt"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Submit N sequential Try68 open_sparse_lowrise 4-GPU jobs.")
    ...
    try:
        ...
        for i in range(args.count):
            ...
            cmd = (
                ...
                "cluster/run_sixtyeighth_try68_4gpu.slurm"
            )
