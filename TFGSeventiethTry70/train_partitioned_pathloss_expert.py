...
from model_try70_multiquad import PMHHNetTry70MultiQuad, try70_auxiliary_loss, try70_blend_search_rmse_physical
...
def _build_pmnet_from_cfg(cfg: Dict[str, Any], in_channels: int) -> nn.Module:
    ...
    if arch in ("pmhhnet_try70", "try70", "pmhhnet_multiquad"):
...
def evaluate_validation(
    ...
    *,
    ...
    try70_blend_diag: Optional[Dict[str, Any]] = None,
) -> dict[str, Any]:
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(loader, desc="val", leave=False, disable=distributed and not is_main_process(rank))):
            ...
            gen_val = unwrap_model(model)
            if hasattr(gen_val, "pop_last_aux"):
                aux_b = gen_val.pop_last_aux()
                if (
                    aux_b is not None
                    and try70_blend_diag is not None
                    and len(try70_blend_diag) == 0
                    and bool(dict(cfg.get("try70", {})).get("blend_report_first_batch", False))
                    and is_main_process(rank)
                ):
                    try70_blend_diag.update(
                        try70_blend_search_rmse_physical(aux_b, pred, prior, y[:, :1], m[:, :1], meta)
                    )
    ...
    if try70_blend_diag is not None and len(try70_blend_diag) > 0:
        summary["try70_blend_first_batch"] = dict(try70_blend_diag)
    return summary
...
def train_one_epoch(
    ...
    *,
    ...
    for batch in tqdm(loader, desc="train", leave=False, disable=distributed and not is_main_process(rank)):
        ...
        with amp.autocast(device_type="cuda", enabled=amp_enabled):
            term_try70_aux = torch.zeros((), device=target.device)
            ...
            w70 = float(try70_cfg.get("aux_loss_weight", 0.0))
            if hasattr(source_generator, "pop_last_aux"):
                pack70 = source_generator.pop_last_aux()
                if w70 > 0.0 and pack70 is not None:
                    term_try70_aux = try70_auxiliary_loss(
                        pack70, target, mask, huber_delta=huber_delta_eff, weight=w70
                    )
                    g_loss = g_loss + term_try70_aux
        ...
        running_term_try70_aux += float(term_try70_aux.item())
        running_main_map_regression += float(main_map_reg.detach().item())
        steps += 1
        if is_main_process(rank) and out_dir is not None and (steps == 1 or steps % progress_every == 0 or steps == len(loader)):
            ...
            live_payload = {
                ...
                "loss_components_running": {
                    ...
                    "term_try70_aux": float(running_term_try70_aux / steps),
    ...
    train_loss_components = {
        ...
        "term_try70_aux": float(running_term_try70_aux / denom),
...
def main() -> None:
    ...
    _arch_for_ddp = str(cfg.get("model", {}).get("arch", "pmnet")).lower()
    _ddp_find_unused = _arch_for_ddp in ("pmhhnet_try70", "try70", "pmhhnet_multiquad")
    if distributed:
    ...
    if not (resume_path and resume_path.exists()):
        try70_init = dict(cfg.get("try70", {})).get("init_checkpoint") or dict(cfg.get("try70", {})).get(
            "warm_start_checkpoint"
        )
        if try70_init:
            p70 = Path(str(try70_init))
            if not p70.exists():
                p70 = (Path(__file__).resolve().parent / str(try70_init)).resolve()
            if p70.exists():
                tg70 = generator_for_training.module if isinstance(generator_for_training, DistributedDataParallel) else generator_for_training
                st70 = load_torch_checkpoint(p70, torch.device("cpu"))
                raw70 = _checkpoint_model_state(st70)
                miss70, unex70 = tg70.load_state_dict(raw70, strict=False)
                if is_main_process(rank):
                    print(
                        json.dumps(
                            {
                                "try70_init_checkpoint": str(p70),
                                "missing_keys": len(miss70),
                                "unexpected_keys": len(unex70),
                            }
                        )
                    )
            elif is_main_process(rank):
                print(json.dumps({"try70_init_checkpoint_missing": str(try70_init)}))
    ...
    try:
        for epoch in range(start_epoch, int(cfg["training"]["epochs"]) + 1):
            ...
            try70_blend_box: Optional[Dict[str, Any]] = None
            if bool(dict(cfg.get("try70", {})).get("blend_report_first_batch", False)):
                try70_blend_box = {}
            val_summary = evaluate_validation(
                ...
                try70_blend_diag=try70_blend_box,
            )
