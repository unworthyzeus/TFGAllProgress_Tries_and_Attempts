#!/usr/bin/env python3
"""Short component ablations for Try 68 (same expert YAML, same seed).
...
Try 68 trainer does **not** read ``corridor_weighting`` (YAML may still list it
from the Try 66 template; toggling it here is a no-op until wired in code).

Example (from ``TFGSixtyEighthTry68/``):

  python scripts/run_try68_component_ablation.py \\
    --base-config experiments/sixtyeighth_try68_experts/try68_expert_open_sparse_lowrise.yaml \\
    --epochs-override 4 \\
...
Solo validación con checkpoint ya entrenado (DirectML):
  ...
  python scripts/run_try68_component_ablation.py \\
    --eval-only --checkpoint outputs/try68_expert_open_sparse_lowrise/best_model.pt \\
    --base-config experiments/sixtyeighth_try68_experts/try68_expert_open_sparse_lowrise.yaml \\
...
def _ablation_presets() -> List[Tuple[str, Dict[Tuple[str, ...], Any]]]:
    """Name -> nested-key overrides. Only keys honored by Try 68 train/data."""
    return [
        ("baseline", {}),
        (
            "no_multiscale",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Try 68 short component ablations")
    parser.add_argument("--base-config", type=Path, required=True, help="Expert YAML (Try 68)")
    parser.add_argument("--epochs-override", type=int, default=4, help="Short train for each variant")
    parser.add_argument(
        "--staging-dir",
        type=Path,
        default=ROOT / "experiments" / "sixtyeighth_try68_ablation_staging",
