"""Generate Try 68 configs: 6 topology experts (single-stage 513x513) + classifier.
...
Anti-overfitting: CutMix, higher dropout, higher weight decay, gradient accumulation.
  ...
  - Gradient accumulation of 4 with batch_size=1 gives effective batch of 4.

NLoS convergence: pixel-level NLoS reweighting in the loss.
  - LoS:NLoS valid pixel ratio is ~10:1. NLoS pixels get 2.5x weight.
"""
...
MODEL_SPECS = {
    "open_sparse_lowrise":   {"base_channels": 40, "hf_channels": 16},
    "open_sparse_vertical":  {"base_channels": 40, "hf_channels": 16},
    "mixed_compact_lowrise": {"base_channels": 40, "hf_channels": 16},
    "mixed_compact_midrise": {"base_channels": 48, "hf_channels": 20},
    "dense_block_midrise":   {"base_channels": 48, "hf_channels": 20},
    "dense_block_highrise":  {"base_channels": 48, "hf_channels": 20},
}
...
def _apply_common_data(cfg: dict, topology_class: str) -> None:
    ...
    formula["cache_dir"] = "prior_cache/try68_prior_auto_city_v1"
    formula["cache_version"] = "try68_prior_auto_city_v1"
...
def main() -> None:
    ...
    expert_dir = root / "experiments" / "sixtyeighth_try68_experts"
    cl_dir = root / "experiments" / "sixtyeighth_try68_classifier"
    expert_dir.mkdir(parents=True, exist_ok=True)
    cl_dir.mkdir(parents=True, exist_ok=True)

    _cleanup(expert_dir, "try68_expert_*.yaml")
    ...
    for spec in EXPERT_SPECS:
        ...
        name = f"try68_expert_{spec['expert_id']}"
    ...
    cl_cfg = {
        ...
        "runtime": {
            "device": "cuda",
            "output_dir": "outputs/try68_topology_classifier",
            "resume_checkpoint": "",
        },
    }
    cl_path = cl_dir / "try68_topology_classifier.yaml"
    with cl_path.open("w", encoding="utf-8") as f:
        yaml.safe_dump(cl_cfg, f, sort_keys=False)

    with (expert_dir / "try68_expert_registry.yaml").open("w", encoding="utf-8") as f:
