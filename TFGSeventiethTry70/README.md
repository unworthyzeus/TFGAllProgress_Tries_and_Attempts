# Try 70 — Multi-scale quad heads on Try 68 PMHHNet

This directory is **`TFGSeventiethTry70`**: same HDF5 / six-expert layout as **Try 68**, but the default expert YAML can use **`model.arch: pmhhnet_try70`** so `train_partitioned_pathloss_expert.py` builds **`PMHHNetTry70MultiQuad`** (`model_try70_multiquad.py`): one **513×513** residual plus auxiliary residuals at **257×257** (4 quadrants + 1 global), **129×129** (4×4 + 1 global), **65×65** (8×8 + 1 global). Optional **`try70.init_checkpoint`** warm-starts from a Try 68 `best_model.pt` (`strict=False` for new heads). **`try70.blend_report_first_batch`** logs residual-space blend RMSE on the first validation batch.

**Cluster (sert):** cancel all user jobs and submit four chained 4×RTX2080 segments (warm-start job 1 from Try 68 open sparse `best_model.pt`):

```bash
cd TFGpractice
export SSH_PASSWORD=...   # or use --ssh-key
python TFGSeventiethTry70/cluster/submit_try70_open_sparse_4gpu_sequential.py --cancel-all-user-jobs
```

Last cluster submit (2026-04-14, sert): chained Slurm jobs **10030660–10030663** (`t70-osl-4gpu-1..4`, node `sert-2001`); see `VERSIONS.md` Try 70 for exports and warm-start path.

Slurm: `cluster/run_seventieth_try70_4gpu.slurm`. Runtime HDF5 and optional `TRY70_INIT_CHECKPOINT` are merged by `cluster/prepare_runtime_config.py`.

Full version table: `TFGpractice/VERSIONS.md` (Try 70 row + section).

---

# Try 68 — Try 66 re-run with PMHHNet stem + HF fix (inherited README body)

The text below still describes the **Try 66 / Try 68** synthesis design copied into this folder. For Try-70-specific training, use **`experiments/seventieth_try70_experts/`** and `model.arch: pmhhnet_try70`.

`Try 68` reuses the **Try 66** pipeline (6 topology-partitioned experts, same configs and training code layout). Intentional code differences from the Try 66 fork:

1. **`PMHHNetResidualRegressor.forward`:** restore **`x0 = stem(x) + hf_project(|∇²x|)`** before FiLM on the stem, matching `PMHNetResidualRegressor`, instead of feeding HF only at the FPN fusion (that omission hurt edge-driven regression).
2. **`train_partitioned_pathloss_expert.py`:** **no CutMix** when height **`scalar_cond`** is active (FiLM), so spatial mixes do not paste another sample’s height-conditioned maps under the current sample’s global height (same logic as Try 67; Try 66 still had the inconsistent pattern).

Bootstrap: `TFGpractice/scripts/bootstrap_try68_from_try66.py`. Cluster resume: `cluster/launch_try68_resume_from_try66.py` (sync `outputs/try66_expert_*` → `try68_expert_*`); when submitting after a sync, use **`submit_try68_experts_4gpu_sequential.py --no-clean-outputs`** so the uploader does not delete resumed checkpoints.

Short component ablations (multiscale, CutMix, NLoS reweight, TTA, Huber vs RMSE main term): `scripts/run_try68_component_ablation.py` plus `train_partitioned_pathloss_expert.py --epochs-override`.

Full changelog: `TFGpractice/VERSIONS.md` (Try 66 architecture note, Try 68 section).

---

The remainder of this README still describes the **Try 66 synthesis** design (literature, inputs, losses) that Try 68 inherits unchanged.
...
| 7 | ~~Stage 2 height FiLM~~ | **Removed** — single-stage 513x513 direct training; tail refiner added <0.5 dB |
...
|---|---|---|
| Topology-partitioned experts (6 classes) | Try 54 | Per-expert RMSE is consistently lower than monolithic models |
...
| ~~Two-stage coarse-to-fine~~ (removed) | Try 62-64 | Refiner added <0.5 dB gain; single-stage 513 is simpler and at full-res |
...
## What is new in Try 68 — Paper-backed additions
...
### 3. ~~Height-aware Stage 2 refiner~~ → Removed (single-stage 513x513)

**Decision:** The two-stage pipeline (256→513 refiner) has been removed in favor
of direct 513x513 training. Across Try 49-64, the Stage 2 tail refiner
...
**How we implement it:** At data loading time, compute a soft 2D weight map for
each sample where:
...
Input (8 channels):
  ...
  tx_depth_map + elevation_angle_map + building_mask

       ↓ [per-sample routing by topology class]

Topology Classifier → 1 of 6 experts:
  - open_sparse_lowrise   (base_ch=40, hf_ch=16)
  - open_sparse_vertical  (base_ch=40, hf_ch=16)
  - mixed_compact_lowrise (base_ch=40, hf_ch=16)
  - mixed_compact_midrise (base_ch=48, hf_ch=20)
  - dense_block_midrise   (base_ch=48, hf_ch=20)
  - dense_block_highrise  (base_ch=48, hf_ch=20)
...
Loss:
  - Huber loss (δ=6.0 dB) with NLoS pixel reweighting (2.5×)
  - Multiscale loss at scales [2, 4]
  - LR warmup (5 epochs, linear from 1% to 100%)
  - CutMix augmentation (p=0.25)
...
|---|---|---|
| Resolution | **513×513 direct** | Single-stage; no Stage 2 refiner (added <0.5 dB in Try 49-64) |
| Optimizer | AdamW | Standard for modern training; decoupled weight decay |
| Learning rate | 3e-4 | Cosine warm restarts handles annealing |
| Weight decay | 1.5e-2 | Higher than Try 64 (1e-2) for anti-overfitting |
| LR schedule | CosineAnnealingWarmRestarts | SGDR (Loshchilov & Hutter 2017), `T_0=40`, `T_mult=2`, 5-epoch linear warmup |
| Loss (main) | Huber (`delta=6.0 dB`) | Robust to NLoS outliers; linear above 6 dB |
...
| NLoS reweight | 2.5× on NLoS pixels | Compensates 10:1 LoS/NLoS imbalance |
| CutMix | p=0.25, Beta(1,1) | Strongest regularizer for limited data (MICCAI 2024) |
| EMA decay | 0.995 | Slightly higher than 0.99 for more stable averaging |
| Batch size | 1 (grad accum 4 → eff. 4) | 513×513 images; accumulation simulates larger batch |
| base_channels | 40 (sparse) / 48 (dense) | Capacity needed for 8 FiLM injection points |
| hf_channels | 16 (sparse) / 20 (dense) | High-frequency Laplacian branch |
| Epochs | 300 | ~7 cosine cycles (40 + 80 + 160 + partial) |
| Dropout | 0.12 | Raised from 0.08; compensates increased model capacity |
...
| Stage 2 tail refiner | Only 0.2-0.5 dB gain; below uint8 quantization noise; doubles training time |
...
- **NLoS pixel reweighting**: Try 61 used *regime-level* reweighting (by city type),
  which was too coarse. Try 68 uses *pixel-level* NLoS reweighting (2.5× on NLoS
  pixels identified by LoS mask channel), which directly addresses the 10:1
  LoS/NLoS gradient imbalance.
...
- `TOPOLOGY_CLUSTERING_6_CLASSES.md` — topology partitioning heuristics
- `PMNET_VS_PMHHNET.md` — PMHHNet architecture rationale
...
The target for Try 68 is to push per-expert RMSE lower than Try 64 while
...
- `train_topology_classifier.py` — Topology classifier
- `scripts/generate_try68_configs.py` — Config generator (6 experts + classifier)
- `scripts/plot_try68_metrics.py` — Metric plotting (5-panel: RMSE, LoS/NLoS, gain, loss, timing)
- `cluster/run_sixtyeighth_try68_4gpu.slurm` — Cluster entry
- `cluster/submit_try68_stage1_stage2_4gpu.py` — Cluster submission (sequential 6-expert chain)
...
```powershell
C:\TFG\.venv\Scripts\python.exe C:\TFG\TFGpractice\TFGSixtyEighthTry68\scripts\generate_try68_configs.py
```
...
```powershell
$env:SSH_PASSWORD = '***'
python C:\TFG\TFGpractice\TFGSixtyEighthTry68\cluster\submit_try68_stage1_stage2_4gpu.py
```
