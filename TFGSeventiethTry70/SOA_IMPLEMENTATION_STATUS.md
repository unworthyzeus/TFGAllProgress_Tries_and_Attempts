# Try 68 — SOA claims vs. what is implemented

> Companion to **`SOA_COMPARISON.md`**. This file tracks which *thesis / SOA narrative* items are **already in the Try 68 codebase** versus **not implemented, partial, or config-only**.  
...
| **Topology-partitioned experts** (6 classes + registry) | **Implemented** | `try68_expert_registry.yaml`, `partition_filter.topology_class`, separate checkpoints |
...
| **Topology classifier** (Try 68) | **Separate script** | `train_topology_classifier.py` + `try68_topology_classifier.yaml` — not the same run as path-loss experts |
...
| **Test-time adaptation / calibration** (cf. RadioPiT, AIRMap-style) | **Not implemented** | Explicitly “No” in SOA fair-comparison table for Try 68 |
...
These are cited in **`SOA_COMPARISON.md` §7** as *future* ideas — **none** are integrated as first-class Try 68 features today:
