#!/usr/bin/env python3
"""Submit chained 4-GPU Slurm jobs for Try74 experts (one job per expert, sequential).

Each job uses ``cluster/run_seventyfourth_try74_4gpu.slurm`` (4 GPUs, 4:00:00 wall-time).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try74 4-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
        help=(
            "expert_id values from try74_expert_registry.yaml. "
    ...
    parser.add_argument(
        "--no-clean-outputs",
        action="store_true",
        help="When uploading, pass --no-clean-outputs (keep remote outputs/ for partial Try74 reruns).",
    )
