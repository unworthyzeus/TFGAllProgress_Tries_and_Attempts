#!/usr/bin/env python3
"""Submit chained 2-GPU Slurm jobs for Try74 experts (one job per expert, sequential).

Each job uses ``cluster/run_seventyfourth_try74_2gpu.slurm`` (2 GPUs, 2 days wall-time).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try74 2-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
