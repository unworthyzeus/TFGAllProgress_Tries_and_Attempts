#!/usr/bin/env python3
"""Upload Try 73 and submit 6 single-stage expert jobs on 4 GPUs.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Upload Try 73 and submit 6 single-stage expert jobs on 4 GPUs."
    )
    ...
    registry = load_registry(
        LOCAL_DIR / "experiments" / "seventythird_try73_experts" / "try73_expert_registry.yaml"
    )
    ...
    try:
        ...
        for idx, expert in enumerate(registry):
            ...
            job_name = f"t73-{tc.replace('_', '-')[:32]}"
            exports = [
