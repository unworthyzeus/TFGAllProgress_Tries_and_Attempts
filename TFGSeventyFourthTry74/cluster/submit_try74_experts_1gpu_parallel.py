#!/usr/bin/env python3
"""Submit Try74 experts as parallel 1-GPU Slurm jobs (no sequential dependency chain).

Each train job uses cluster/run_seventyfourth_try74_1gpu.slurm.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit parallel Try74 1-GPU jobs (one Slurm job per expert, same time)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
    ...
    try:
        ...
        if args.final_cleanup and train_job_ids:
            ...
            submitted.append(("t74-cleanup-final", cleanup_job_id))
