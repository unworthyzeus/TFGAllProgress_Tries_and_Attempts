#!/usr/bin/env python3
"""Submit chained 4-GPU Slurm jobs for Try73 experts (one job per expert, sequential).

Each job uses ``cluster/run_seventythird_try73_4gpu.slurm`` (4 GPUs, 4:00:00 wall-time).
Experts are resolved from ``experiments/seventythird_try73_experts/try73_expert_registry.yaml``.
...
LOCAL_DIR = ROOT / "TFGSeventyThirdTry73"
REGISTRY = LOCAL_DIR / "experiments" / "seventythird_try73_experts" / "try73_expert_registry.yaml"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try73 4-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
        help=(
            "expert_id values from try73_expert_registry.yaml. "
    ...
    parser.add_argument(
        "--no-clean-outputs",
        action="store_true",
        help="When uploading, pass --no-clean-outputs (keep remote outputs/ e.g. after try66->try73 sync).",
    )
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            job_name = f"t73-{expert_id.replace('_', '-')[:28]}"
            exports = [
            ...
            cleanup_name = f"t73-cleanup-{expert_id.replace('_', '-')[:20]}"
            cleanup_cmd = (
                f"{sbatch_prefix} --dependency=afterany:{train_job_id} "
                f"-J {cleanup_name} "
                "cluster/run_seventythird_try73_cleanup_sert2001_1gpu.slurm"
            )
