#!/usr/bin/env python3
"""Submit chained 1-GPU Slurm jobs for Try74 experts (one job per expert, sequential).

Each job uses cluster/run_seventyfourth_try74_1gpu.slurm (1 GPU, 2 days wall-time).
Experts are resolved from experiments/seventyfourth_try74_experts/try74_expert_registry.yaml.
...
LOCAL_DIR = ROOT / "TFGSeventyFourthTry74"
REGISTRY = LOCAL_DIR / "experiments" / "seventyfourth_try74_experts" / "try74_expert_registry.yaml"
REMOTE_DIR = "/scratch/nas/3/gmoreno/TFGpractice/TFGSeventyFourthTry74"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try74 1-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
        help="expert_id values from try74_expert_registry.yaml. Defaults to all experts in registry order.",
    )
    ...
    parser.add_argument("--base-master-port", type=int, default=30586)
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            job_name = f"t74-{expert_id.replace('_', '-')[:28]}"
            exports = [
            ...
            train_cmd = (
                ...
                "cluster/run_seventyfourth_try74_1gpu.slurm"
            )
            train_job_id = remote_sbatch(client, train_cmd)
            submitted.append((job_name, train_job_id))

            cleanup_name = f"t74-cleanup-{expert_id.replace('_', '-')[:20]}"
            cleanup_cmd = (
                f"{sbatch_prefix} --dependency=afterany:{train_job_id} "
                f"-J {cleanup_name} "
                "cluster/run_seventyfourth_try74_cleanup_sert2001_1gpu.slurm"
            )
