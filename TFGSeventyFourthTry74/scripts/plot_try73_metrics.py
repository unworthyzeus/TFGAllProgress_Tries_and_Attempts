"""Plot Try 74 training metrics from validate_metrics_epoch_*.json files.

Reads the Try 74 JSON schema (single-stage 513x513, no no-data head) and
...
Usage:
  python scripts/plot_try74_metrics.py
  python scripts/plot_try73_metrics.py
  python scripts/plot_try74_metrics.py --root D:/cluster_outputs/TFGSeventyFourthTry74
  python scripts/plot_try74_metrics.py --expert-id open_lowrise
"""
...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGSeventyFourthTry74"
EXPERT_PREFIX = "try74_expert_"
...
def _expert_key(path: Path) -> str:
    ...
    for prefix in (
        "seventyfourth_try74_expert_",
        "try74_expert_",
        "seventythird_try73_expert_",
        "try73_expert_",
        "seventyfourth_try74_",
    ):
...
def _focus_role(expert_id: str) -> str | None:
    eid = expert_id.lower()
    if eid.endswith("_los") or "los_only" in eid:
        return "LoS-only"
    if eid.endswith("_nlos") or "nlos_only" in eid:
        return "NLoS-only"
    return None


def _display_name(expert_id: str, topology_class: str) -> str:
    role = _focus_role(expert_id)
    if role:
        return f"{topology_class} [{role}]"
...
def plot_expert(expert_id: str, output_dir: Path, save_path: Path | None = None) -> dict[str, Any]:
    ...
    topo_raw = rows[0].get("topology_class") or expert_id
    topo = _display_name(expert_id, str(topo_raw))
    ...
    ax.set_title(f"Try 74 — {topo} — Overall RMSE" + (" [EMA for val]" if uses_ema else ""))
    ax.grid(alpha=0.25)
    ...
    if role is None:
        ...
        ax.set_title(f"LoS vs NLoS RMSE ({role} supervision)")
    ax.grid(alpha=0.25)
    ...
    resolved = save_path or output_dir / "metrics_plot_try74.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 74 single-stage metrics per expert.")
    ...
    for d in dirs:
        eid = _expert_key(d)
        save = Path(args.save_path) / f"{eid}_metrics_try73.png" if args.save_path else None
        try:
