"""Cancel all jobs, wipe Try 72 logs (keep checkpoints), upload code, and relaunch all experts."""
...
def main() -> None:
    ...
    run(
        c,
        f"rm -f {REMOTE_DIR}/logs_train_try72_*.out {REMOTE_DIR}/logs_train_try72_*.err "
        f"{REMOTE_DIR}/logs_train_try72_chain_*.out {REMOTE_DIR}/logs_train_try72_chain_*.err",
    )

    # 3. Upload code
    print("\n=== Step 3: Upload code ===")
    run(c, f"mkdir -p {REMOTE_DIR}/experiments/seventysecond_try72_experts")
    run(c, f"mkdir -p {REMOTE_DIR}/experiments/seventysecond_try72_classifier")
    ...
    config = "experiments/seventysecond_try72_experts/try72_expert_open_sparse_lowrise.yaml"
    job_name = "t72-open-sparse-lowrise"
    cleanup_script = "cluster/run_seventysecond_try72_cleanup_sert2001_1gpu.slurm"
    submitted = []
    current_dep = ""
    ...
    for run_idx in range(3):
        ...
        cmd = (
            ...
            "cluster/run_seventysecond_try72_3gpu.slurm"
        )
        out = run(c, cmd)
