#!/usr/bin/env python3
"""Copy cluster training outputs from Try 66 into Try 72, then optionally submit Try 72 jobs.

Try 72 is the same recipe as Try 66 with the PMHHNet HF-at-stem fix (see VERSIONS.md). Checkpoints
are layout-compatible (same state dict keys); training resumes from ``best_model.pt`` paths in
``try72_expert_registry.yaml`` after this copy.
...
Examples:
  ...
  python cluster/launch_try72_resume_from_try66.py --dry-run

  # Upload Try72 code (does not wipe remote outputs), copy Try66 checkpoints into Try72 outputs, then sbatch
  python cluster/launch_try72_resume_from_try66.py --submit

  # Copy only on the cluster (no code upload, no Slurm)
  python cluster/launch_try72_resume_from_try66.py
"""
...
mkdir -p "{REMOTE_TRY72}"
shopt -s nullglob
for src in "{REMOTE_TRY66}"/try66_expert_*; do
  [ -d "$src" ] || continue
  base=$(basename "$src")
  suf=${{base#try66_expert_}}
  dst="{REMOTE_TRY72}/try72_expert_${{suf}}"
  echo "[sync] $src -> $dst"
  rm -rf "$dst"
  cp -a "$src" "$dst"
done
echo "[sync] done. Listing Try72 outputs:"
ls -la "{REMOTE_TRY72}" || true
"""
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload Try72 with --no-clean-outputs, sync checkpoints from Try66, then run "
            "submit_try72_experts_4gpu_sequential.py --skip-upload."
        ),
    )
    ...
    try:
        print("REMOTE> bash -s  (sync Try66 outputs -> Try72)")
    ...
    if args.submit:
        submit = TRY72_DIR / "cluster" / "submit_try72_experts_4gpu_sequential.py"
