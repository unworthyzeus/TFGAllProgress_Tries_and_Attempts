#!/usr/bin/env python3
"""Submit chained 2-GPU Slurm jobs for Try72 experts (one job per expert, sequential).

Each job uses ``cluster/run_seventysecond_try72_2gpu.slurm`` (2 GPUs, 4:00:00 wall-time).
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit sequential Try72 2-GPU jobs (one per expert, with cleanup between)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            train_cmd = (
                ...
                "cluster/run_seventysecond_try72_2gpu.slurm"
            )
