...
def apply_receiver_subsample_mask(
    mask: torch.Tensor,
    data_cfg: Dict[str, Any],
    *,
    seed: int,
) -> torch.Tensor:
    """Gate receiver pixels for sparse supervision / metrics (Try 72+).

    When ``data.receiver_subsample.enabled`` is false, returns a tensor of ones so
    ``mask * gate`` leaves ``mask`` unchanged.

    Otherwise:
      1. Partition valid pixels into ground tiles of ``tile_side_m`` meters using
         ``meters_per_pixel`` (from subsample block or ``path_loss_formula_input``).
      2. In each tile, keep up to ``max_rx_per_tile`` receivers sampled uniformly.
      3. From the union, subsample down to about ``keep_fraction`` times the **original**
         valid pixel count (at least one pixel when possible).

    ``seed`` should change per train step / epoch for train; use a fixed derivation for val.
    """
    rcfg = dict(data_cfg.get("receiver_subsample", {}) or {})
    if not bool(rcfg.get("enabled", False)):
        return torch.ones_like(mask)
    keep_fraction = float(rcfg.get("keep_fraction", 1.0))
    keep_fraction = min(max(keep_fraction, 1e-6), 1.0)
    tile_side_m = float(rcfg.get("tile_side_m", 1000.0))
    if tile_side_m <= 1e-9:
        tile_side_m = 1000.0
    max_rx_per_tile = int(rcfg.get("max_rx_per_tile", 10**9))
    if max_rx_per_tile < 1:
        max_rx_per_tile = 1
    formula = dict(data_cfg.get("path_loss_formula_input", {}) or {})
    mpp = float(rcfg.get("meters_per_pixel", formula.get("meters_per_pixel", 1.0)) or 1.0)
    if mpp <= 0.0:
        mpp = 1.0
    ...
    dtype = mask.dtype
    b, _, h, w = mask.shape
    base = (mask > 0).detach().cpu().numpy()
    out_np = np.zeros((b, 1, h, w), dtype=np.float32)
    rng = np.random.default_rng(int(seed) & 0x7FFFFFFF)

    for bi in range(b):
        rows, cols = np.nonzero(base[bi, 0])
        if rows.size == 0:
            continue
        n_orig = int(rows.size)
        tr = (rows.astype(np.float64) * mpp / tile_side_m).astype(np.int64)
        tc = (cols.astype(np.float64) * mpp / tile_side_m).astype(np.int64)
        groups: Dict[Tuple[int, int], list[int]] = defaultdict(list)
        for k in range(n_orig):
            groups[(int(tr[k]), int(tc[k]))].append(k)
        pool_rc: list[Tuple[int, int]] = []
        for _tid, idxs in groups.items():
            idxs_arr = np.asarray(idxs, dtype=np.int64)
            k_take = min(int(idxs_arr.size), max_rx_per_tile)
            pick = rng.choice(idxs_arr, size=k_take, replace=False)
            for j in pick:
                pool_rc.append((int(rows[j]), int(cols[j])))
        target_k = max(1, int(round(keep_fraction * float(n_orig))))
        target_k = min(target_k, n_orig)
        if len(pool_rc) > target_k:
            sel = rng.choice(len(pool_rc), size=target_k, replace=False)
            pool_rc = [pool_rc[int(i)] for i in sel]
        for r, c in pool_rc:
            out_np[bi, 0, r, c] = 1.0
