# Try 72 — Try 68 fork: sparse receivers + compact model

`Try 72` is based on **`TFGSixtyEighthTry68`** (six topology experts, `train_partitioned_pathloss_expert.py`, PMHHNet, prior + residual, same HDF5 and `city_holdout` splits), with these **deliberate changes**:

1. **Sparse receiver supervision** — `data.receiver_subsample` gates which valid ground pixels contribute to the loss and to the **primary** validation RMSE. Roughly **`keep_fraction` of all originally valid pixels** remain (default **0.01** = 1%), after a **per `tile_side_m` metre** cap (`max_rx_per_tile`). Training draws a **new random mask each step**; validation uses `val_seed` for a fixed mask per run. Dense reference metrics: `path_loss_dense_reference` in the epoch JSON (same as `build_validation_payload` → `metrics.path_loss_dense_reference`).
2. **Smaller backbone** — `base_channels: 20`, `hf_channels: 10` for every expert (set in `scripts/generate_try72_configs.py`).
3. **Larger per-step batch** — `batch_size: 4`, `val_batch_size: 4`, `gradient_accumulation_steps: 4` (effective batch still scaled by accumulation).

**Regenerate expert YAMLs:** `python scripts/generate_try72_configs.py`  
**Train:** `python train_partitioned_pathloss_expert.py --config experiments/seventysecond_try72_experts/try72_expert_<id>.yaml`  
**Cluster:** `cluster/submit_try72_experts_4gpu_sequential.py` (and sibling `submit_try72_*.py`, `run_seventysecond_try72_*.slurm`).  
**Plots:** `python scripts/plot_try72_metrics.py` (sparse vs dense-ref RMSE on panel 0).

**Canonical version write-up:** `TFGpractice/VERSIONS.md` (header **Try 1 → Try 72**, section **Try 72**, and **Current status** bullet).

---

The long sections below still describe the **shared CKM path-loss project** (inputs, literature, losses) as inherited from the Try 54–68 line; they are not repeated in `VERSIONS.md` in full.
...
## What is new in Try 72 — Paper-backed additions
...
- **NLoS pixel reweighting**: Try 61 used *regime-level* reweighting (by city type),
  which was too coarse. Try 72 uses *pixel-level* NLoS reweighting (2.5× on NLoS
...
The target for Try 72 is to push per-expert RMSE lower than Try 64 while
...
- `scripts/generate_try72_configs.py` — Config generator (6 experts + classifier)
- `scripts/plot_try72_metrics.py` — Metric plotting (5-panel: RMSE, LoS/NLoS, gain, loss, timing)
- `cluster/run_seventysecond_try72_4gpu.slurm` — Cluster entry
- `cluster/submit_try72_stage1_stage2_4gpu.py` — Cluster submission (sequential 6-expert chain)
...
```powershell
C:\TFG\.venv\Scripts\python.exe C:\TFG\TFGpractice\TFGSeventySecondTry72\scripts\generate_try72_configs.py
```
...
```powershell
$env:SSH_PASSWORD = '***'
python C:\TFG\TFGpractice\TFGSeventySecondTry72\cluster\submit_try72_stage1_stage2_4gpu.py
```
