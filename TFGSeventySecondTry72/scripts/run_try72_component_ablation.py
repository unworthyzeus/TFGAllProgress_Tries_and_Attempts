#!/usr/bin/env python3
"""Short component ablations for Try 72 (same expert YAML, same seed).
...
Try 72 trainer does **not** read ``corridor_weighting`` (YAML may still list it
from the Try 66 template; toggling it here is a no-op until wired in code).

Example (from ``TFGSeventySecondTry72/``):

  python scripts/run_try72_component_ablation.py \\
    --base-config experiments/seventysecond_try72_experts/try72_expert_open_sparse_lowrise.yaml \\
    --epochs-override 4 \\
    --output-json outputs/try72_ablation_table.json
...
Solo validación con checkpoint ya entrenado (DirectML):
  ...
  python scripts/run_try72_component_ablation.py \\
    --eval-only --checkpoint outputs/try72_expert_open_sparse_lowrise/best_model.pt \\
    --base-config experiments/seventysecond_try72_experts/try72_expert_open_sparse_lowrise.yaml \\
...
def _ablation_presets() -> List[Tuple[str, Dict[Tuple[str, ...], Any]]]:
    """Name -> nested-key overrides. Only keys honored by Try 72 train/data."""
    return [
        ("baseline", {}),
        (
            "no_multiscale",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Try 72 short component ablations")
    parser.add_argument("--base-config", type=Path, required=True, help="Expert YAML (Try 72)")
    parser.add_argument("--epochs-override", type=int, default=4, help="Short train for each variant")
    parser.add_argument(
        "--staging-dir",
        type=Path,
        default=ROOT / "experiments" / "seventysecond_try72_ablation_staging",
    ...
    if args.eval_only:
        for name, overrides, eval_extra in _eval_presets_filtered(args.presets):
            ...
            tmp_yaml = staging / f"try72_eval_ablation_{name}.yaml"
        ...
        for name, overrides in _ablation_presets_filtered(args.presets):
            ...
            out_rel = f"outputs/try72_ablation_{name}"
            cfg.setdefault("runtime", {})["output_dir"] = out_rel

            tmp_yaml = staging / f"try72_ablation_{name}.yaml"
            with tmp_yaml.open("w", encoding="utf-8") as f:
                yaml.safe_dump(cfg, f, sort_keys=False, allow_unicode=True)
