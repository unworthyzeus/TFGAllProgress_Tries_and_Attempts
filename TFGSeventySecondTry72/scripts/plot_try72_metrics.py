"""Plot Try 72 training metrics from validate_metrics_epoch_*.json files.

Reads the Try 72 JSON schema (single-stage 513x513, no no-data head) and
generates multi-panel PNG plots per expert.

JSON schema (from build_validation_payload):
  metrics.path_loss          .rmse_physical / .mae_physical (sparse receivers when enabled)
  metrics.path_loss_dense_reference .rmse_physical (all valid pixels; Try 72+)
...
Usage:
  python scripts/plot_try72_metrics.py
  python scripts/plot_try72_metrics.py --root D:/cluster_outputs/TFGSeventySecondTry72
  python scripts/plot_try72_metrics.py --expert-id open_sparse_lowrise
"""
...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGSeventySecondTry72"
EXPERT_PREFIX = "seventysecond_try72_expert_"
...
def _expert_key(path: Path) -> str:
    ...
    for prefix in ("seventysecond_try72_expert_", "sixtysixth_expert_", "try72_expert_", "seventysecond_try72_"):
...
def load_rows(output_dir: Path) -> list[dict[str, Any]]:
    ...
    for path in json_files:
        ...
        rows.append({
            ...
            "val_rmse_dense": _f(_nested(data, "metrics.path_loss_dense_reference.rmse_physical")),
            "sparse_over_dense": _f(_nested(data, "metrics.path_loss.receiver_subsample_sparse_over_dense_pixels")),
            # --- train metrics ---
...
def plot_expert(expert_id: str, output_dir: Path, save_path: Path | None = None) -> dict[str, Any]:
    ...
    val_rmse_dense = [r["val_rmse_dense"] for r in rows]

    ax.plot(epochs, val_rmse, label="val RMSE (sparse)", color="#0b7285", linewidth=2.2)
    if any(math.isfinite(v) for v in val_rmse_dense):
        ax.plot(epochs, val_rmse_dense, label="val RMSE dense ref", color="#7048e8", linestyle=":", linewidth=2.0)
    ...
    ax.set_title(f"Try 72 — {topo} — Overall RMSE" + (" [EMA for val]" if uses_ema else ""))
    ax.grid(alpha=0.25)
    ...
    resolved = save_path or output_dir / "metrics_plot_try72.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 72 single-stage metrics per expert.")
    ...
    for d in dirs:
        eid = _expert_key(d)
        save = Path(args.save_path) / f"{eid}_metrics_try72.png" if args.save_path else None
        try:
