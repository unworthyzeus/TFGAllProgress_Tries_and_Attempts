...
def build_validation_payload(
    ...
    *,
    ...
    if "path_loss_dense_reference" in summary:
        payload["metrics"]["path_loss_dense_reference"] = dict(summary["path_loss_dense_reference"])
    if "path_loss__prior__dense_reference" in summary:
        payload["metrics"]["path_loss__prior__dense_reference"] = dict(summary["path_loss__prior__dense_reference"])
...
def evaluate_validation(
    ...
    *,
    ...
    totals_dense = init_metric_totals()
    totals_dense_quantized = init_metric_totals()
    ...
    prior_totals_dense = init_metric_totals()
    prior_totals_dense_quantized = init_metric_totals()
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(loader, desc="val", leave=False, disable=distributed and not is_main_process(rank))):
            ...
            valid_full = m[:, :1] > 0.0
            _rs_cfg = dict(cfg.get("data", {}).get("receiver_subsample", {}) or {})
            if bool(_rs_cfg.get("enabled", False)):
                _vseed = int(_rs_cfg.get("val_seed", int(cfg.get("seed", 42)))) + 902501
                gate = apply_receiver_subsample_mask(valid_full.to(dtype=m.dtype), cfg["data"], seed=_vseed)
                valid_mask = valid_full & (gate > 0.5)
            else:
            ...
            if bool(_rs_cfg.get("enabled", False)):
                diff_dense = (pred_phys - target_phys)[valid_full]
                update_metric_total(totals_dense, diff_dense)
                update_metric_total_quantized_u8(totals_dense_quantized, pred_phys, target_phys, valid_full)
            ...
            if use_formula_prior:
                ...
                if bool(_rs_cfg.get("enabled", False)):
                    prior_diff_dense = (prior_phys - target_phys)[valid_full]
                    update_metric_total(prior_totals_dense, prior_diff_dense)
                    update_metric_total_quantized_u8(
                        prior_totals_dense_quantized, prior_phys, target_phys, valid_full
                    )
    ...
    if distributed and dist.is_initialized():
        payload = {
            ...
            "totals_dense": totals_dense,
            "totals_dense_quantized": totals_dense_quantized,
            ...
            "prior_totals_dense": prior_totals_dense,
            "prior_totals_dense_quantized": prior_totals_dense_quantized,
        ...
        for part in gathered:
            ...
            for k, v in part.get("totals_dense", {}).items():
                totals_dense[k] += float(v)
            for k, v in part.get("totals_dense_quantized", {}).items():
                totals_dense_quantized[k] += float(v)
            ...
            for k, v in part.get("prior_totals_dense", {}).items():
                prior_totals_dense[k] += float(v)
            for k, v in part.get("prior_totals_dense_quantized", {}).items():
                prior_totals_dense_quantized[k] += float(v)
    ...
    rs_done = dict(cfg.get("data", {}).get("receiver_subsample", {}) or {})
    dense_pixel_total = float(totals_dense.get("count", 0.0))
    path_loss_dense_reference: Optional[Dict[str, float]] = None
    prior_dense_reference: Optional[Dict[str, float]] = None
    if bool(rs_done.get("enabled", False)) and dense_pixel_total > 0.0:
        path_loss_dense_reference = finalize_metric_total(totals_dense, unit, total_count=dense_pixel_total)
        attach_quantized_metric_fields(
            path_loss_dense_reference, totals_dense_quantized, unit, total_count=dense_pixel_total
        )
        path_loss_summary["receiver_subsample_sparse_over_dense_pixels"] = float(total_count / dense_pixel_total)
        path_loss_summary["receiver_subsample_keep_fraction_cfg"] = float(rs_done.get("keep_fraction", 1.0))
        if use_formula_prior:
            prior_dense_reference = finalize_metric_total(prior_totals_dense, unit, total_count=dense_pixel_total)
            attach_quantized_metric_fields(
                prior_dense_reference,
                prior_totals_dense_quantized,
                unit,
                total_count=dense_pixel_total,
            )
    summary: dict[str, Any] = {
    ...
    if path_loss_dense_reference is not None:
        summary["path_loss_dense_reference"] = path_loss_dense_reference
        summary["_support"]["valid_pixel_count_dense_reference"] = int(round(dense_pixel_total))
    if no_data_enabled:
    ...
    if use_formula_prior:
        ...
        if prior_dense_reference is not None:
            summary["path_loss__prior__dense_reference"] = prior_dense_reference
...
def train_one_epoch(
    ...
    *,
    ...
    for batch in tqdm(loader, desc="train", leave=False, disable=distributed and not is_main_process(rank)):
        ...
        _rs_cfg = dict(cfg.get("data", {}).get("receiver_subsample", {}) or {})
        if bool(_rs_cfg.get("enabled", False)):
            base_gate = (mask > 0).to(dtype=mask.dtype)
            _step_seed = int(cfg.get("seed", 42)) + int(epoch or 0) * 1_000_003 + int(steps) * 47
            gate = apply_receiver_subsample_mask(base_gate, cfg["data"], seed=_step_seed)
            mask = mask * gate
