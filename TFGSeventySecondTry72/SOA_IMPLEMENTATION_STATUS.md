# Try 72 — SOA claims vs. what is implemented

> Companion to **`SOA_COMPARISON.md`**. This file tracks which *thesis / SOA narrative* items are **already in the Try 72 codebase** versus **not implemented, partial, or config-only**.  
...
| **Topology-partitioned experts** (6 classes + registry) | **Implemented** | `try72_expert_registry.yaml`, `partition_filter.topology_class`, separate checkpoints |
...
| **Topology classifier** (Try 72) | **Separate script** | `train_topology_classifier.py` + `try72_topology_classifier.yaml` — not the same run as path-loss experts |
...
| **Test-time adaptation / calibration** (cf. RadioPiT, AIRMap-style) | **Not implemented** | Explicitly “No” in SOA fair-comparison table for Try 72 |
...
These are cited in **`SOA_COMPARISON.md` §7** as *future* ideas — **none** are integrated as first-class Try 72 features today:
