...
def main() -> None:
    parser = argparse.ArgumentParser(description="Reevaluate Try 14 checkpoint using the current building mask.")
    parser.add_argument(
        "--config",
        default=(
    ...
    parser.add_argument(
        ...
        default=(
            ...
            r"blend_db_tinygan_batchnorm_lowresdisc_fourteenthtry14_los_film_ddp2_t14_los_film\\best_cgan.pt"
        ),
    )
    parser.add_argument(
        "--dataset",
    ...
    try14_root = repo_root / "TFGFourteenthTry14"
    mask_root = repo_root / "TFGThirtyThirdTry33"
    if str(mask_root) not in sys.path:
        sys.path.insert(0, str(mask_root))
    if str(try14_root) not in sys.path:
        sys.path.insert(0, str(try14_root))

    config_utils = load_module("t14_config_utils", try14_root / "config_utils.py")
    model_cgan = load_module("t14_model_cgan", try14_root / "model_cgan.py")
