...
class Stage2Teacher:
    def __init__(self, cfg: Dict[str, Any], stage2_cfg: Dict[str, Any], checkpoint_path: str, device: object) -> None:
        ...
        stage1_checkpoint_path = resolve_teacher_checkpoint(stage1_checkpoint_raw, try_root)
...
def _expand_dim_repeat(tensor: torch.Tensor, target_size: int, dim: int) -> torch.Tensor:
    src_size = tensor.shape[dim]
    if src_size == target_size:
        return tensor.clone()
    if src_size > target_size:
        index = [slice(None)] * tensor.ndim
        index[dim] = slice(0, target_size)
    ...
    while remain > 0:
        ...
        index = [slice(None)] * tensor.ndim
        index[dim] = slice(start, start + take)
        chunks.append(tensor[tuple(index)].clone())
        remain -= take
        start = (start + take) % src_size
    return torch.cat(chunks, dim=dim)


def _adapt_tensor_to_shape(src_tensor: torch.Tensor, dst_shape: torch.Size) -> torch.Tensor:
    out = src_tensor.clone()
    if out.ndim == 1 and out.shape[0] != dst_shape[0]:
        out = _expand_dim_repeat(out, dst_shape[0], 0)
    elif out.ndim == 2:
        if out.shape[0] != dst_shape[0]:
            out = _expand_dim_repeat(out, dst_shape[0], 0)
        if out.shape[1] != dst_shape[1]:
            out = _expand_dim_repeat(out, dst_shape[1], 1)
    elif out.ndim == 4:
        if out.shape[0] != dst_shape[0]:
            out = _expand_dim_repeat(out, dst_shape[0], 0)
        if out.shape[1] != dst_shape[1]:
...
def _adapt_model_state_dict(
    ...
    for key, dst_tensor in dst_state.items():
        ...
        if tuple(src_tensor.shape) == tuple(dst_tensor.shape):
            ...
            new_state[key] = _adapt_tensor_to_shape(src_tensor, dst_tensor.shape)
            adapted += 1
    return new_state, copied, adapted
...
def main() -> None:
    ...
    if resume_path and resume_path.exists():
        ...
        source_model_state = state["model"] if "model" in state else state["generator"]
        resume_was_adapted = False
        copied_params = 0
        adapted_params = 0
        try:
            target_model.load_state_dict(source_model_state)
        except RuntimeError:
            adapted_state, copied_params, adapted_params = _adapt_model_state_dict(
                source_model_state,
                target_model.state_dict(),
            )
            target_model.load_state_dict(adapted_state, strict=True)
            resume_was_adapted = True
            if is_main_process(rank):
                adapted_resume_path = out_dir / f"adapted_{resume_path.name}"
                adapted_payload = dict(state)
                adapted_payload["model"] = target_model.state_dict()
                if "generator" in adapted_payload:
                    adapted_payload["generator"] = target_model.state_dict()
                adapted_payload["auto_adapted_from"] = str(resume_path)
                adapted_payload["auto_adapted_note"] = f"copied={copied_params}, adapted={adapted_params}"
                torch.save(adapted_payload, adapted_resume_path)
        if not resume_was_adapted and "optimizer" in state:
            optimizer.load_state_dict(state["optimizer"])
            move_optimizer_state_to_device(optimizer, device)
        if not resume_was_adapted and "scheduler" in state and scheduler is not None:
            scheduler.load_state_dict(state["scheduler"])
        if not resume_was_adapted and "scaler" in state:
            scaler.load_state_dict(state["scaler"])
        start_epoch = int(state.get("epoch", 0)) + 1
        best_score = float(state.get("best_score", best_score))
        best_epoch = int(state.get("best_epoch", best_epoch))
        if is_main_process(rank):
            payload = {"resume_from": str(resume_path), "start_epoch": start_epoch}
            if resume_was_adapted:
                payload["resume_auto_adapted"] = True
                payload["resume_params_copied"] = copied_params
                payload["resume_params_adapted"] = adapted_params
