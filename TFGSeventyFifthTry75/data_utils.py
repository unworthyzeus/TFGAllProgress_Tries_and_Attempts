...
def _filter_hdf5_refs_by_partition(
    ...
    *,
    ...
    antenna_height_min_m: Optional[float] = None,
    antenna_height_max_m: Optional[float] = None,
) -> List[Tuple[str, str]]:
    ...
    if (
        ...
        and not antenna_bin
        and not city_type
        and antenna_height_min_m is None
        and antenna_height_max_m is None
    ):
        return refs
    ...
    with h5py.File(hdf5_path, "r") as handle:
        for city, sample in refs:
            ...
            if city_type and meta["city_type"] != city_type:
                continue
            if antenna_height_min_m is not None and antenna_height_m < float(antenna_height_min_m):
                continue
            if antenna_height_max_m is not None and antenna_height_m > float(antenna_height_max_m):
                continue
...
def log_scalar_data_report(train_dataset: Dataset, cfg: Dict[str, Any], sample_limit: int = 1000) -> None:
    ...
    data_cfg = cfg['data']
    n_prefix = 1
    if bool(data_cfg.get('los_input_column')) and bool(data_cfg.get('use_los_as_input', True)):
        n_prefix += 1
...
class CKMHDF5Dataset(Dataset):
    def __init__(
        ...
        scalar_feature_norms: Optional[Dict[str, float]] = None,
        los_input_column: Optional[str] = None,
        use_los_as_input: bool = True,
        los_region_mask_mode: Optional[str] = None,
        los_region_mask_threshold: float = 0.5,
        ...
        self.scalar_feature_norms_cfg = scalar_feature_norms or {}
        self.los_input_column = los_input_column
        self.use_los_as_input = bool(use_los_as_input)
        self.los_region_mask_mode = str(los_region_mask_mode or "").strip().lower() or None
        if self.los_region_mask_mode not in (None, "los_only", "nlos_only"):
            raise ValueError(
                f"data.los_region_mask_mode must be null, 'los_only', or 'nlos_only', got {los_region_mask_mode!r}"
            )
        self.los_region_mask_threshold = float(los_region_mask_threshold)
        if self.los_region_mask_mode and not self.los_input_column:
            raise ValueError("data.los_region_mask_mode requires data.los_input_column to load the LoS mask")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.use_los_as_input and los_input_tensor is not None:
        ...
        if 'path_loss' in self.target_columns:
            ...
            if los_input_tensor is not None and self.los_region_mask_mode:
                los_valid = (los_input_tensor.squeeze(0) > self.los_region_mask_threshold).float()
                if self.los_region_mask_mode == "nlos_only":
                    los_valid = 1.0 - los_valid
                mask_tensor[path_loss_idx] = mask_tensor[path_loss_idx] * los_valid
            ...
            # Mark masked-out path-loss pixels as NaN so excluded regions are not
            # semantically seen as a real "0 dB" target by downstream consumers.
            target_tensor[path_loss_idx] = torch.where(
                mask_tensor[path_loss_idx] > 0.0,
                target_tensor[path_loss_idx],
                torch.full_like(target_tensor[path_loss_idx], float("nan")),
            )
        if self.return_scalar_cond and scalar_values:
            cond = torch.tensor(scalar_values, dtype=torch.float32)
            return model_input, target_tensor, mask_tensor, cond
...
def build_dataset_splits_from_config(cfg: Dict[str, Any]) -> Dict[str, Dataset]:
    ...
    antenna_height_min_raw = partition_filter.get("antenna_height_m_min")
    antenna_height_max_raw = partition_filter.get("antenna_height_m_max")
    antenna_height_min = None if antenna_height_min_raw in (None, "", "null") else float(antenna_height_min_raw)
    antenna_height_max = None if antenna_height_max_raw in (None, "", "null") else float(antenna_height_max_raw)
    if topology_class_filter or antenna_bin_filter or city_type_filter or antenna_height_min is not None or antenna_height_max is not None:
        ...
        sample_refs = _filter_hdf5_refs_by_partition(
            ...
            antenna_height_min_m=antenna_height_min,
            antenna_height_max_m=antenna_height_max,
        ...
        print(
            ...
            f"antenna_height_m_min={antenna_height_min!r} antenna_height_m_max={antenna_height_max!r} "
    ...
    common_hdf5 = dict(
        ...
        scalar_feature_norms=dict(data_cfg.get('scalar_feature_norms', {})),
        los_input_column=data_cfg.get('los_input_column'),
        use_los_as_input=bool(data_cfg.get('use_los_as_input', True)),
        los_region_mask_mode=data_cfg.get('los_region_mask_mode'),
        los_region_mask_threshold=float(data_cfg.get('los_region_mask_threshold', 0.5)),
        input_column=str(data_cfg.get('input_column', 'topology_map')),
