#!/usr/bin/env python3
"""Submit chained 1-GPU Slurm jobs for Try73 experts (train -> cleanup -> …).

Same registry as ``submit_try73_experts_4gpu_sequential.py`` but uses
``cluster/run_seventythird_try73_1gpu.slurm``.
"""
...
TRAIN_SLURM = "cluster/run_seventythird_try73_1gpu.slurm"
CLEANUP_SLURM = "cluster/run_seventythird_try73_cleanup_sert2001_1gpu.slurm"
...
def sbatch_afterany_prefix(flag_value: str) -> str:
    ...
    if bad:
        raise SystemExit(
            ...
            "Omit the flag for an independent Try73 chain."
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Submit sequential Try73 1-GPU jobs (train + cleanup).")
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            job_name = f"t73-1gpu-{expert_id.replace('_', '-')[:24]}"
            exports = [
            ...
            cleanup_name = f"t73-cleanup-{expert_id.replace('_', '-')[:18]}"
            cleanup_cmd = (
        ...
        if "cleanup" in _n:
            print(
                f"\nLAST_TRY68_CLEANUP_JOB_ID={last_id}\n"
