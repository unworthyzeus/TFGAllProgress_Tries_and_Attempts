#!/usr/bin/env python3
"""Submit Try75 experts as parallel 1-GPU Slurm jobs (no sequential dependency chain).

Each train job uses cluster/run_seventyfifth_try75_1gpu.slurm.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit parallel Try75 1-GPU jobs (one Slurm job per expert, same time)."
    )
    parser.add_argument(
        "expert_ids",
        nargs="*",
    ...
    try:
        ...
        for i, row in enumerate(rows):
            ...
            train_cmd = (
                ...
                "cluster/run_seventyfifth_try75_1gpu.slurm"
            )
