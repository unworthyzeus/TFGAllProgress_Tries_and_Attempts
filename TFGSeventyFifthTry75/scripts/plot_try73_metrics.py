"""Plot Try 75 training metrics from validate_metrics_epoch_*.json files.

Reads the Try 75 JSON schema (single-stage 513x513, no no-data head) and
...
Usage:
  python scripts/plot_try75_metrics.py
  python scripts/plot_try73_metrics.py
  python scripts/plot_try75_metrics.py --root D:/cluster_outputs/TFGSeventyFifthTry75
  python scripts/plot_try75_metrics.py --expert-id open_lowrise
"""
...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGSeventyFifthTry75"
EXPERT_PREFIX = "try75_expert_"
...
def plot_expert(expert_id: str, output_dir: Path, save_path: Path | None = None) -> dict[str, Any]:
    ...
    ax.set_title(f"Try 75 — {topo} — Overall RMSE" + (" [EMA for val]" if uses_ema else ""))
    ax.grid(alpha=0.25)
    ...
    resolved = save_path or output_dir / "metrics_plot_try75.png"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 75 single-stage metrics per expert.")
