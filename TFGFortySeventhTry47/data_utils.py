...
def _get_angle_bin_rays(image_size: int, angle_bins: int) -> List[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
    ...
    dy = yy - half
    dx = xx - half
    radius = np.sqrt(dx ** 2 + dy ** 2).astype(np.float32)
    angle = np.arctan2(dy, dx)
    bin_ids = np.floor(((angle + math.pi) / (2.0 * math.pi)) * float(angle_bins)).astype(np.int32)
    bin_ids = np.mod(bin_ids, int(angle_bins))
...
def return_regime_meta_from_config(cfg: Dict[str, Any]) -> bool:
    return bool(cfg.get('training', {}).get('return_regime_meta', False))
...
class CKMHDF5Dataset(Dataset):
    def __init__(
        ...
        self.return_regime_meta = bool(return_regime_meta)
        ...
        if self.obstruction_precomputed_hdf5 is not None and not self.obstruction_precomputed_hdf5.exists():
            raise FileNotFoundError(f"Precomputed obstruction HDF5 not found: {self.obstruction_precomputed_hdf5}")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.return_scalar_cond and scalar_values:
            cond = torch.tensor(scalar_values, dtype=torch.float32)
            if self.return_regime_meta:
                city_type = dict(self.formula_regime_calibration.get("city_type_by_city", {})).get(city) if self.formula_regime_calibration else None
                if city_type is None:
                    non_zero = raw_input[raw_input != self.non_ground_threshold]
                    city_type = _city_type_from_thresholds(
                        float(np.mean(raw_input != self.non_ground_threshold)),
                        float(np.mean(non_zero)) if non_zero.size else 0.0,
                        dict((self.formula_regime_calibration or {}).get("city_type_thresholds", {})),
                    )
                antenna_bin = _antenna_height_bin(
                    float(antenna_height_m),
                    dict((self.formula_regime_calibration or {}).get("antenna_height_thresholds", {})),
                )
                meta = {
                    'city': city,
                    'sample': sample,
                ...
                return model_input, target_tensor, mask_tensor, cond, meta
            return model_input, target_tensor, mask_tensor, cond
        if self.return_regime_meta:
            city_type = dict(self.formula_regime_calibration.get("city_type_by_city", {})).get(city) if self.formula_regime_calibration else None
            if city_type is None:
            ...
            return model_input, target_tensor, mask_tensor, meta
        return model_input, target_tensor, mask_tensor
...
def build_dataset_splits_from_config(cfg: Dict[str, Any]) -> Dict[str, Dataset]:
    ...
    common_hdf5 = dict(
        ...
        add_scalar_channels=add_scalar_channels_from_config(cfg),
        return_scalar_cond=return_scalar_cond_from_config(cfg),
        return_regime_meta=return_regime_meta_from_config(cfg),
        scalar_feature_columns=list(data_cfg.get('scalar_feature_columns', [])),
        constant_scalar_features=dict(data_cfg.get('constant_scalar_features', {})),
        scalar_feature_norms=dict(data_cfg.get('scalar_feature_norms', {})),
        los_input_column=data_cfg.get('los_input_column'),
