...
def compute_fused_path_loss_mse_differentiable(
    outputs: torch.Tensor,
    targets: torch.Tensor,
    masks: torch.Tensor,
    target_columns: List[str],
    ...
    """
    MSE on the fused path-loss map in dB (differentiable).
    Used when path_loss_hybrid is enabled to align training loss with path_loss_rmse_physical.
    """
    if not is_path_loss_hybrid_enabled(cfg) or 'path_loss' not in target_columns:
    ...
    conf = torch.sigmoid(confidence_logits).clamp(0.0, 1.0)
    ...
    los_correction = bool(post_cfg.get('path_loss_los_correction', False))
    freq_ghz = float(post_cfg.get('path_loss_los_frequency_ghz', 7.125))
    los_blend = float(post_cfg.get('path_loss_los_blend_weight', 0.3))

    los_mask = None
    if cfg.get('data', {}).get('los_input_column') and model_input.size(1) >= 2:
        los_mask = model_input[:, 1:2]

    heuristic = _build_heuristic_prior_differentiable(
        pred_db,
        path_meta,
        kernel_size=kernel_size,
        los_mask=los_mask,
        los_correction_enabled=los_correction,
        frequency_ghz=freq_ghz,
        blend_weight=los_blend,
    )

    fallback_mode = str(hybrid_cfg.get('fallback_mode', 'blend'))
    if fallback_mode == 'blend':
        fused = conf * pred_db + (1.0 - conf) * heuristic
    else:
        thresh = float(hybrid_cfg.get('fallback_threshold', 0.5))
        fused = torch.where(conf >= thresh, pred_db, heuristic)

    diff = fused - target_db
    squared = diff ** 2
        ...
        for x, y, m in tqdm(
            ...
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                if is_path_loss_hybrid_enabled(cfg) and 'path_loss' in target_columns:
                    ...
                    fused_path_loss_mse = compute_fused_path_loss_mse_differentiable(
                        fake, y, m, target_columns, target_metadata, cfg, x,
                    )
                    n_total = len(target_columns)
                    n_other = n_total - 1
                    if n_other <= 0:
                        recon_loss = fused_path_loss_mse
                    else:
                        recon_loss = (recon_other * n_other + fused_path_loss_mse) / n_total
                else:
