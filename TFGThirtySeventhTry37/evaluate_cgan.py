    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc='eval_cgan', leave=False):
            ...
            if 'path_loss' in target_columns:
                ...
                valid_mask_np = masks[:, path_loss_index : path_loss_index + 1].detach().cpu().numpy()
                ...
                for batch_idx in range(pred_phys.shape[0]):
                    valid_mask = valid_mask_np[batch_idx, 0]
                    if confidence_np is not None:
                        ...
                        path_loss_preds.append(np.where(valid_mask > 0, np.asarray(fallback_outputs['final_path_loss_db'], dtype=np.float32), np.nan))
                        hybrid_confidence_values.append(float(np.mean(fallback_outputs['confidence_map'])))
                    else:
                        path_loss_preds.append(np.where(valid_mask > 0, np.asarray(pred_phys[batch_idx, 0], dtype=np.float32), np.nan))
                    path_loss_targets.append(np.where(valid_mask > 0, np.asarray(tgt_phys[batch_idx, 0], dtype=np.float32), np.nan))
