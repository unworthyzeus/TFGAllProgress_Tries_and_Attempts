        ...
        for idx, batch in enumerate(tqdm(loader, desc="val", leave=False)):
            ...
            if los is not None:
                ...
                for label, subtype_mask in [
                    ...
                    update_metric_total(
                        regime_totals[f"path_loss__prior__nlos_shadow__{label}"],
                        (prior_phys - target_phys)[subtype_mask],
                    )
    ...
    for batch in tqdm(loader, desc="train", leave=False):
        ...
        with amp.autocast(device_type="cuda", enabled=amp_enabled):
            ...
            if bool(branch_cfg.get("enabled", True)) and los_mask is not None and hasattr(inner_model, "last_branch_outputs"):
                los_residual_pred, nlos_residual_pred = inner_model.last_branch_outputs()
                if los_residual_pred is not None and nlos_residual_pred is not None:
                    ...
                    total_loss = total_loss + float(branch_cfg.get("los_loss_weight", 0.15)) * los_branch_loss
                    total_loss = total_loss + float(branch_cfg.get("nlos_loss_weight", 0.75)) * nlos_branch_loss
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Try 46 LoS/NLoS-branch PMNet residual path-loss model")
