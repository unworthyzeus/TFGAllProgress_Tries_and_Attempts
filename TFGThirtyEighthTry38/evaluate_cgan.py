...
def _decode_outputs_if_available(
    ...
    inputs: torch.Tensor,
    scalar_cond: torch.Tensor | None,
    target_columns: List[str],
    target_metadata: Dict[str, Dict[str, object]],
    cfg: Dict,
) -> torch.Tensor:
    try:
        if 'train_cgan' in sys.modules:
            del sys.modules['train_cgan']
        train_mod = importlib.import_module('train_cgan')
    except Exception:
...
def is_path_loss_hybrid_enabled(cfg: Dict) -> bool:
    return bool(dict(cfg.get('path_loss_hybrid', {})).get('enabled', False))


def extract_confidence_probs(outputs: torch.Tensor, target_columns: List[str], cfg: Dict) -> torch.Tensor | None:
    if not is_path_loss_hybrid_enabled(cfg):
        return None
    confidence_index = len(target_columns)
    if outputs.size(1) <= confidence_index:
        return None
    return torch.sigmoid(outputs[:, confidence_index : confidence_index + 1])


def overwrite_hybrid_path_loss_summary(
...
def _linear_normalized_to_power_ratio(normalized: torch.Tensor) -> torch.Tensor:
    ...
    n = normalized.to(dtype=torch.float32).clamp(0.0, 1.0)
    log_linear = n * 18.0 - 18.0
    return torch.pow(10.0, log_linear).clamp(min=1e-18)


def _path_loss_db_to_power_ratio(db_values: torch.Tensor) -> torch.Tensor:
    values = db_values.to(dtype=torch.float32)
    return torch.pow(10.0, -values / 10.0).clamp(min=1e-18, max=1.0)
...
def update_metric_totals(
    ...
    for i, name in enumerate(target_columns):
        ...
        if metadata and loss_name != 'bce':
            ...
            if name == 'path_loss':
                pred_lin = _path_loss_db_to_power_ratio(pred_phys)
                tgt_lin = _path_loss_db_to_power_ratio(tgt_phys)
...
def summarize_loader(
    ...
    hybrid_confidence_values: List[float] = []
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc='eval_cgan', leave=False):
            ...
            primary_outputs = _decode_outputs_if_available(outputs, inputs, sc, target_columns, target_metadata, cfg)

            if 'path_loss' in target_columns:
                path_loss_index = target_columns.index('path_loss')
                pred = primary_outputs[:, path_loss_index : path_loss_index + 1]
                tgt = targets[:, path_loss_index : path_loss_index + 1]
                msk = masks[:, path_loss_index : path_loss_index + 1]
                ...
                mask_np = (msk.detach().cpu().numpy() > 0.5)
                confidence_probs = extract_confidence_probs(outputs, target_columns, cfg)
                confidence_np = confidence_probs.detach().cpu().numpy() if confidence_probs is not None else None
                los_mask_batch = None
                if cfg.get('data', {}).get('los_input_column'):
                    los_mask_batch = inputs[:, 1:2].detach().cpu().numpy()
                for batch_idx in range(pred_phys.shape[0]):
                    if confidence_np is not None:
                        fallback_outputs = apply_path_loss_confidence_fallback(
                            pred_phys[batch_idx, 0],
                            confidence_np[batch_idx, 0],
                            metadata,
                            confidence_threshold=float(dict(cfg.get('path_loss_hybrid', {})).get('fallback_threshold', 0.5)),
                            fallback_mode=str(dict(cfg.get('path_loss_hybrid', {})).get('fallback_mode', 'replace')),
                        ...
                        pred_map = np.asarray(fallback_outputs['final_path_loss_db'], dtype=np.float32)
                        pred_map = np.where(mask_np[batch_idx, 0], pred_map, np.nan)
                        path_loss_preds.append(pred_map)
                        hybrid_confidence_values.append(float(np.mean(fallback_outputs['confidence_map'])))
                    else:
                        pred_map = np.asarray(pred_phys[batch_idx, 0], dtype=np.float32)
                        pred_map = np.where(mask_np[batch_idx, 0], pred_map, np.nan)
                        path_loss_preds.append(pred_map)
                    tgt_map = np.asarray(tgt_phys[batch_idx, 0], dtype=np.float32)
                    tgt_map = np.where(mask_np[batch_idx, 0], tgt_map, np.nan)
                    path_loss_targets.append(tgt_map)

            update_metric_totals(totals, primary_outputs, targets, masks, target_columns, target_losses, target_metadata)

    summary = finalize_metric_totals(totals, target_columns, target_losses, target_metadata)
    overwrite_hybrid_path_loss_summary(summary, path_loss_preds, path_loss_targets)
    if hybrid_confidence_values:
        summary.setdefault('_hybrid', {})
        summary['_hybrid']['confidence_mean'] = float(np.mean(hybrid_confidence_values))
...
def main() -> None:
    ...
    if args.split == 'both':
        ...
        try:
            ...
            summary['test'] = summarize_loader(generator, test_loader, device, cfg, target_columns, target_losses, target_metadata, amp_enabled)
        except ValueError:
            pass
    else:
