...
LOCAL_DIR = ROOT / "TFGSixtySecondTry62"
REMOTE_DIR = "/scratch/nas/3/gmoreno/TFGpractice/TFGSixtySecondTry62"
HOST = "sert-entry-4"
USER = "gmoreno"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upload Try62 and submit stage1->stage2 paper-like expert chain on 4 GPUs.")
    ...
    parser.add_argument("--wipe-try62-outputs", action="store_true")
    ...
    stage1_registry = load_registry(
        LOCAL_DIR / "experiments" / "sixtysecondtry62_partitioned_stage1" / "sixtysecondtry62_expert_registry.yaml"
    )
    stage2_registry = load_registry(
        LOCAL_DIR / "experiments" / "sixtysecondtry62_tail_refiner_stage2" / "sixtysecondtry62_tail_refiner_registry.yaml"
    )
    ...
    try:
        ...
        remote_exec(
            ...
            (
                f"ssh -T {args.node} "
                "\""
                "pkill -9 -u gmoreno -f 'torchrun.*TFGSixtySecondTry62' || true; "
        ...
        for idx, expert in enumerate(stage1_registry):
            ...
            stage1_job_name = f"ckm-t62-s1-{topology_class.replace('_', '-')[:32]}"
            stage1_exports = [
        ...
        if not args.stage1_only:
            ...
            for expert in stage1_registry:
                ...
                stage2_job_name = f"ckm-t62-s2-{topology_class.replace('_', '-')[:32]}"
                stage2_exports = [
