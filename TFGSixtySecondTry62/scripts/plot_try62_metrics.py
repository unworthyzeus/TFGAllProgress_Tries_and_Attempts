...
DEFAULT_ROOT = REPO_ROOT / "cluster_outputs" / "TFGSixtySecondTry62"
...
def _expert_key_from_stage1_dir(path: Path) -> str:
    name = path.name
    prefix = "sixtysecondtry62_expert_"
    return name[len(prefix):] if name.startswith(prefix) else name


def _expert_key_from_stage2_dir(path: Path) -> str:
    name = path.name
    prefix = "sixtysecondtry62_tail_refiner_"
    return name[len(prefix):] if name.startswith(prefix) else name
...
def plot_expert_pair(
    ...
    s1_val_rmse = [r["val_rmse"] for r in stage1_rows]
    ...
    s1_best_idx = min(range(len(stage1_rows)), key=lambda i: stage1_rows[i]["val_rmse"])
    ...
    axes[0].plot(s1_epochs, s1_val_rmse, label="stage1 val RMSE", color="#0b7285", linewidth=2.0)
    ...
    axes[0].set_title(f"Try 62 two-stage metrics - {topo}")
    ...
    resolved_save_path = save_path or stage1_dir / "metrics_plot_try62_twostage.png"
    ...
    return {
        ...
        "stage1_best_val_rmse": stage1_rows[s1_best_idx]["val_rmse"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Plot Try 62 two-stage metrics by pairing stage1 and tail-refiner outputs.")
    ...
    parser.add_argument("--root-dir", default=str(DEFAULT_ROOT), help="Root containing downloaded Try62 output folders.")
