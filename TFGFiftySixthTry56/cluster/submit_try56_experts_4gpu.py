...
LOCAL_DIR = ROOT / "TFGFiftySixthTry56"
REMOTE_DIR = "/scratch/nas/3/gmoreno/TFGpractice/TFGFiftySixthTry56"
HOST = "sert-entry-3"
USER = "gmoreno"
...
def load_registry() -> list[dict[str, Any]]:
    registry_path = (
        LOCAL_DIR
        / "experiments"
        / "fiftysixthtry56_topology_experts"
        / "fiftysixthtry56_expert_registry.yaml"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Upload Try56 and submit the 6 topology experts as 4-GPU jobs with cleanup between jobs.")
    ...
    parser.add_argument("--wipe-try56-outputs", action="store_true", default=False)
    ...
    try:
        ...
        if args.wipe_try56_outputs:
            remote_exec(
                client,
                f"find {REMOTE_DIR}/outputs -mindepth 1 -maxdepth 1 -exec rm -rf {{}} + 2>/dev/null || true; "
                f"rm -f {REMOTE_DIR}/logs_try56_expert_* {REMOTE_DIR}/logs_cleanup_fiftysixthtry56_*",
                check=False,
            )
        ...
        initial_cleanup = remote_sbatch(
            client,
            f"{sbatch_prefix} cluster/run_fiftysixthtry56_cleanup_sert2001_1gpu.slurm",
        )
        ...
        for expert in experts:
            ...
            job_id = remote_sbatch(
                ...
                "cluster/run_try56_topology_expert_4gpu.slurm",
            )
            cleanup_after = remote_sbatch(
                client,
                f"{sbatch_prefix} --dependency=afterany:{job_id} cluster/run_fiftysixthtry56_cleanup_sert2001_1gpu.slurm",
            )
            submitted.append(
                {
