...
def main() -> None:
    parser = argparse.ArgumentParser(description="Submit Try56 experts as two 3-job waves of 1-GPU jobs.")
    ...
    parser.add_argument("--wipe-try56-outputs", action="store_true")
    ...
    try:
        ...
        initial_cleanup = remote_sbatch(
            client,
            f"{sbatch_prefix} {cleanup_dependency}cluster/run_fiftysixthtry56_cleanup_sert2001_1gpu.slurm",
        )
        ...
        for idx, expert in enumerate(wave1):
            ...
            job_id = remote_sbatch(
                ...
                "cluster/run_try56_topology_expert_1gpu.slurm",
            )
        ...
        final_cleanup = remote_sbatch(
            client,
            f"{sbatch_prefix} --dependency=afterany:{':'.join(second_wave_job_ids)} "
            "cluster/run_fiftysixthtry56_cleanup_sert2001_1gpu.slurm",
        )
