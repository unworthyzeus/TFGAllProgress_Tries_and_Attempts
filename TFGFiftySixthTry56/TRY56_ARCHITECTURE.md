...
`Try 56` reuses the same base model family as `Try 26`:
- same `UNetGenerator`
...
`Try 56` uses six topology experts instead of one global model. The routing is deterministic and comes from `topology_map` statistics:
- building density
