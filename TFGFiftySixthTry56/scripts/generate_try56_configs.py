...
from data_utils import TRY56_TOPOLOGY_CLASSES
...
OUTPUT_DIR = ROOT / 'experiments' / 'fiftysixthtry56_topology_experts'
...
def _make_expert_cfg(base_cfg: Dict, topology_class: str) -> Dict:
    ...
    cfg['target_columns'] = ['delay_spread', 'angular_spread', 'no_data']
    cfg['target_losses'] = {
        'delay_spread': 'mse',
        'angular_spread': 'mse',
        'no_data': 'bce',
    ...
    cfg['target_metadata'] = {
        'delay_spread': cfg['target_metadata']['delay_spread'],
    ...
    gradient_cfg['enabled'] = False

    runtime_cfg = cfg.setdefault('runtime', {})
    runtime_cfg['output_dir'] = f'outputs/fiftysixthtry56_{topology_class}'
    runtime_cfg['resume_checkpoint'] = None

    training_cfg['selection_metrics'] = {
        'delay_spread.rmse_physical': 1.0,
        'angular_spread.rmse_physical': 1.0,
    }

    cfg.setdefault('experiment', {})
    cfg['experiment'] = {
        'try': '56',
        'family': 'try26_unet_topology_experts',
...
def main() -> None:
    ...
    for topology_class in TRY56_TOPOLOGY_CLASSES:
        ...
        out_path = OUTPUT_DIR / f'fiftysixthtry56_expert_{topology_class}.yaml'
    ...
    registry_path = OUTPUT_DIR / 'fiftysixthtry56_expert_registry.yaml'
    with registry_path.open('w', encoding='utf-8') as handle:
        yaml.safe_dump({'experts': registry}, handle, sort_keys=False)

    print(f'Generated {len(registry)} Try 56 expert configs in {OUTPUT_DIR}')
