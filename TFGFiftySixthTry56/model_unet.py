...
class CKMUNet(nn.Module):
    def __init__(
        ...
        self.down3 = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(bc * 4, bc * 8, dropout=float(dropout_down3), norm_type=self.norm_type),
        )
        self.down4 = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv(bc * 8, bc * 16, dropout=float(dropout_down4), norm_type=self.norm_type),
        )

        self.up1 = UpsampleProject(bc * 16, bc * 8, mode=self.upsample_mode)
        self.conv1 = DoubleConv(bc * 16, bc * 8, dropout=float(dropout_up1), norm_type=self.norm_type)
