...
def _normalize_target_channel_from_metadata(
    channel: torch.Tensor,
    metadata: Dict[str, object],
) -> torch.Tensor:
    physical = denormalize_channel(channel, metadata)
    clip_min = float(metadata.get('clip_min', metadata.get('offset', 0.0)))
    clip_max = float(
        metadata.get(
            'clip_max',
            clip_min + float(metadata.get('scale', 1.0)),
        )
    )
    denom = max(clip_max - clip_min, 1e-6)
    normalized = (physical - clip_min) / denom
    return normalized.clamp(0.0, 1.0)


def compute_value_weighted_regression_loss(
    outputs: torch.Tensor,
    targets: torch.Tensor,
    masks: torch.Tensor,
    target_columns: List[str],
    ...
    weighted_cfg = dict(cfg.get('value_weighted_targets', {}))
    if not bool(weighted_cfg.get('enabled', False)):
        return torch.tensor(0.0, device=outputs.device)

    configured_targets = [str(name) for name in weighted_cfg.get('targets', target_columns)]
    target_weights = {str(name): float(weight) for name, weight in dict(weighted_cfg.get('target_weights', {})).items()}
    loss_weight = float(weighted_cfg.get('loss_weight', 0.0))
    min_valid_ratio = float(weighted_cfg.get('min_valid_ratio', 0.5))
    alpha = float(weighted_cfg.get('alpha', 1.5))
    gamma = float(weighted_cfg.get('gamma', 1.5))
    if loss_weight <= 0.0:
        return torch.tensor(0.0, device=outputs.device)
    ...
    for name in configured_targets:
        ...
        tgt = targets[:, idx : idx + 1]
        msk = masks[:, idx : idx + 1]
        valid = (msk >= min_valid_ratio).float()
        metadata = target_metadata.get(name, {})
        target_norm = _normalize_target_channel_from_metadata(tgt, metadata)
        emphasis = 1.0 + alpha * torch.pow(target_norm, gamma)
        weighted_valid = valid * emphasis
        denom = weighted_valid.sum().clamp_min(1.0)
        channel_loss = (((pred - tgt) ** 2) * weighted_valid).sum() / denom
        total = total + channel_weight * channel_loss
        total_weight += channel_weight
...
def compute_hotspot_regression_loss(
    outputs: torch.Tensor,
    targets: torch.Tensor,
    masks: torch.Tensor,
    target_columns: List[str],
    ...
    hotspot_cfg = dict(cfg.get('hotspot_targets', {}))
    if not bool(hotspot_cfg.get('enabled', False)):
        return torch.tensor(0.0, device=outputs.device)

    configured_targets = [str(name) for name in hotspot_cfg.get('targets', target_columns)]
    target_weights = {str(name): float(weight) for name, weight in dict(hotspot_cfg.get('target_weights', {})).items()}
    loss_weight = float(hotspot_cfg.get('loss_weight', 0.0))
    percentile = float(hotspot_cfg.get('percentile', 0.90))
    min_valid_ratio = float(hotspot_cfg.get('min_valid_ratio', 0.5))
    min_hotspot_fraction = float(hotspot_cfg.get('min_hotspot_fraction', 0.02))
    if loss_weight <= 0.0:
        return torch.tensor(0.0, device=outputs.device)
    ...
    for name in configured_targets:
        ...
        metadata = target_metadata.get(name, {})
        target_norm = _normalize_target_channel_from_metadata(tgt, metadata)
        hotspot_mask = torch.zeros_like(valid)

        for batch_idx in range(target_norm.size(0)):
            valid_values = target_norm[batch_idx][valid[batch_idx] > 0.0]
            if valid_values.numel() == 0:
                continue
            threshold = torch.quantile(valid_values, percentile)
            sample_mask = ((target_norm[batch_idx] >= threshold).float() * valid[batch_idx]).float()
            hotspot_pixels = sample_mask.sum()
            min_pixels = max(int(min_hotspot_fraction * float(valid[batch_idx].sum().item())), 1)
            if hotspot_pixels.item() < min_pixels:
                flat_values = target_norm[batch_idx].reshape(-1)
                flat_valid = valid[batch_idx].reshape(-1)
                candidate_indices = torch.nonzero(flat_valid > 0.0, as_tuple=False).squeeze(1)
                if candidate_indices.numel() == 0:
                    continue
                candidate_values = flat_values[candidate_indices]
                top_k = min(max(min_pixels, 1), candidate_indices.numel())
                _, top_positions = torch.topk(candidate_values, k=top_k, largest=True, sorted=False)
                chosen = candidate_indices[top_positions]
                flat_mask = torch.zeros_like(flat_values)
                flat_mask[chosen] = 1.0
                sample_mask = flat_mask.reshape_as(valid[batch_idx]) * valid[batch_idx]
            hotspot_mask[batch_idx] = sample_mask

        denom = hotspot_mask.sum().clamp_min(1.0)
        channel_loss = (((pred - tgt) ** 2) * hotspot_mask).sum() / denom
        total = total + channel_weight * channel_loss
        total_weight += channel_weight
        ...
        for batch in tqdm(
            ...
            with amp.autocast(device_type='cuda', enabled=amp_enabled):
                ...
                if 'path_loss' in target_columns:
                    ...
                    recon_loss = recon_loss + compute_value_weighted_regression_loss(
                        fake, y, m, target_columns, target_losses, target_metadata, cfg,
                    )
                    recon_loss = recon_loss + compute_hotspot_regression_loss(
                        fake, y, m, target_columns, target_losses, target_metadata, cfg,
                    )
